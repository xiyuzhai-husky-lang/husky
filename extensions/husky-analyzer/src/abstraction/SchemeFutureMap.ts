...
class SchemeFutureMap<T> {
    ...
    get_store(
        ...
        if (opt_input_id === null) {
            ...
            if (opt_input_id in values) {
                return values[opt_input_id];
            } else {
                make_request();
                return (values[opt_input_id] = writable(null));
            }
        }
    }

    has(trace_id: number, input_id: number | null): boolean {
        if (input_id === null) {
            return trace_id in this.generic_points;
        } else {
            let values: { [input_id: number]: Writable<T | null> };
            if (trace_id in this.closed_points) {
                values = this.closed_points[trace_id];
