...
export type RustDocument = vscode.TextDocument & { languageId: "husky" };
...
export function isRustDocument(
    ...
    return document.languageId === "husky" && document.uri.scheme === "file";
}
...
export function isValidExecutable(path: string): boolean {
    ...
    const res = child_process.spawnSync(path, ["--version"], {
        encoding: "utf8",
    });

    const printOutput =
        res.error && (res.error as any).code !== "ENOENT"
            ? log.warn
            : log.debug;
