...
pub(crate) fn is_convex<'eval>(line_segment_sketch: &crate::line_segment_sketch::LineSegmentSketch<'eval>, index: i32) -> bool {
    ...
    let current_displacement = line_segment_sketch.line_segments[(index.rem_euclid(L)) as usize].displacement();
    let previous_displacement = line_segment_sketch.line_segments[((index - 1).rem_euclid(L)) as usize].displacement();
    ...
    if is_rotation_counterclockwise_result == 0 {
        ...
        let previous_interval = &line_segment_sketch.line_segments[((index - 1).rem_euclid(L)) as usize].points;
        ...
        let current_interval = &line_segment_sketch.line_segments[(index.rem_euclid(L)) as usize].points;
