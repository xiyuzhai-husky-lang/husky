...
pub fn parse_ovld_matchers(
    ...
    let mut ovld_matchers = HashMap::<*const Scope, OvldMatcher>::new();
            ...
            Binary::Combine => {
                ...
                if tmpl_arguments.len() == 0 {
                    let element = throw_on_error!(
                        resolve_scope(ropd, module, root),
            ...
            Keyword::Func | Keyword::PubFunc => {
                let func = match &sentence_group[1].clause.phrase.variant {
                    ...
                    PhraseVariant::Binary { opr, lopd, ropd } => {
                        ...
                        throw_on_error!(
                            resolve_scope(lopd, module, root),
                ...
                let (inputs, output) = match &sentence_group[0].clause.phrase.variant {
                    PhraseVariant::Binary { opr, lopd, ropd } => (
                        unjoin!(
                            lopd,
                            Join::List,
                            &|phrase: &'repl Phrase| -> Result<&'repl Scope, ParserError> {
                                resolve_scope(phrase, module, root)
                            },
                            "list of types".to_string()
                        ),
                        throw_on_error!(resolve_scope(ropd, module, root), "whatever".to_string()),
                    ),
                    _ => td!(),
                };
                let pfunc: *const semantics::scope::Scope = &*func;
...
fn add_func_ovld(
    clause: &Clause,
    ovld_matchers: &mut OvldMatchers,
    resolve_scope: &Fn(PhraseID) -> &Scope,
) {
        ...
        PhraseVariant::Binary { opr, lopd, ropd } => match opr {
            Binary::Arrow => match &lopd.variant {
                PhraseVariant::Binary {
                    ...
                    let func = resolve_scope(funcname);
                    td!()
                }
                _ => td!(),
            },
            Binary::Call => {
                let func = resolve_scope(lopd);
                td!()
            }
            _ => td!(),
        },
        PhraseVariant::Join { separator, phrases } => {
            // let func = resolve_scope(phrases[0]);
            td!()
        }
        _ => {
            p!(&clause.phrase);
