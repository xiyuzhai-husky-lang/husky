...
fn get_line_group_end(source: &HuskyFile, start: usize) -> usize {
        ...
        file::Indent::Indent(indent) => {
            if indent % 2 != 0 {
                syntax_error!(
                    ...
                    "incorrect indent, first line in a line group must have even indent"
                        .to_string(),
                    IndentError
                )
            } else {
...
pub fn format_parser_error(source: &HuskyFile, e: &ParserError) -> String {
    let mut result: String = "".to_string();
