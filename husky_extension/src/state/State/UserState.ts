...
class UserState {
    ...
    input_id_store: Writable<number | null> = writable(null);
    ...
    did_toggle_expansion(id: number) {
        this.expansion_stores.update(id, (expanded) => !expanded);
    }
    ...
    did_lock_input(
        ...
        if (input_locked_on !== undefined) {
            this.input_id_store.set(input_locked_on);
