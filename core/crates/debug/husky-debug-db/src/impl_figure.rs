...
impl HuskyDebugTime {
        ...
        match focus.opt_input_id {
            Some(input_id) => {
                if let Ok(value) = self.runtime().eval_feature_expr(expr, input_id) {
                    let visualizer = self.runtime().visualizer(expr.expr.ty());
                    let visual_props = visualizer.visualize(
                        self.runtime(),
                        value.any_ref(),
    ...
    fn eager_expr_figure(&self, expr: &EagerExpr, history: &History) -> FigureProps {
        let visualizer = self.runtime().visualizer(expr.ty());
        if let Some(entry) = history.get(expr) {
            match entry {
                            ...
                            .find(|frame_mutation| frame_mutation.varidx() == mutation.varidx())
                        {
                            ...
                            MutationFigureProps {
                                ...
                                after: FigureProps::new_specific(
                                    self.runtime().visualizer(mutation.ty).visualize(
                                        self.runtime(),
