    ...
    pub(crate) fn feature_expr_figure(
        ...
        if let Some(sample_id) = self.restriction.opt_sample_id() {
            ...
            match visualizer.visual_ty {
                ...
                VisualTy::B64 => todo!(),
                VisualTy::I32 => {
                    let mut partitioned_samples_collector = PartitionedSamplesCollector::<i32>::new(
                        self.restriction.partitions().clone(),
                    );
                ...
                VisualTy::F32 => {
                    let mut partitioned_samples_collector = PartitionedSamplesCollector::<f32>::new(
                        self.restriction.partitions().clone(),
                    );
                    for labeled_data in dev_division.each_labeled_data() {
                        let label = labeled_data.label;
