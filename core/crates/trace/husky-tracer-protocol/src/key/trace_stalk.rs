...
pub enum TraceStalkKey {
    ...
    Lazy { sample_id: usize, trace_id: TraceId },
}

impl TraceStalkKey {
    pub fn from_trace_data(sample_id: usize, trace_data: &TraceData) -> Self {
        Self::new(sample_id, trace_data.kind, trace_data.id)
    }
    pub fn from_trace_raw_data(sample_id: usize, trace_raw_data: &TraceRawData) -> Self {
        Self::new(sample_id, trace_raw_data.kind, trace_raw_data.id)
    }

    pub fn new(sample_id: usize, trace_kind: TraceKind, trace_id: TraceId) -> Self {
        match trace_kind {
            TraceKind::Main
