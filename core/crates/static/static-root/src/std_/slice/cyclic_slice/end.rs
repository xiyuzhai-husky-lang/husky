...
pub static STD_SLICE_CYCLIC_SLICE_END_DEFN: EntityStaticDefn = EntityStaticDefn {
    ...
    variant: EntityStaticDefnVariant::TyField {
        ...
        static_linkage_source: &LinkageSource::MemberAccess {
            copy_access: linkage!(cyclic_slice_end_copy_access, 1),
            eval_ref_access: linkage!(|values| todo!(), 1),
            temp_ref_access: linkage!(|values| todo!(), 1),
            temp_mut_access: linkage!(|values| todo!(), 1),
            move_access: linkage!(|values| todo!(), 1),
        },
    },
    dev_src: dev_utils::static_dev_src!(),
};

fn cyclic_slice_end_copy_access<'temp, 'eval>(
