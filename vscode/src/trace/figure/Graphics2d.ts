...
export function decode_graphics2d(data: unknown): Graphics2dProps {
    let image = decode_opt(decode_member(data, "image"), decode_image);
    let shape_groups = decode_array(
        decode_member(data, "shape_groups"),
        decode_shape_group
    );
    let xrange = decode_number_pair(decode_member(data, "xrange"));
    let yrange = decode_number_pair(decode_member(data, "yrange"));
    return { kind: "Graphics2d", image, shape_groups, xrange, yrange };
}

function decode_image(raw: unknown): ImageProps {
    let kind = decode_string(decode_member(raw, "kind"));
    switch (kind) {
        case "Binary28":
            const rows_unknown = decode_member(raw, "rows");
            return {
                kind: "Binary28",
