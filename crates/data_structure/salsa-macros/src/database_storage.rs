...
pub(crate) fn database(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    output.extend(quote! {
        impl salsa::plumbing::SalsaInternalOpns for #database_name {
            ...
            fn opn_salsa_runtime(&self) -> &salsa::SalsaRuntime {
                self.#db_storage_field.get_salsa_runtime()
            }

            fn opn_salsa_runtime_mut(&mut self) -> &mut salsa::SalsaRuntime {
                self.#db_storage_field.get_salsa_runtime_mut()
            }
            ...
            fn for_each_query(
                &self,
                mut op: &mut dyn FnMut(&dyn salsa::plumbing::WholeQueryStorageTrait),
            ) {
    ...
    // ANCHOR_END:SalsaDatabaseOpnTrait
