...
use crate::Query;
use crate::{CycleError, Database, QueryDb, SalsaQueryKeyIndex};
...
struct Slot<K> {
    ...
    /// SalsaQueryKeyIndex for this slot.
    ...
    Q::Value: InternKey,
{
    ...
    fn intern_index(&self, db: &<Q as QueryDb<'_>>::DynDb, key: &Q::Key) -> Arc<Slot<Q::Key>> {
        ...
        let revision_now = db.get_salsa_runtime().current_revision();
...
impl<Q> QueryStorageTrait<Q> for InternedStorage<Q>
where
    Q: Query,
    Q::Value: InternKey,
{
    ...
    fn try_fetch(
        ...
        db.get_salsa_runtime().report_query_read(
...
impl<Q> WholeQueryStorageTrait for InternedStorage<Q>
where
    Q: Query,
    Q::Value: InternKey,
{
...
impl<Q, IQ> QueryStorageTrait<Q> for LookupInternedStorage<Q, IQ>
where
    Q: Query,
    Q::Key: InternKey,
    Q::Value: Eq + Hash,
    ...
    for<'d> Q: EqualDynDb<'d, IQ>,
{
    ...
    fn fmt_db_key_index(
        ...
        interned_storage.fmt_db_key_index(Q::convert_db(db), index, fmt)
    }
...
impl<Q, IQ> WholeQueryStorageTrait for LookupInternedStorage<Q, IQ>
where
    Q: Query,
    Q::Key: InternKey,
    Q::Value: Eq + Hash,
...
impl<K> Slot<K> {
    fn maybe_changed_since(&self, revision: RevisionId) -> bool {
