...
fn a(db: &dyn QueryGroup, x: u32) -> u32 {
    ...
    db.get_salsa_runtime().report_synthetic_read(durability);
...
struct Database {
    ...
    on_event: Option<Box<dyn Fn(salsa::SalsaEvent)>>,
}
...
fn on_demand_input_durability() {
    ...
    db.on_event = Some(Box::new({
        ...
        move |event| {
            if let salsa::SalsaEventKind::BeforeRetValidMemoizedValue { .. } = event.kind {
                validated.set(validated.get() + 1)
            }
        }
    }));

    db.get_salsa_runtime_mut().synthetic_write(Durability::LOW);
    validated.set(0);
    assert_eq!(db.c(1), 10);
    assert_eq!(db.c(2), 20);
    assert_eq!(validated.get(), 2);

    db.get_salsa_runtime_mut().synthetic_write(Durability::HIGH);
    validated.set(0);
    assert_eq!(db.c(1), 10);
    assert_eq!(db.c(2), 20);
