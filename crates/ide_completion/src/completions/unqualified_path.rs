...
pub(crate) fn complete_unqualified_path(acc: &mut Completions, ctx: &CompletionContext) {
    ...
    if ctx.in_use_tree() {
        ...
        ctx.process_all_names(&mut |name, res| {
            if let ScopeDef::ModuleDef(semantics::ModuleDef::Module(_)) = res {
    ...
    match &ctx.completion_location {
        ...
        Some(ImmediateLocation::ItemList | ImmediateLocation::Trait | ImmediateLocation::Impl) => {
            ...
            ctx.process_all_names(&mut |name, res| {
                if let semantics::ScopeDef::MacroDef(mac) = res {
                    if mac.is_fn_like() {
                        acc.add_macro(ctx, Some(name.clone()), mac);
                    }
                }
                if let semantics::ScopeDef::ModuleDef(semantics::ModuleDef::Module(_)) = res {
                    acc.add_resolution(ctx, name, &res);
                }
            });
            return;
        ...
        Some(ImmediateLocation::TypeBound) => {
            ctx.process_all_names(&mut |name, res| {
                let add_resolution = match res {
                    ...
                    ScopeDef::ModuleDef(semantics::ModuleDef::Trait(_) | semantics::ModuleDef::Module(_)) => {
                        true
                    }
                    _ => false,
                };
    ...
    if let Some(ImmediateLocation::GenericArgList(arg_list)) = &ctx.completion_location {
        if let Some(path_seg) = arg_list.syntax().parent().and_then(ast::PathSegment::cast) {
            if let Some(semantics::PathResolution::Def(semantics::ModuleDef::Trait(trait_))) =
                ctx.sema.resolve_path(&path_seg.parent_path())
            {
                trait_.items(ctx.sema.db).into_iter().for_each(|it| {
                    if let semantics::AssocItem::TypeAlias(alias) = it {
    ...
    ctx.process_all_names(&mut |name, res| {
        let add_resolution = match res {
            ScopeDef::GenericParam(semantics::GenericParam::LifetimeParam(_)) | ScopeDef::Label(_) => {
            ...
            ScopeDef::ModuleDef(semantics::ModuleDef::Const(_))
            | ScopeDef::GenericParam(semantics::GenericParam::ConstParam(_)) => {
