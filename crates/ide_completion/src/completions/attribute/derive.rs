...
use semantics::{HasAttrs, MacroDef, MacroKind};
...
fn get_derives_in_scope(ctx: &CompletionContext) -> Vec<(semantics::Name, MacroDef)> {
    let mut result = Vec::default();
    ctx.process_all_names(&mut |name, scope_def| {
        if let semantics::ScopeDef::MacroDef(mac) = scope_def {
            if mac.kind() == semantics::MacroKind::Derive {
    ...
    acc.add_all(
        import_assets
            ...
            .filter_map(|import| match import.original_item {
                semantics::ItemInNs::Macros(mac) => Some((import, mac)),
                _ => None,
            })
            .filter(|&(_, mac)| !ctx.is_item_hidden(&semantics::ItemInNs::Macros(mac)))
