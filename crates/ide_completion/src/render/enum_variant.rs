...
use semantics::{HasAttrs, HirDisplay};
...
struct EnumRender<'a> {
    ...
    variant: semantics::Variant,
    path: Option<semantics::ModPath>,
    qualified_name: semantics::ModPath,
    short_qualified_name: semantics::ModPath,
    variant_kind: semantics::StructKind,
}
...
impl<'a> EnumRender<'a> {
    fn new(
        ...
        let (qualified_name, short_qualified_name) = match &path {
            Some(path) => {
                let short = semantics::ModPath::from_segments(
            ...
            None => (
                semantics::ModPath::from_segments(semantics::PathKind::Plain, iter::once(name.clone())),
                semantics::ModPath::from_segments(semantics::PathKind::Plain, iter::once(name)),
            ),
        };
    ...
    fn render(self, import_to_add: Option<ImportEdit>) -> CompletionItem {
        ...
        if self.variant_kind == semantics::StructKind::Tuple {
    ...
    fn detail(&self) -> String {
        ...
        match self.variant_kind {
            semantics::StructKind::Tuple | semantics::StructKind::Unit => format!(
                "({})",
                detail_types.map(|(_, t)| t.display(self.ctx.db()).to_string()).format(", ")
            ),
            semantics::StructKind::Record => format!(
                "{{ {} }}",
                detail_types
