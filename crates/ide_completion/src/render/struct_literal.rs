...
use semantics::{db::HirDatabase, HasAttrs, HasVisibility, Name, StructKind};
...
fn render_tuple_as_literal(fields: &[semantics::Field], name: &str) -> String {
    format!(
        "{name}({})",
...
fn visible_fields(
    ...
    fields: &[semantics::Field],
    item: impl HasAttrs,
) -> Option<(Vec<semantics::Field>, bool)> {
