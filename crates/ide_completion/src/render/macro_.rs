...
struct MacroRender<'a> {
    ...
    name: SmolStr,
    macro_: semantics::MacroDef,
    docs: Option<semantics::Documentation>,
    bra: &'static str,
    ket: &'static str,
}

impl<'a> MacroRender<'a> {
    fn new(ctx: RenderContext<'a>, name: semantics::Name, macro_: semantics::MacroDef) -> MacroRender<'a> {
    ...
    fn label(&self) -> SmolStr {
        ...
        } else if self.macro_.kind() == semantics::MacroKind::Derive {
            self.name.clone()
        } else {
            self.banged_name()
        }
