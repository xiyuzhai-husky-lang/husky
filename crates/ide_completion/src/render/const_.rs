...
use semantics::{AsAssocItem, HasSource};
use ide_db::SymbolKind;
use syntax::{ast::Const, display::const_label};

use crate::{item::CompletionItem, render::RenderContext};

pub(crate) fn render_const(ctx: RenderContext<'_>, const_: semantics::Const) -> Option<CompletionItem> {
...
struct ConstRender<'a> {
    ctx: RenderContext<'a>,
    const_: semantics::Const,
    ast_node: Const,
}

impl<'a> ConstRender<'a> {
    fn new(ctx: RenderContext<'a>, const_: semantics::Const) -> Option<ConstRender<'a>> {
