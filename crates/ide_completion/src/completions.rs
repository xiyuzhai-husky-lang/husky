...
impl Completions {
    ...
    pub(crate) fn add_resolution(
        ...
        local_name: semantics::Name,
        resolution: &semantics::ScopeDef,
    ) {
    ...
    pub(crate) fn add_macro(
        &mut self,
        ctx: &CompletionContext,
        name: Option<semantics::Name>,
        macro_: semantics::MacroDef,
    ) {
        let name = match name {
            Some(it) => it,
            None => return,
    ...
    pub(crate) fn add_function(
        ...
        local_name: Option<semantics::Name>,
    ) {
    ...
    pub(crate) fn add_method(
        ...
        ctx: &CompletionContext,
        func: semantics::Function,
        receiver: Option<semantics::Name>,
        local_name: Option<semantics::Name>,
    ) {
        if !ctx.is_visible(&func) {
            return;
    ...
    pub(crate) fn add_const(&mut self, ctx: &CompletionContext, konst: semantics::Const) {
    ...
    pub(crate) fn add_type_alias(&mut self, ctx: &CompletionContext, type_alias: semantics::TypeAlias) {
    ...
    pub(crate) fn add_type_alias_with_eq(
        &mut self,
        ctx: &CompletionContext,
        type_alias: semantics::TypeAlias,
    ) {
    ...
    pub(crate) fn add_struct_literal(
        &mut self,
        ctx: &CompletionContext,
        strukt: semantics::Struct,
        path: Option<semantics::ModPath>,
        local_name: Option<semantics::Name>,
    ) {
...
fn enum_variants_with_paths(
    acc: &mut Completions,
    ctx: &CompletionContext,
    enum_: semantics::Enum,
    cb: impl Fn(&mut Completions, &CompletionContext, semantics::Variant, semantics::ModPath),
) {
    ...
    if let Some(impl_) = ctx.impl_def.as_ref().and_then(|impl_| ctx.sema.to_def(impl_)) {
        if impl_.self_ty(ctx.db).as_adt() == Some(semantics::Adt::Enum(enum_)) {
            for &variant in &variants {
                let self_path = semantics::ModPath::from_segments(
                    semantics::PathKind::Plain,
    ...
    for variant in variants {
        if let Some(path) = module.find_use_path(ctx.db, semantics::ModuleDef::from(variant)) {
