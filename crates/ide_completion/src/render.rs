...
use semantics::{AsAssocItem, HasAttrs, HirDisplay};
use ide_db::{
...
impl<'a> RenderContext<'a> {
    ...
    fn is_deprecated_assoc_item(&self, as_assoc_item: impl AsAssocItem) -> bool {
        ...
        let is_assoc_deprecated = match assoc {
            semantics::AssocItem::Function(it) => self.is_deprecated(it),
            semantics::AssocItem::Const(it) => self.is_deprecated(it),
            semantics::AssocItem::TypeAlias(it) => self.is_deprecated(it),
        };
        is_assoc_deprecated
            || assoc
    ...
    fn docs(&self, node: impl HasAttrs) -> Option<semantics::Documentation> {
        node.docs(self.db())
    }
}
...
pub(crate) fn render_resolution_with_import(
    ...
    let resolution = semantics::ScopeDef::from(import_edit.import.original_item);
    let local_name = match resolution {
        semantics::ScopeDef::ModuleDef(semantics::ModuleDef::Function(f)) => f.name(ctx.completion.db),
        semantics::ScopeDef::ModuleDef(semantics::ModuleDef::Const(c)) => c.name(ctx.completion.db)?,
        semantics::ScopeDef::ModuleDef(semantics::ModuleDef::TypeAlias(t)) => t.name(ctx.completion.db),
...
fn render_resolution_(
    ...
    let kind = match resolution {
        semantics::ScopeDef::ModuleDef(Function(func)) => {
            return render_fn(ctx, import_to_add, Some(local_name), *func);
        }
        semantics::ScopeDef::ModuleDef(Variant(_)) if ctx.completion.pattern_ctx.is_some() => {
            CompletionItemKind::SymbolKind(SymbolKind::Variant)
        }
        semantics::ScopeDef::ModuleDef(Variant(var)) => {
            let item = render_variant(ctx, import_to_add, Some(local_name), *var, None);
            return Some(item);
        }
        semantics::ScopeDef::MacroDef(mac) => {
            let item = render_macro(ctx, import_to_add, local_name, *mac);
            return item;
        }

        semantics::ScopeDef::ModuleDef(Module(..)) => CompletionItemKind::SymbolKind(SymbolKind::Module),
        semantics::ScopeDef::ModuleDef(Adt(adt)) => CompletionItemKind::SymbolKind(match adt {
            semantics::Adt::Struct(_) => SymbolKind::Struct,
            semantics::Adt::Union(_) => SymbolKind::Union,
            semantics::Adt::Enum(_) => SymbolKind::Enum,
        }),
        semantics::ScopeDef::ModuleDef(Const(..)) => CompletionItemKind::SymbolKind(SymbolKind::Const),
        semantics::ScopeDef::ModuleDef(Static(..)) => CompletionItemKind::SymbolKind(SymbolKind::Static),
        semantics::ScopeDef::ModuleDef(Trait(..)) => CompletionItemKind::SymbolKind(SymbolKind::Trait),
        semantics::ScopeDef::ModuleDef(TypeAlias(..)) => {
            CompletionItemKind::SymbolKind(SymbolKind::TypeAlias)
        }
        semantics::ScopeDef::ModuleDef(BuiltinType(..)) => CompletionItemKind::BuiltinType,
        semantics::ScopeDef::GenericParam(param) => CompletionItemKind::SymbolKind(match param {
            semantics::GenericParam::TypeParam(_) => SymbolKind::TypeParam,
            semantics::GenericParam::LifetimeParam(_) => SymbolKind::LifetimeParam,
            semantics::GenericParam::ConstParam(_) => SymbolKind::ConstParam,
        }),
        semantics::ScopeDef::Local(..) => CompletionItemKind::SymbolKind(SymbolKind::Local),
        semantics::ScopeDef::Label(..) => CompletionItemKind::SymbolKind(SymbolKind::Label),
        semantics::ScopeDef::AdtSelfType(..) | semantics::ScopeDef::ImplSelfType(..) => {
            CompletionItemKind::SymbolKind(SymbolKind::SelfParam)
        }
        semantics::ScopeDef::Unknown => {
            let mut item = CompletionItem::new(
    ...
    if let semantics::ScopeDef::Local(local) = resolution {
    ...
    ) && ctx.completion.config.add_call_parenthesis
    {
        if let Some(cap) = ctx.snippet_cap() {
            let has_non_default_type_params = match resolution {
                semantics::ScopeDef::ModuleDef(Adt(it)) => it.has_non_default_type_params(ctx.db()),
                semantics::ScopeDef::ModuleDef(TypeAlias(it)) => it.has_non_default_type_params(ctx.db()),
                _ => false,
            };
...
fn scope_def_docs(db: &RootDatabase, resolution: &semantics::ScopeDef) -> Option<semantics::Documentation> {
    use semantics::ModuleDef::*;
    match resolution {
        semantics::ScopeDef::ModuleDef(Module(it)) => it.docs(db),
        semantics::ScopeDef::ModuleDef(Adt(it)) => it.docs(db),
        semantics::ScopeDef::ModuleDef(Variant(it)) => it.docs(db),
        semantics::ScopeDef::ModuleDef(Const(it)) => it.docs(db),
        semantics::ScopeDef::ModuleDef(Static(it)) => it.docs(db),
        semantics::ScopeDef::ModuleDef(Trait(it)) => it.docs(db),
        semantics::ScopeDef::ModuleDef(TypeAlias(it)) => it.docs(db),
        _ => None,
    }
}

fn scope_def_is_deprecated(ctx: &RenderContext<'_>, resolution: &semantics::ScopeDef) -> bool {
    match resolution {
        semantics::ScopeDef::ModuleDef(it) => ctx.is_deprecated_assoc_item(*it),
        semantics::ScopeDef::MacroDef(it) => ctx.is_deprecated(*it),
        semantics::ScopeDef::GenericParam(it) => ctx.is_deprecated(*it),
        semantics::ScopeDef::AdtSelfType(it) => ctx.is_deprecated(*it),
        _ => false,
    }
}
    ...
    if completion_ty != expected_type {
        ...
        if completion_ty.autoderef(ctx.db).any(|deref_ty| deref_ty == expected_type_without_ref) {
            ...
            let mutability = if expected_type.is_mutable_reference() {
                semantics::Mutability::Mut
            } else {
                semantics::Mutability::Shared
            };
