...
fn visibility_partial_ord_works() {
    ...
    assert!(Pub.with_db(&db) > Private(path_menu.core_num()).with_db(&db));
    assert!(Pub.with_db(&db) >= Pub.with_db(&db));
    // equals
    assert_eq!(Pub.with_db(&db), Pub.with_db(&db));
    assert_eq!(
        Private(path_menu.core_num()).with_db(&db),
        Private(path_menu.core_num()).with_db(&db)
    );
    assert_eq!(
        PubUnder(path_menu.core_prelude()).with_db(&db),
    ...
    // not equals
    assert_ne!(Pub.with_db(&db), Private(path_menu.core_num()).with_db(&db));
    assert_ne!(Private(path_menu.core_num()).with_db(&db), Pub.with_db(&db));
    assert_ne!(
        Private(path_menu.core_num()).with_db(&db),
        PubUnder(path_menu.core_num()).with_db(&db)
    );
