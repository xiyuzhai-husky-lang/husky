...
pub(crate) fn entity_variance_crate_dependencies(
    ...
    let term_menu = db.term_menu(id.path.toolchain(db)).as_ref().unwrap();
    match id.path {
        EntityPath::Module(_) => todo!(),
        EntityPath::ModuleItem(path) => match path {
            ModuleItemPath::Type(path) => ty_entity_variance_crate_dependencies(db, path, id.idx)
                .as_ref()
                .map(|t| t.as_ref()),
...
pub(crate) fn ty_entity_variance_crate_dependencies(
    db: &dyn TypeDb,
    path: TypePath,
    idx: u8,
