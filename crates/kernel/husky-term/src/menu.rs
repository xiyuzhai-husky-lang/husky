...
impl TermMenu {
    fn new(db: &dyn TermDb, toolchain: Toolchain) -> TermResult<TermMenu> {
        ...
        Ok(TermMenu { parent: menu5 })
    }
}

#[salsa::tracked(jar = TermJar,return_ref)]
pub(crate) fn term_menu(db: &dyn TermDb, toolchain: Toolchain) -> TermResult<TermMenu> {
