...
fn find_breakable<'c>(
    ...
    match label {
        Some(_) => ctxs
            .iter_mut()
            .rev()
            .find(|ctx| ctx.label.as_ref() == label),
        None => ctxs.last_mut(),
    }
}
...
impl<'a> InferenceContext<'a> {
    fn new(db: &'a dyn HirDatabase, owner: DefWithBodyId, resolver: Resolver) -> Self {
        ...
        let trait_env = owner.as_generic_def_id().map_or_else(
    ...
    fn insert_type_vars(&mut self, ty: Ty) -> Ty {
        fold_tys(
            ty,
            |ty, _| self.insert_type_vars_shallow(ty),
    ...
    fn resolve_into_iter_item(&self) -> Option<TypeAliasId> {
        ...
        self.db
            .trait_data(trait_)
            .associated_type_by_name(&name![Item])
    }
    ...
    fn resolve_range_full(&self) -> Option<AdtId> {
        ...
        let struct_ = self
            .resolver
            .resolve_known_struct(self.db.upcast(), &path)?;
        Some(struct_.into())
    }
