...
impl<'a> MatchCheckCtx<'a> {
    ...
    pub(super) fn is_foreign_non_exhaustive_enum(&self, enum_id: hir_def::EnumId) -> bool {
        let has_non_exhaustive_attr = self
            .db
            .attrs(enum_id.into())
            .by_key("non_exhaustive")
            .exists();
        let is_local = hir_def::AdtId::from(enum_id)
            .module(self.db.upcast())
            .krate()
            == self.module.krate();
...
impl PatStack {
    ...
    fn head_ctor(&self, cx: &MatchCheckCtx<'_>) -> &Constructor {
        self.head_ctor
            .get_or_init(|| Constructor::from_pat(cx, self.head()))
    }
    ...
    fn pop_head_constructor(
        ...
        // `self.head()`.
        let mut new_fields = ctor_wild_subpatterns
            .replace_with_pattern_arguments(self.head(), cx)
            .into_patterns();
...
impl SubPatSet {
    ...
    fn unsplit_or_pat(mut self, alt_id: usize, alt_count: usize, pat: PatId) -> Self {
        ...
        let set_first_col = Alt {
            subpats: subpats_first_col,
            pat,
            alt_count,
        };
...
fn is_useful(
    ...
    let ty = matrix
        .heads()
        .next()
        .map_or(cx.type_of(v.head()), |r| cx.type_of(r));
    let pcx = PatCtxt {
        cx,
        ty: &ty,
        is_top_level,
...
pub(crate) fn compute_match_usefulness(
    ...
    let wild_pattern = cx
        .pattern_arena
        .borrow_mut()
        .alloc(Pat::wildcard_from_ty(cx.infer[cx.match_expr].clone()));
