...
pub fn record_ty_raw_signature(
    db: &dyn RawSignatureDb,
    decl: RecordTypeDecl,
) -> RawSignatureResult<RecordTypeRawSignature> {
    ...
    Ok(RecordTypeRawSignature::new(db, implicit_parameters))
}

#[salsa::interned(db = RawSignatureDb, jar = RawSignatureJar)]
pub struct RecordTypeRawSignature {
    #[return_ref]
    pub implicit_parameters: ImplicitParameterRawSignatures,
}

impl RecordTypeRawSignature {}
