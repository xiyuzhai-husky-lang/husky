...
pub fn morphism_raw_signature(
    db: &dyn RawSignatureDb,
    decl: MorphismDecl,
) -> RawSignatureResult<MorphismRawSignature> {
    ...
    Ok(MorphismRawSignature::new(db, implicit_parameters))
}

#[salsa::interned(db = RawSignatureDb, jar = RawSignatureJar)]
pub struct MorphismRawSignature {
    #[return_ref]
    pub implicit_parameters: ImplicitParameterRawSignatures,
}

impl MorphismRawSignature {}
