...
pub(crate) fn ty_associated_value_raw_signature(
    db: &dyn RawSignatureDb,
    decl: TypeAssociatedValueDecl,
) -> RawSignatureResult<TypeAssociatedValueRawSignature> {
    ...
    let raw_term_menu = db
        ...
        .as_ref()
        .unwrap();
    Ok(TypeAssociatedValueRawSignature::new(db))
}

#[salsa::interned(db = RawSignatureDb, jar = RawSignatureJar)]
pub struct TypeAssociatedValueRawSignature {}
