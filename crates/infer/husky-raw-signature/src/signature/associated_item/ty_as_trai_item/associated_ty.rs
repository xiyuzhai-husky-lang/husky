...
pub(crate) fn ty_as_trai_associated_ty_raw_signature(
    db: &dyn RawSignatureDb,
    decl: TypeAsTraitAssociatedTypeDecl,
) -> RawSignatureResult<TypeAsTraitAssociatedTypeRawSignature> {
    ...
    let raw_term_menu = db
        ...
        .as_ref()
        .unwrap();
    Ok(TypeAsTraitAssociatedTypeRawSignature::new(db))
}

#[salsa::interned(db = RawSignatureDb, jar = RawSignatureJar)]
pub struct TypeAsTraitAssociatedTypeRawSignature {}
