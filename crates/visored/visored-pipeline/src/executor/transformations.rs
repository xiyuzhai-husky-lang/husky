    ...
    pub(super) fn simplification_transformations(&self) -> Vec<AllLlmsStringTransformation> {
        ...
        AllLlmsStringTransformation {
            ...
            instruction: LlmStringTransformationInstruction::MainInputSide {
                ...
                side: Some(format!(
                    r#"
There should only be one solution. No need to include alternatives. Remove unnecessary variables. If the problem is entirely trivial or just some straightforward literal calculation, just say that it's trivial that XXX holds.
...
Wrap the proof in \begin{{proof}} and \end{{proof}}.
    "#
        ...
        AllLlmsStringTransformation {
            model: self.routing_resolved.solver.mathematical_understanding.model,
            instruction: LlmStringTransformationInstruction::MainInputSide {
                main: "Fix all unnecessary expansions in the following proof generated by AI. Sometimes LLMs are stupid and do expansion despite it's totally unnecessary for the purpose of the proof.".to_string(),
                side: Some(format!(
                    r#"
There should only be one solution. No need to include alternatives. Remove unnecessary variables. If the problem is entirely trivial or just some straightforward literal calculation, just say that it's trivial that XXX holds.
...
Prove: $(x + y)^2 \ge 0$.
\begin{proof}
We have $(x + y)^2 = x^2 + 2xy + y^2 \ge 0$ because these are real numbers.
\end{proof}
```

In the above proof, the expansion of $(x + y)^2 = x^2 + 2xy + y^2$ is unnecessary and dumb. We should absolutely remove it like below.
