            ...
            VdMirExprData::Variable(local_defn_idx) => {
                ...
                if expr_entry.ty().is_numeric(self.eterner_db()) {
                    ...
                    p!(
                        expr_entry.ty(),
                        expr_entry.ty().data(),
                        expr_entry.ty().is_numeric(self.eterner_db())
                    );
                    match *expr_entry.ty().data() {
                        VdTermData::Literal(_) => todo!(),
                        VdTermData::ItemPath(ref data) => {
                            p!(data.item_path());
                            match data.item_path() {
                                visored_entity_path::path::VdItemPath::Category(
                                    vd_category_path,
                                ) => todo!(),
                                visored_entity_path::path::VdItemPath::Set(path) => match path {
                                    visored_entity_path::path::set::VdSetPath::Prelude(
                                        vd_prelude_set_path,
                                    ) => {
                                        p!(vd_prelude_set_path);
                                        use eterned::db::EternerDb;
                                        use visored_entity_path::path::set::VdPreludeSetPath;
                                        use visored_entity_path::path::set::VdSetPath;
                                        use visored_entity_path::path::VdItemPath;
                                        use visored_term::term::VdTermId;

                                        struct is_vd_ty_numeric {}

                                        impl ::eterned::memo::IsMemo for is_vd_ty_numeric {
                                            type Jar = ::eterned::memo::jar::Jar<(VdTermId), bool>;
                                        }
                                        fn is_vd_ty_numeric<'db>(
                                            ty: VdTermId,
                                            db: &'db ::eterned::db::EternerDb,
                                        ) -> bool {
                                            fn __is_vd_ty_numeric(
                                                ty: VdTermId,
                                                db: &::eterned::db::EternerDb,
                                            ) -> bool {
                                                match *ty.data() {
            VdTermData::ItemPath(ref data) => match data.item_path() {
                VdItemPath::Category(vd_category_path) => {
                ...
                VdItemPath::Function(vd_function_path) => {
                    panic!("not yet implemented")
                }
                VdItemPath::Trait(vd_trait_path) => panic!("not yet implemented"),
                VdItemPath::TraitItem(vd_trait_item_path) => {
                    panic!("not yet implemented")
                }
            },
            VdTermData::Literal(_) => panic!("not yet implemented"),
            VdTermData::ForAll(_) => panic!("not yet implemented"),
            VdTermData::Exists(_) => panic!("not yet implemented"),
            VdTermData::Limit(_) => panic!("not yet implemented"),
            VdTermData::Eval(_) => panic!("not yet implemented"),
            VdTermData::SymbolicVariable(_) => panic!("not yet implemented"),
            VdTermData::AbstractVariable(_) => panic!("not yet implemented"),
            VdTermData::StackVariable(_) => panic!("not yet implemented"),
            VdTermData::Application(_) => panic!("not yet implemented"),
            VdTermData::Abstraction(_) => panic!("not yet implemented"),
        }
                                            }

                                            p!(*db.memo_jar::<is_vd_ty_numeric>());
                                            todo!();
                                            *db.memo_jar::<is_vd_ty_numeric>().get_or_alloc(
                                                (ty),
                                                || {
                                                    todo!();
                                                    p!(ty, __is_vd_ty_numeric(ty, db));
                                                    __is_vd_ty_numeric(ty, db)
                                                },
                                            )
                                        }

                                        p!(is_vd_ty_numeric(**expr_entry.ty(), self.eterner_db()));
                                        todo!()
                                    }
                                },
                                visored_entity_path::path::VdItemPath::Function(
                                    vd_function_path,
                                ) => todo!(),
                                visored_entity_path::path::VdItemPath::Trait(vd_trait_path) => {
                                    todo!()
                                }
                                visored_entity_path::path::VdItemPath::TraitItem(
                                    vd_trait_item_path,
                                ) => todo!(),
                            }
                            todo!()
