...
pub trait EntitySymbolDb: DbWithJar<EntitySymbolJar> + AstDb + EntityPathDb {
    fn module_item_entity_kind(
        &self,
        module_item_path: ModuleItemPath,
    ) -> &EntitySymbolResult<EntityKind>;
...
impl<T> EntitySymbolDb for T
where
    T: DbWithJar<EntitySymbolJar> + AstDb + EntityPathDb,
{
    fn module_item_entity_kind(
        &self,
        module_item_path: ModuleItemPath,
