...
impl<'a, 'b, 'c> AtomParser<'a, 'b, 'c> {
    ...
    fn normal_route(&mut self, route: Ty) -> AtomResult<HuskyAtomVariant> {
    ...
    pub(crate) fn ty(&mut self) -> AtomResult<Option<Ty>> {
        Ok(
    ...
    fn generics(&mut self, route: Ty) -> AtomResult<ThinVec<SpatialArgument>> {
    ...
    fn func_args(&mut self) -> AtomResult<ThinVec<SpatialArgument>> {
        ...
        args.push(if deprecated_try_eat!(self, "->") {
            ...
            Ty::Root(RootBuiltinIdentifier::Void).into()
        });
        Ok(args)
    }
    ...
    fn intern(&self, scope: EntityRoute) -> Ty {
        self.atom_context
            .entity_syntax_db()
