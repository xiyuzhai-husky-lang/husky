...
pub(crate) struct EntityTreeCollector<'a> {
    ...
    presheets: VecMap<ModulePath, &'a EntityTreePresheet>,
}

impl<'a> EntityTreeCollector<'a> {
    pub(crate) fn new(db: &'a dyn EntityTreeDb, crate_path: CratePath) -> VfsResult<Self> {
        ...
        let presheets = all_modules
            ...
            .collect::<VfsResult<VecMap<ModulePath, &'a EntityTreePresheet>>>()?;
