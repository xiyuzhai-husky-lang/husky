...
impl ParseResult<SourceFile> {
    ...
    pub fn reparse(&self, indel: &Indel) -> ParseResult<SourceFile> {
        self.incremental_reparse(indel).unwrap_or_else(|| self.full_reparse(indel))
    }

    fn incremental_reparse(&self, indel: &Indel) -> Option<ParseResult<SourceFile>> {
        // FIXME: validation errors are not handled here
        parsing::incremental_reparse(self.tree().syntax(), indel, self.errors.to_vec()).map(
            |(green_node, errors, _reparsed_range)| ParseResult {
                green: green_node,
    ...
    fn full_reparse(&self, indel: &Indel) -> ParseResult<SourceFile> {
