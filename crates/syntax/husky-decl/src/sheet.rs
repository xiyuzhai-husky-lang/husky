...
pub struct DeclSheet {
    decls: VecPairMap<EntityPath, EntityTreeResult<Decl>>,
}
...
impl DeclSheet {
    pub fn collect_from_module(db: &dyn DeclDb, path: ModulePath) -> EntityTreeResult<Self> {
        ...
        let mut decls: VecPairMap<EntityPath, EntityTreeResult<Decl>> = Default::default();
    ...
    fn new(decls: VecPairMap<EntityPath, EntityTreeResult<Decl>>) -> Self {
        Self { decls }
    }

    pub fn decls(&self) -> &VecPairMap<EntityPath, EntityTreeResult<Decl>> {
        &self.decls
    }
}
