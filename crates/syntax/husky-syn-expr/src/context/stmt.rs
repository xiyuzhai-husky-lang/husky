...
pub struct SynStmtContext<'a> {
    ...
    dedy_tokra_region_data: DefnTokraRegionDataRef<'a>,
}
...
impl<'a> SynStmtContext<'a> {
    pub fn new(
        ...
        Some(Self {
            expr_context,
            dedy_tokra_region_data: syn_node_path.defn_tokra_region(db)?.data(db),
        })
    }
    ...
    pub(crate) fn parse_root_body(&mut self) -> SynExprIdx {
        let body = self.dedy_tokra_region_data.root_body();
