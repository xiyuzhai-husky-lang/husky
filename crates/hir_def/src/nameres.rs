...
impl ModuleOrigin {
    ...
    fn definition_source(&self, db: &dyn DefDatabase) -> InFile<ModuleSource> {
        match self {
            ...
            ModuleOrigin::BlockExpr { block } => InFile::new(
                block.file_id,
                ModuleSource::BlockExpr(block.to_node(db.upcast())),
            ),
        }
    }
}
...
impl DefMap {
    pub(crate) fn crate_def_map_query(db: &dyn DefDatabase, krate: CrateId) -> Arc<DefMap> {
        ...
        let origin = ModuleOrigin::CrateRoot {
            definition: crate_graph[krate].root_file_id,
        };
    ...
    pub(crate) fn block_def_map_query(
        ...
        if item_tree
            .inner_items_of_block(block.ast_id.value)
            .is_empty()
        {
            return None;
        }
    ...
    fn empty(krate: CrateId, edition: Edition, root_module_origin: ModuleOrigin) -> DefMap {
        ...
        let local_id = LocalModuleId::from_raw(arena::RawIdx::from(0));
        // NB: we use `None` as block here, which would be wrong for implicit
        // modules declared by blocks with items. At the moment, we don't use
        // this visibility for anything outside IDE, so that's probably OK.
        let visibility = Visibility::Module(ModuleId {
            krate,
            local_id,
            block: None,
        });
