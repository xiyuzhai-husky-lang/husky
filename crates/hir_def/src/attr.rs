...
impl RawAttrs {
        ...
        let entries = collect_attrs(owner)
            .flat_map(|(id, attr)| match attr {
                Either::Left(attr) => attr
                    .meta()
                    .and_then(|meta| Attr::from_src(db, meta, hygiene, id)),
    ...
    pub(crate) fn merge(&self, other: Self) -> Self {
        ...
        match (&self.entries, &other.entries) {
            ...
            (Some(entries), None) | (None, Some(entries)) => Self {
                entries: Some(entries.clone()),
            },
            (Some(a), Some(b)) => Self {
                entries: Some(a.iter().chain(b.iter()).cloned().collect()),
            },
        }
    }
...
impl Attrs {
    ...
    pub fn docs(&self) -> Option<Documentation> {
        let docs = self
            .by_key("doc")
            .attrs()
            .flat_map(|attr| match attr.input.as_deref()? {
    ...
    pub(crate) fn attrs_query(db: &dyn DefDatabase, def: AttrDefId) -> Self {
        ...
        let raw_attrs = match def {
            ...
            AttrDefId::FieldId(it) => {
                return Self {
                    attrs: db.fields_attrs(it.parent)[it.local_id].clone(),
                    owner: def,
                };
            }
            ...
            AttrDefId::GenericParamId(it) => match it {
                GenericParamId::TypeParamId(it) => {
                    ...
                    RawAttrs::from_attrs_owner(
                        db,
                        src.with_value(
                            src.value[it.local_id]
                                .as_ref()
                                .either(|it| it as _, |it| it as _),
                        ),
                    )
                }
    ...
    pub fn source_map(&self, db: &dyn DefDatabase) -> AttrSourceMap {
        let owner = match self.owner {
            ...
            AttrDefId::EnumVariantId(id) => {
                ...
                InFile::new(
                    file_id,
                    ast::AnyHasAttrs::new(map[id.local_id].to_node(&root)),
                )
            }
            ...
            AttrDefId::GenericParamId(id) => match id {
                GenericParamId::TypeParamId(id) => {
                    id.parent
                        .child_source(db)
                        .map(|source| match &source[id.local_id] {
...
fn inner_attributes(
    ...
    impl Iterator<Item = ast::Attr>,
    impl Iterator<Item = ast::Comment>,
)> {
    ...
    pub(crate) fn parse_derive(&self) -> Option<impl Iterator<Item = ModPath> + '_> {
        ...
        let paths = args
            ...
            .group_by(|tt| {
                matches!(
                    tt,
                    tt::TokenTree::Leaf(tt::Leaf::Punct(Punct { char: ',', .. }))
                )
            })
            .into_iter()
            .map(|(_, tts)| {
...
impl<'a> AttrQuery<'a> {
    pub fn tt_values(self) -> impl Iterator<Item = &'a Subtree> {
        self.attrs()
            .filter_map(|attr| match attr.input.as_deref()? {
    ...
    pub fn attrs(self) -> impl Iterator<Item = &'a Attr> + Clone {
        let key = self.key;
        self.attrs.iter().filter(move |attr| {
            attr.path
                .as_ident()
                .map_or(false, |s| s.to_smol_str() == key)
        })
    }
}
...
fn collect_attrs(
    ...
    docs.chain(attrs)
        .sorted_by_key(|&(_, offset, _)| offset)
        .map(|(id, _, attr)| (id, attr))
}
    ...
    for (idx, variant) in child_source.value.iter() {
        res.insert(
            ...
            variant.as_ref().either(
                |l| Either::Left(AstPtr::new(l)),
                |r| Either::Right(AstPtr::new(r)),
            ),
        );
    }
