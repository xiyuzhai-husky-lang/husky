...
macro_rules! transfer_linkage {
    ($wrapper: expr, some base $raw_fp: expr) => {{
        __LinkageGroup::Transfer(resolved_linkage!($wrapper, some base $raw_fp))
    }};
    ($wrapper: expr, some ctx $raw_fp: expr) => {{
        __LinkageGroup::Transfer(resolved_linkage!($wrapper, some ctx $raw_fp))
    }};
    ($wrapper: expr, none) => {{
        __LinkageGroup::Transfer(resolved_linkage!($wrapper, none))
    }};
}
...
macro_rules! feature_linkage {
    ($f: expr, $OUTPUT_TY: ty, $OUTPUT_TY_VTABLE: expr) => {{
        ...
        __LinkageGroup::Transfer(resolved_linkage!(__wrapper, none))
    }};
}
