...
use syntax::{
    ...
    tokenize, AstToken, ParseResult, PreorderWithTokens, SmolStr, SyntaxElement, SyntaxKind,
    SyntaxKind::*,
...
) -> Result<(ParseResult<SyntaxNode>, TokenMap), ExpandError> {
    let buffer = match tt {
        tt::Subtree {
            delimiter: None,
            token_trees,
...
pub fn parse_exprs_with_sep(tt: &tt::Subtree, sep: char) -> Vec<tt::Subtree> {
    ...
    while iter.peek_n(0).is_some() {
        ...
        res.push(match expanded.value {
            None => break,
            Some(tt @ tt::TokenTree::Leaf(_)) => tt::Subtree {
                delimiter: None,
                token_trees: vec![tt],
            },
    ...
    if iter.peek_n(0).is_some() {
        res.push(tt::Subtree {
            delimiter: None,
            token_trees: iter.into_iter().cloned().collect(),
        });
    }
...
impl<'a> TtTreeSink<'a> {
    ...
    fn finish(mut self) -> (ParseResult<SyntaxNode>, TokenMap) {
