//! Discovery of `cargo` & `huskyc` executables.
...
pub fn huskyc() -> PathBuf {
    get_path_for_executable("huskyc")
}

pub fn huskyup() -> PathBuf {
    get_path_for_executable("huskyup")
}

pub fn huskyfmt() -> PathBuf {
    get_path_for_executable("huskyfmt")
}
...
fn get_path_for_executable(executable_name: &'static str) -> PathBuf {
    ...
    //      example: for cargo, this checks $CARGO environment variable; for huskyc, $huskyC; etc
    // 2) `<executable_name>`
    //      example: for cargo, this tries just `cargo`, which will succeed if `cargo` is on the $PATH
    // 3) `~/.cargo/bin/<executable_name>`
    //      example: for cargo, this tries ~/.cargo/bin/cargo
    //      It seems that this is a reasonable place to try for cargo, huskyc, and huskyup
