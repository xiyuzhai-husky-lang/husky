...
impl Devtime {
    ...
    fn update_trace_statss_within_trace(&mut self, trace_id: TraceId) -> VMResult<()> {
    ...
    fn gen_trace_stats(&mut self, trace_id: TraceId, key: TraceStatsKey) -> VMResult<()> {
        todo!()
    ...
    fn updating_t(&self, result: VMResult<()>) -> VMResult<()> {
        match result {
            Ok(()) => Ok(()),
