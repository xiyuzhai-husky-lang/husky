...
use crate::Database;
use crate::Query;
use crate::{QueryDb, SalsaQueryKeyIndex};
...
impl<Q> QueryStorageTrait<Q> for InputStorage<Q>
where
    Q: Query,
{
    ...
    fn try_fetch(
        ...
        db: &<Q as QueryDb<'_>>::DynDb,
        key: &Q::Key,
    ) -> Result<Q::Value, CycleError<SalsaQueryKeyIndex>> {
...
where
    Q: Query,
{
    fn maybe_changed_since(&self, _db: &<Q as QueryDb<'_>>::DynDb, revision: RevisionId) -> bool {
        debug!(
...
impl<Q> WholeQueryStorageTrait for InputStorage<Q>
where
    Q: Query,
{
    fn purge(&self) {
            ...
            .with_incremented_revision(&mut |next_revision| {
                ...
                match slots.entry(key.clone()) {
                    ...
                    Entry::Vacant(entry) => {
                        ...
                        let database_key_index = SalsaQueryKeyIndex {
