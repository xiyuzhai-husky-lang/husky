...
pub use crate::{revision::RevisionId, QueryDb, SalsaQueryKeyIndex, SalsaRuntime};

#[derive(Clone, Debug)]
pub struct CycleDetected {
    pub(crate) from: SalsaRuntimeId,
    pub(crate) to: SalsaRuntimeId,
}
...
pub trait SalsaDatabaseOpnTrait {
    ...
    fn opn_salsa_runtime(&self) -> &SalsaRuntime;
    ...
    fn maybe_changed_since(&self, input: SalsaQueryKeyIndex, revision: RevisionId) -> bool;

    /// Executes the callback for each kind of query.
    fn for_each_query(&self, op: &mut dyn FnMut(&dyn WholeQueryStorageTrait));
}

pub trait WholeQueryStorageTrait {
    fn purge(&self);
}
...
pub trait QueryStorageTrait<Q>
where
    Self: WholeQueryStorageTrait,
    Q: Query,
{
