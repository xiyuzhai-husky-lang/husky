...
use crate::revision::{AtomicRevision, RevisionId};
use crate::{durability::Durability, Cancelled};
use crate::{CycleError, Database, SalsaEvent, SalsaEventKind, SalsaQueryKeyIndex};
use log::debug;
...
pub struct SalsaRuntime {
    ...
    local_state: SalsaRuntimeLocalState,

    /// Shared state that is accessible via all runtimes.
    shared_state: Arc<SharedState>,
}

impl Default for SalsaRuntime {
    fn default() -> Self {
        SalsaRuntime {
            id: SalsaRuntimeId { counter: 0 },
            revision_guard: None,
...
impl std::fmt::Debug for SalsaRuntime {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fmt.debug_struct("SalsaRuntime")
            .field("id", &self.id())
...
impl SalsaRuntime {
    ...
    #[inline]
    pub fn id(&self) -> SalsaRuntimeId {
        self.id
    }

    /// Returns the database-key for the query that this thread is
    /// actively executing (if any).
    pub fn active_query(&self) -> Option<SalsaQueryKeyIndex> {
        self.local_state.active_query()
    }

    /// Read current value of the revision counter.
    #[inline]
    pub(crate) fn current_revision(&self) -> RevisionId {
    ...
    /// dependencies.
    #[inline]
    pub(crate) fn last_changed_revision(&self, d: Durability) -> RevisionId {
        self.shared_state.revisions[d.index()].load()
    }

    /// Read current value of the revision counter.
    #[inline]
    pub(crate) fn pending_revision(&self) -> RevisionId {
    ...
    pub(crate) fn with_incremented_revision(
        &mut self,
        op: &mut dyn FnMut(RevisionId) -> Option<Durability>,
    ) {
        ...
        DB: ?Sized + Database,
    {
        ...
        db.salsa_event(SalsaEvent {
            runtime_id: self.id(),
            kind: SalsaEventKind::WillExecute {
    ...
    pub(crate) fn report_unexpected_cycle(
        ...
        error: CycleDetected,
        changed_at: RevisionId,
    ) -> crate::CycleError<SalsaQueryKeyIndex> {
        debug!(
    ...
    pub(crate) fn mark_cycle_participants(&self, err: &CycleError<SalsaQueryKeyIndex>) {
    ...
    pub(crate) fn try_block_on(
        ...
        self.shared_state.dependency_graph.lock().try_add_edge(
            self.id(),
            database_key,
            other_id,
            self.local_state
    ...
    pub(crate) fn unblock_queries_blocked_on_self(&self, database_key_index: SalsaQueryKeyIndex) {
        self.shared_state
            .dependency_graph
            .lock()
...
struct SharedState {
    ...
    dependency_graph: Mutex<DependencyGraph<SalsaQueryKeyIndex>>,
}
...
struct ActiveQuery {
    ...
    dependencies: Option<FxIndexSet<SalsaQueryKeyIndex>>,

    /// Stores the entire cycle, if one is found and this query is part of it.
    cycle: Vec<SalsaQueryKeyIndex>,
}
...
pub(crate) struct ComputedQueryResult<V> {
    ...
    /// Maximum revision of all inputs observed. If we observe an
    /// untracked read, this will be set to the most recent revision.
    pub(crate) changed_at: RevisionId,

    /// Complete set of subqueries that were accessed, or `None` if
    /// there was an untracked read.
    pub(crate) dependencies: Option<FxIndexSet<SalsaQueryKeyIndex>>,

    /// The cycle if one occured while computing this value
    pub(crate) cycle: Vec<SalsaQueryKeyIndex>,
}

impl ActiveQuery {
    fn new(database_key_index: SalsaQueryKeyIndex, max_durability: Durability) -> Self {
        ActiveQuery {
            database_key_index,
            durability: max_durability,
            changed_at: RevisionId::start(),
    ...
    fn add_untracked_read(&mut self, changed_at: RevisionId) {
        self.dependencies = None;
        self.durability = Durability::LOW;
        self.changed_at = changed_at;
    }

    fn add_synthetic_read(&mut self, durability: Durability, current_revision: RevisionId) {
        self.durability = self.durability.min(durability);
...
/// a snapshot, a fresh `SalsaRuntimeId` is generated. Once a snapshot is
/// complete, its `SalsaRuntimeId` may potentially be re-used.
#[derive(Copy, Clone, Debug, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct SalsaRuntimeId {
    counter: usize,
}
...
struct DependencyGraph<K: Hash + Eq> {
    ...
    /// will result).
    edges: FxHashMap<SalsaRuntimeId, Edge<K>>,
    labels: FxHashMap<K, SmallVec<[SalsaRuntimeId; 4]>>,
}
    ...
    K: Hash + Eq + Clone,
{
    ...
    fn try_add_edge(
        &mut self,
        from_id: SalsaRuntimeId,
        database_key: K,
        to_id: SalsaRuntimeId,
    ...
    fn remove_edge(&mut self, database_key: K, to_id: SalsaRuntimeId) {
...
mod tests {
    ...
    fn dependency_graph_path1() {
        ...
        let a = SalsaRuntimeId { counter: 0 };
        let b = SalsaRuntimeId { counter: 1 };
        assert!(graph.try_add_edge(a, 2, b, vec![1]));
    ...
    fn dependency_graph_path2() {
        ...
        let a = SalsaRuntimeId { counter: 0 };
        let b = SalsaRuntimeId { counter: 1 };
        let c = SalsaRuntimeId { counter: 2 };
        assert!(graph.try_add_edge(a, 3, b, vec![1]));
        assert!(graph.try_add_edge(b, 4, c, vec![2, 3]));
