...
pub use crate::runtime::SalsaRuntime;
pub use crate::runtime::SalsaRuntimeId;
...
pub trait Database: plumbing::SalsaInternalOpns {
    ...
    fn salsa_event(&self, event: SalsaEvent) {
    ...
    fn unwind_if_cancelled(&self) {
        let runtime = self.get_salsa_runtime();
        self.salsa_event(SalsaEvent {
            runtime_id: runtime.id(),
            kind: SalsaEventKind::WillCheckCancellation,
        });
    ...
    /// final
    fn get_salsa_runtime(&self) -> &SalsaRuntime {
        self.opn_salsa_runtime()
    }

    /// final
    fn get_salsa_runtime_mut(&mut self) -> &mut SalsaRuntime {
...
pub struct SalsaEvent {
    ...
    pub runtime_id: SalsaRuntimeId,

    /// What sort of event was it.
    pub kind: SalsaEventKind,
}

impl fmt::Debug for SalsaEvent {
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt.debug_struct("SalsaEvent")
...
pub enum SalsaEventKind {
    BeforeRetValidMemoizedValue {
        /// The database-key for the affected value. Implements `Debug`.
        database_key: SalsaQueryKeyIndex,
    },
    ...
    WillBlockOn {
        ...
        other_runtime_id: SalsaRuntimeId,
...
impl fmt::Debug for SalsaEventKind {
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            SalsaEventKind::BeforeRetValidMemoizedValue { database_key } => fmt
                .debug_struct("BeforeRetValidMemoizedValue")
                .field("database_key", database_key)
                .finish(),
            SalsaEventKind::WillBlockOn {
                other_runtime_id,
                database_key,
            } => fmt
                ...
                .field("database_key", database_key)
                .finish(),
            SalsaEventKind::WillExecute { database_key } => fmt
                .debug_struct("WillExecute")
                .field("database_key", database_key)
                .finish(),
            SalsaEventKind::WillCheckCancellation => {
...
pub trait ParallelDatabase: Database + Send {
    ...
    /// for those queries to use the [`SalsaRuntime::unwind_if_cancelled`]
    ...
    /// use [the `SalsaRuntime::snapshot` method][rfm] to create a snapshot of the
    ...
    ///
    /// [rfm]: struct.SalsaRuntime.html#method.snapshot
    ///
    /// ```rust,ignore
...
pub struct SalsaQueryKeyIndex {
    group_index: u16,
    query_index: u16,
    key_index: u32,
}

impl SalsaQueryKeyIndex {
...
pub trait Query: Debug + Default + Sized + for<'d> QueryDb<'d> {
    ...
    // type Storage: plumbing::QueryStorageTrait<Self>;
    type Storage;
    ...
    Q::Storage: QueryStorageTrait<Q>,
{
    ...
    fn try_get(&self, key: Q::Key) -> Result<Q::Value, CycleError<SalsaQueryKeyIndex>> {
    ...
    pub fn purge(&self)
    where
        Q::Storage: plumbing::WholeQueryStorageTrait,
    {
        self.storage.purge();
    }
}
