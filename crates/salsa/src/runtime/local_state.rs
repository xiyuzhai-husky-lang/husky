...
use crate::runtime::RevisionId;
use crate::SalsaQueryKeyIndex;
...
pub(super) struct SalsaRuntimeLocalState {
...
impl Default for SalsaRuntimeLocalState {
    fn default() -> Self {
        SalsaRuntimeLocalState {
            query_stack: Default::default(),
        }
    }
}

impl SalsaRuntimeLocalState {
    ...
    /// you should not use any mutating methods of `SalsaRuntimeLocalState` while
    /// reading from it.
    ...
    pub(super) fn active_query(&self) -> Option<SalsaQueryKeyIndex> {
        self.query_stack
            .borrow()
            .last()
    ...
    pub(super) fn report_untracked_read(&self, current_revision: RevisionId) {
        if let Some(top_query) = self.query_stack.borrow_mut().last_mut() {
            top_query.add_untracked_read(current_revision);
        }
    }

    pub(super) fn report_synthetic_read(
        &self,
        durability: Durability,
...
impl std::panic::RefUnwindSafe for SalsaRuntimeLocalState {}
...
pub(super) struct ActiveQueryGuard<'me> {
    local_state: &'me SalsaRuntimeLocalState,
    push_len: usize,
}
