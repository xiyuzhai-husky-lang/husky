...
use crate::Cancelled;
use crate::{CycleError, Database, QueryDb, SalsaEvent, SalsaEventKind, SalsaQueryKeyIndex};
use log::{debug, info};
...
{
    key: Q::Key,
    database_key_index: SalsaQueryKeyIndex,
    ...
    Q: QueryFunction,
{
    ...
    InProgress {
        id: SalsaRuntimeId,
        waiting: Mutex<SmallVec<[Promise<WaitResult<Q::Value, SalsaQueryKeyIndex>>; 2]>>,
    },
...
pub(super) enum MemoInputs {
    ...
    Tracked { inputs: Arc<[SalsaQueryKeyIndex]> },
...
where
    ...
    MP: MemoizationPolicy<Q>,
{
    pub(super) fn new(key: Q::Key, database_key_index: SalsaQueryKeyIndex) -> Self {
        Self {
            key,
            database_key_index,
    ...
    pub(super) fn database_key_index(&self) -> SalsaQueryKeyIndex {
    ...
    ) -> Result<StampedValue<Q::Value>, CycleError<SalsaQueryKeyIndex>> {
        let runtime = db.get_salsa_runtime();
    ...
    fn read_upgrade(
        ...
        if let Some(memo) = &mut panic_guard.memo {
            if let Some(value) = memo.validate_memoized_value(db, revision_now) {
                ...
                db.salsa_event(SalsaEvent {
                    runtime_id: runtime.id(),
                    kind: SalsaEventKind::BeforeRetValidMemoizedValue {
    ...
    fn probe<StateGuard>(
        ...
        revision_now: RevisionId,
    ) -> ProbeState<StampedValue<Q::Value>, SalsaQueryKeyIndex, StateGuard>
    where
            ...
            QueryState::InProgress { id, waiting } => {
                ...
                return match self.register_with_in_progress_thread(db, runtime, other_id, waiting) {
                    Ok(future) => {
                        ...
                        db.salsa_event(SalsaEvent {
                            runtime_id: runtime.id(),
                            kind: SalsaEventKind::WillBlockOn {
    ...
    pub(super) fn durability(&self, db: &<Q as QueryDb<'_>>::DynDb) -> Durability {
        match &*self.state.read() {
            ...
            QueryState::Memoized(memo) => {
                if memo.revisions.check_durability(db.get_salsa_runtime()) {
    ...
    pub(super) fn invalidate(&self, new_revision: RevisionId) -> Option<Durability> {
    ...
    pub(super) fn maybe_changed_since(
        ...
        //
        // Keep in mind, though, that we released the lock before checking the inputs and a lot
    ...
    fn register_with_in_progress_thread(
        ...
        runtime: &SalsaRuntime,
        other_id: SalsaRuntimeId,
        waiting: &Mutex<SmallVec<[Promise<WaitResult<Q::Value, SalsaQueryKeyIndex>>; 2]>>,
    ) -> Result<BlockingFuture<WaitResult<Q::Value, SalsaQueryKeyIndex>>, CycleDetected> {
        let id = runtime.id();
        if other_id == id {
...
where
    Q: QueryFunction,
{
    fn in_progress(id: SalsaRuntimeId) -> Self {
        QueryState::InProgress {
            id,
    ...
    MP: MemoizationPolicy<Q>,
{
    ...
    /// are out of scope.
    fn proceed(mut self, new_value: &StampedValue<Q::Value>, cycle: Vec<SalsaQueryKeyIndex>) {
    ...
    fn overwrite_placeholder(
        &mut self,
        new_value: Option<(&StampedValue<Q::Value>, Vec<SalsaQueryKeyIndex>)>,
    ) {
...
impl MemoRevisions {
    fn validate_memoized_value(&mut self, db: &dyn Database, revision_now: RevisionId) -> bool {
        ...
        if self.check_durability(db.get_salsa_runtime()) {
    ...
    fn check_durability(&self, runtime: &SalsaRuntime) -> bool {
    ...
    fn mark_value_as_verified(&mut self, revision_now: RevisionId) -> bool {
