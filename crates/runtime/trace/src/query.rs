...
pub trait TraceQueryGroup: ProduceTrace<'static> {
    #[salsa::input]
...
pub fn trace_stalk(
    ...
    Arc::new(match trace.variant {
        TraceVariant::Main(ref repr) => TraceStalk {
            extra_tokens: vec![fade!(" = "), this.eval_feature_repr(repr, input_id).into()],
        },
        TraceVariant::FeatureStmt(ref stmt) => match stmt.variant {
            FeatureStmtVariant::Init { varname, ref value } => TraceStalk {
                extra_tokens: vec![fade!(" = "), this.eval_feature_expr(value, input_id).into()],
            },
        ...
        TraceVariant::FeatureExpr(ref expr) => TraceStalk {
            extra_tokens: vec![fade!(" = "), this.eval_feature_expr(expr, input_id).into()],
        },
