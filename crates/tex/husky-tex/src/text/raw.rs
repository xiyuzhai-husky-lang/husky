...
impl TexContentRefined<RawElem> {
    ...
    fn highlight(&self, styles: StyleChain) -> Vec<TexContentRefined<RawLine>> {
        ...
        let mut seq = vec![];
        if matches!(lang.as_deref(), Some("typ" | "tex" | "typc")) {
...
/// Wrapper struct for the state required to highlight tex code.
...
fn styled(piece: &str, foreground: synt::Color, style: synt::Style) -> TexContent {
    ...
    if style.foreground != foreground {
        body = body.styled(TextElem::set_fill(to_tex(style.foreground).into()));
    }
...
fn to_tex(synt::Color { r, g, b, a }: synt::Color) -> TexColor {
...
#[comemo::memoize]
#[husky_tex_macros::time(name = "load syntaxes")]
...
#[comemo::memoize]
#[husky_tex_macros::time(name = "load theme")]
...
pub static RAW_THEME: Lazy<synt::Theme> = Lazy::new(|| synt::Theme {
    ...
    scopes: vec![
        ...
        item("string.other.math.tex", None, None),
