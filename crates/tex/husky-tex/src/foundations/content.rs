...
///
/// This type is at the heart of Tex. All markup you write and most
...
#[derive(Hash)]
struct TexContentInner<T: ?Sized> {
...
impl TexContent {
    ...
    pub fn get(&self, id: u8, styles: Option<StyleChain>) -> Option<TexValue> {
        if id == 255 {
    ...
    /// instead.
    pub fn get_by_name(&self, name: &str) -> Option<TexValue> {
        if name == "label" {
            if let Some(label) = self.label() {
                return Some(label.into_value());
            }
    ...
    /// if you have set the field IDs yourself or are using the field IDs
    /// generated by the `#[elem]` macro.
    pub fn field(&self, id: u8) -> StrResult<TexValue> {
        self.get(id, None)
    ...
    /// instead.
    pub fn field_by_name(&self, name: &str) -> StrResult<TexValue> {
    ...
    pub fn to_sequence(&self) -> Option<impl Iterator<Item = &Prehashed<TexContent>>> {
    ...
    pub fn to_styled(&self) -> Option<(&TexContent, &Styles)> {
        let styled = self.to_packed::<StyledTexElement>()?;
    ...
    pub fn styled(mut self, style: impl Into<Style>) -> Self {
        if let Some(style_elem) = self.to_packed_mut::<StyledTexElement>() {
    ...
    pub fn styled_with_map(mut self, styles: Styles) -> Self {
        ...
        if let Some(style_elem) = self.to_packed_mut::<StyledTexElement>() {
            ...
            StyledTexElement::new(Prehashed::new(self), styles).into()
        }
    }
    ...
    pub fn query(&self, selector: Selector) -> Vec<TexContent> {
    ...
    ///
    /// Elements produced in `show` rules will not be included in the results.
    pub fn query_first(&self, selector: Selector) -> Option<TexContent> {
        let mut result = None;
        self.traverse(&mut |element| {
        ...
        F: FnMut(TexContent),
    {
        ...
        fn walk_value<F>(value: TexValue, f: &mut F)
        where
            F: FnMut(TexContent),
        {
            match value {
                TexValue::Content(content) => content.traverse(f),
...
impl TexContent {
    ...
    pub fn aligned(self, align: TexAlignment) -> Self {
...
impl Default for TexContent {
    fn default() -> Self {
        Self::empty()
    }
}

impl Debug for TexContent {
...
impl Repr for TexContent {
    fn repr(&self) -> EcoString {
        self.inner.elem.repr()
    }
}

impl Add for TexContent {
    type Output = Self;
...
impl<'a> Add<&'a Self> for TexContent {
    type Output = Self;
...
impl AddAssign for TexContent {
    fn add_assign(&mut self, rhs: Self) {
        *self = std::mem::take(self) + rhs;
    }
}

impl AddAssign<&Self> for TexContent {
    fn add_assign(&mut self, rhs: &Self) {
        *self = std::mem::take(self) + rhs;
    }
}

impl Sum for TexContent {
    fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
        Self::sequence(iter)
    }
}

impl Serialize for TexContent {
...
impl<T: IsTexElem> Packed<T> {
    ...
    pub fn from_ref(content: &TexContent) -> Option<&Self> {
        if content.is::<T>() {
            // Safety:
    ...
    /// Try to cast type-erased content into a statically known packed element.
    pub fn from_mut(content: &mut TexContent) -> Option<&mut Self> {
        if content.is::<T>() {
            // Safety:
            // - We have checked the type.
            // - Packed<T> is repr(transparent).
    ...
    /// Try to cast type-erased content into a statically known packed element.
    pub fn from_owned(content: TexContent) -> Result<Self, TexContent> {
        if content.is::<T>() {
            // Safety:
            // - We have checked the type.
            // - Packed<T> is repr(transparent).
...
struct SequenceElem {
    #[required]
    children: Vec<Prehashed<TexContent>>,
}
...
#[elem(Debug, Repr, PartialEq)]
struct StyledTexElement {
    #[required]
    child: Prehashed<TexContent>,
    #[required]
    styles: Styles,
}

impl Debug for StyledTexElement {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
...
impl PartialEq for StyledTexElement {
    fn eq(&self, other: &Self) -> bool {
        *self.child == *other.child
    }
}

impl Repr for StyledTexElement {
    fn repr(&self) -> EcoString {
