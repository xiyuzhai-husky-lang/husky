...
impl Image {
    ...
    #[comemo::memoize]
    #[husky_tex_macros::time(name = "load image")]
    pub fn new(data: Bytes, format: ImageFormat, alt: Option<EcoString>) -> StrResult<Image> {
        let kind = match format {
            ImageFormat::Raster(format) => ImageKind::Raster(RasterImage::new(data, format)?),
            ImageFormat::Vector(VectorFormat::Svg) => ImageKind::Svg(SvgImage::new(data)?),
        };
