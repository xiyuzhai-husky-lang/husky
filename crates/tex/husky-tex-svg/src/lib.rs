...
impl SVGRenderer {
    ...
    fn write_header(&mut self, size: Size) {
        ...
        self.xml.write_attribute("class", "tex-doc");
        ...
        self.xml
            .write_attribute("xmlns:h5", "http://www.w3.org/1999/xhtml");
    }
    ...
    fn render_group(&mut self, state: State, group: &GroupItem) {
        ...
        self.xml.start_element("g");
        self.xml.write_attribute("class", "tex-group");

        if let Some(clip_path) = &group.clip_path {
            let hash = hash128(&group);
            let id = self
                .clip_paths
                .insert_with(hash, || convert_path(clip_path));
            self.xml
    ...
    fn render_text(&mut self, state: State, text: &TextItem) {
        ...
        self.xml.start_element("g");
        self.xml.write_attribute("class", "tex-text");
    ...
    fn render_svg_glyph(
        ...
        let id = self
            .glyphs
            .insert_with(glyph_hash, || RenderedGlyph::Image {
                url: data_url,
                width: upem.to_pt(),
                height: upem.to_pt(),
    ...
    fn render_bitmap_glyph(&mut self, text: &TextItem, id: GlyphId, x_offset: f64) -> Option<()> {
    ...
    fn render_shape(&mut self, state: State, shape: &TexShape) {
        ...
        self.xml.write_attribute("class", "tex-shape");
    ...
    fn push_pattern(&mut self, pattern: &Pattern, size: Size, ts: Transform) -> Id {
        ...
        let pattern_id = self
            .patterns
            .insert_with(hash128(pattern), || pattern.clone());
        self.pattern_refs
    ...
    fn write_stroke(&mut self, stroke: &TexFixedStroke, size: Size, fill_transform: Transform) {
        ...
        if let Some(pattern) = &stroke.dash {
            self.xml
                .write_attribute("stroke-dashoffset", &pattern.phase.to_pt());
            self.xml.write_attribute(
                "stroke-dasharray",
                &pattern
                    .array
            ...
            match &gradient {
                ...
                Gradient::Conic(conic) => {
                    ...
                    for i in 0..CONIC_SEGMENT {
                        ...
                        builder
                            .move_to(correct_pattern_pos(center.0), correct_pattern_pos(center.1));
                        builder.line_to(
                        ...
                        builder.arc(
                            ...
                            1,
                            (
                                correct_pattern_pos(-2.0 * (theta2 + angle).cos() + center.0),
                                correct_pattern_pos(2.0 * (theta2 + angle).sin() + center.1),
                            ),
                        );
                        builder.close();
...
fn convert_svg_glyph_to_base64_url(font: &TexFont, id: GlyphId) -> Option<EcoString> {
    ...
    let b64_encoded = base64::engine::general_purpose::STANDARD.encode(svg_str.as_bytes());
...
fn convert_path(path: &Path) -> EcoString {
    ...
    for item in &path.0 {
        match item {
            PathItem::MoveTo(m) => builder.move_to(m.x.to_pt() as f32, m.y.to_pt() as f32),
            PathItem::LineTo(l) => builder.line_to(l.x.to_pt() as f32, l.y.to_pt() as f32),
