...
use husky_tex::model::MODEL;
use husky_tex::symbols::SYMBOLS;
use husky_tex::text::{TexFont, TexFontBook, TEXT};
use husky_tex::visualize::VISUALIZE;
use husky_tex::Library;
...
pub fn provide(resolver: &dyn Resolver) -> Vec<PageModel> {
    vec![
        markdown_page(resolver, "/docs/", "tex/overview.md").with_route("/docs/"),
        tutorial_pages(resolver),
        reference_pages(resolver),
        guide_pages(resolver),
        packages_page(resolver),
        markdown_page(resolver, "/docs/", "tex/changelog.md"),
        markdown_page(resolver, "/docs/", "tex/roadmap.md"),
        markdown_page(resolver, "/docs/", "tex/community.md"),
    ]
}
...
#[track_caller]
fn markdown_page(resolver: &dyn Resolver, parent: &str, path: impl AsRef<Path>) -> PageModel {
...
fn tutorial_pages(resolver: &dyn Resolver) -> PageModel {
    let mut page = markdown_page(resolver, "/docs/", "tex/tutorial/welcome.md");
    page.children = DOCS_DIR
        .get_dir("tex/tutorial")
        .unwrap()
        .files()
        .filter(|file| file.path() != Path::new("tex/tutorial/welcome.md"))
        .map(|file| markdown_page(resolver, "/docs/tex/tutorial/", file.path()))
        .collect();
    page
}
...
fn reference_pages(resolver: &dyn Resolver) -> PageModel {
    let mut page = markdown_page(resolver, "/docs/", "tex/reference/welcome.md");
    page.children = vec![
        markdown_page(resolver, "/docs/reference/", "tex/reference/syntax.md")
            .with_part("Language"),
        markdown_page(resolver, "/docs/reference/", "tex/reference/styling.md"),
        markdown_page(resolver, "/docs/reference/", "tex/reference/scripting.md"),
...
fn category_page(resolver: &dyn Resolver, category: Category) -> PageModel {
        ...
        .cloned()
    {
        if matches!(group.name.as_str(), "sym" | "emoji") {
            ...
            let BodyModel::Symbols(model) = &subpage.body else {
                continue;
            };
            let list = &model.list;
            markup.extend(
            ...
            math.extend(
                list.iter()
                    .filter(|symbol| symbol.math_shorthand.is_some())
                    .cloned(),
            );
...
fn func_page(resolver: &dyn Resolver, parent: &str, func: &Func, path: &[&str]) -> PageModel {
...
fn func_model(resolver: &dyn Resolver, func: &Func, path: &[&str], nested: bool) -> FuncModel {
    let name = func.name().unwrap();
    ...
    let (details, example) = if nested {
        split_details_and_example(docs)
    } else {
        (docs, None)
    };
    ...
    FuncModel {
        ...
        params: params
            .iter()
            .map(|param| param_model(resolver, param))
            .collect(),
        returns,
...
fn param_model(resolver: &dyn Resolver, info: &ParamInfo) -> ParamModel {
    ...
    ParamModel {
        ...
        default: info.default.map(|default| {
            let node = husky_tex::syntax::parse_code(&default().repr());
            Html::new(husky_tex::syntax::highlight_html(&node))
        }),
    ...
    for (name, value) in group.module().scope().iter() {
        ...
        for (variant, c) in symbol.variants() {
            ...
            list.push(SymbolModel {
                ...
                markup_shorthand: shorthand(husky_tex::syntax::ast::Shorthand::MARKUP_LIST),
                math_shorthand: shorthand(husky_tex::syntax::ast::Shorthand::MATH_LIST),
                codepoint: c as u32,
                accent: husky_tex::symbols::Symbol::combining_accent(c).is_some(),
                unicode_name: unicode_names2::name(c).map(|s| s.to_string().to_title_case().into()),
                alternates: symbol
                    .variants()
