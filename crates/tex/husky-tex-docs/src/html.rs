        ...
        match event {
            ...
            md::Event::Code(code) => {
                ...
                let parser = match (chars.next(), chars.next_back()) {
                    (Some('['), Some(']')) => husky_tex::syntax::parse,
                    (Some('{'), Some('}')) => husky_tex::syntax::parse_code,
                    _ => return true,
                };

                let root = parser(&code[1..code.len() - 1]);
                let html = husky_tex::syntax::highlight_html(&root);
...
fn code_block(resolver: &dyn Resolver, lang: &str, text: &str) -> Html {
    ...
    if lang.is_empty() {
        ...
        let set = &*husky_tex::text::RAW_SYNTAXES;
    ...
    if matches!(lang, "example" | "typ") {
        let root = husky_tex::syntax::parse(&display);
        let html = Html::new(husky_tex::syntax::highlight_html(&root));
        if lang == "typ" {
    ...
    let mut document = match husky_tex::compile(&world, &mut tracer) {
        Ok(doc) => doc,
        Err(err) => {
    ...
    let hash = husky_tex::util::hash128(text);
