...
#[husky_tex_macros::time(name = "compile once")]
pub fn compile_once(
...
fn export_pdf(
    ...
    let buffer = husky_tex_pdf::pdf(document, ident.as_deref(), now());
        ...
        .map(|(i, page)| {
            ...
            match fmt {
                ImageExportFormat::Png => {
                    let pixmap =
                        husky_tex_render::render(&page.frame, command.ppi / 72.0, TexColor::WHITE);
                    pixmap
                        .save_png(path)
                        .map_err(|err| eco_format!("failed to write PNG file ({err})"))?;
                }
                ImageExportFormat::Svg => {
                    let svg = husky_tex_svg::svg(&page.frame);
