...
const TYPST_GITHUB_ORG: &str = "tex";
const TYPST_REPO: &str = "tex";
...
fn extract_binary_from_zip(data: &[u8], asset_name: &str) -> StrResult<Vec<u8>> {
    ...
    let mut file = archive
        .by_name(&format!("{asset_name}/tex.exe"))
...
fn extract_binary_from_tar_xz(data: &[u8]) -> StrResult<Vec<u8>> {
    ...
    let mut file = archive
        ...
        .find(|e| e.path().unwrap_or_default().ends_with("tex"))
...
fn needed_asset() -> StrResult<&'static str> {
    Ok(match env!("TARGET") {
        "x86_64-unknown-linux-gnu" => "tex-x86_64-unknown-linux-musl",
        "x86_64-unknown-linux-musl" => "tex-x86_64-unknown-linux-musl",
        "aarch64-unknown-linux-musl" => "tex-aarch64-unknown-linux-musl",
        "aarch64-unknown-linux-gnu" => "tex-aarch64-unknown-linux-musl",
        "armv7-unknown-linux-musleabi" => "tex-armv7-unknown-linux-musleabi",
        "x86_64-apple-darwin" => "tex-x86_64-apple-darwin",
        "aarch64-apple-darwin" => "tex-aarch64-apple-darwin",
        "x86_64-pc-windows-msvc" => "tex-x86_64-pc-windows-msvc",
...
fn backup_path() -> StrResult<PathBuf> {
    ...
    let backup_dir = root_backup_dir.join("tex");
