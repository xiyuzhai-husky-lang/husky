...
fn main() {
    ...
    if option_env!("TYPST_VERSION").is_none() {
        println!("cargo:rustc-env=TYPST_VERSION={}", tex_version());
    }
    ...
    if let Some(dir) = env::var_os("GEN_ARTIFACTS") {
        ...
        Man::new(cmd.clone())
            .render(&mut File::create(out.join("tex.1")).unwrap())
            .unwrap();

        for subcmd in cmd.get_subcommands() {
            let name = format!("tex-{}", subcmd.get_name());
        ...
        for shell in Shell::value_variants() {
            generate_to(*shell, cmd, "tex", out).unwrap();
        }
    }
}

/// Also used by `args.rs`.
fn tex_version() -> String {
