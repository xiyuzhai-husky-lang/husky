...
use husky_tex::text::TexFont;
use husky_tex::util::SliceExt;
...
#[husky_tex_macros::time(name = "write fonts")]
...
#[comemo::memoize]
#[husky_tex_macros::time(name = "subset font")]
fn subset_font(font: &TexFont, glyphs: &[u16]) -> Arc<Vec<u8>> {
    let data = font.data();
...
fn subset_tag(glyphs: &BTreeMap<u16, EcoString>) -> EcoString {
    const LEN: usize = 6;
    const BASE: u128 = 26;
    let mut hash = husky_tex::util::hash128(&glyphs);
...
fn create_cmap(ttf: &ttf_parser::Face, glyph_set: &mut BTreeMap<u16, EcoString>) -> UnicodeCmap {
        ...
        .flat_map(|table| table.subtables)
    {
        ...
        subtable.codepoints(|n| {
            let Some(c) = std::char::from_u32(n) else {
                return;
            };
            ...
            let Some(GlyphId(g)) = ttf.glyph_index(c) else {
                return;
            };
...
pub(super) fn glyph_cid(font: &TexFont, glyph_id: u16) -> u16 {
    font.ttf()
        .tables()
        .cff
