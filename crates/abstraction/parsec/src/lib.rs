...
pub trait StreamParser: HasStreamState {
    ...
    fn parse_expected_with_context<T: ParseFromStreamWithContext<Self>, E: OriginalError>(
        &mut self,
        ctx: T::Context,
        f: impl FnOnce(Self::State) -> E,
    ) -> Result<T, E::Error>
    where
        E::Error: From<<T as ParseFromStreamWithContext<Self>>::Error>;
    ...
    SP: StreamParser + ?Sized,
{
    ...
    fn parse_from_without_guaranteed_rollback(sp: &mut SP) -> Result<Option<Self>, Self::Error>;
}

pub trait ParseFromStreamWithContext<SP>: Sized
where
    SP: StreamParser + ?Sized,
{
    type Error;
