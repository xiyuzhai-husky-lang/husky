...
///
/// * the "id struct" `struct Foo(timed_salsa::Id)`
...
impl InputStruct {
    ...
    fn input_inherent_impl(&self) -> syn::ItemImpl {
            ...
            if !*is_clone_field {
                parse_quote! {
                    #field_vis fn #get_field_name<'db>(self, __db: &'db #db_dyn_ty) -> &'db #field_ty
                    {
                        ...
                        let __ingredients = <#jar_ty as timed_salsa::storage::HasIngredientsFor< #ident >>::ingredient(__jar);
        ...
        let field_setters: Vec<syn::ImplItemMethod> = field_indices.iter()
            ...
            parse_quote! {
                #field_vis fn #set_field_name<'db>(self, __db: &'db mut #db_dyn_ty) -> timed_salsa::setter::Setter<'db, #ident, #field_ty>
                {
                    let (__jar, __runtime) = <_ as timed_salsa::storage::HasJar<#jar_ty>>::jar_mut(__db);
                    let __ingredients = <#jar_ty as timed_salsa::storage::HasIngredientsFor< #ident >>::ingredient_mut(__jar);
                    timed_salsa::setter::Setter::new(__runtime, self, &mut __ingredients.#field_index)
                }
            }
        })
        .collect();
        ...
        let constructor: syn::ImplItemMethod = if singleton {
            parse_quote! {
                ...
                pub fn #constructor_name(__db: &#db_dyn_ty, #(#field_names: #field_tys,)*) -> Self
                {
                    ...
                    #(
                        __ingredients.#field_indices.store_new(__runtime, __id, #field_names, timed_salsa::Durability::LOW);
                    )*
                    __id
                }
            }
    ...
    fn input_ingredients(&self) -> syn::ItemImpl {
        ...
        parse_quote! {
            impl timed_salsa::storage::IngredientsFor for #ident {
                type Jar = #jar_ty;
                type Ingredients = (
                    #(
                        timed_salsa::input_field::InputFieldIngredient<#ident, #field_ty>,
                    )*
                    timed_salsa::input::InputIngredient<#ident>,
                );
                        ...
                        #(
                            {
                                let index = routes.push(
                                    |jars| {
                                        ...
                                        let ingredients = <_ as timed_salsa::storage::HasIngredientsFor<Self>>::ingredient(jar);
                                        &ingredients.#all_field_indices
                                    },
                                    |jars| {
                                        let jar = <DB as timed_salsa::storage::JarFromJars<Self::Jar>>::jar_from_jars_mut(jars);
                                        let ingredients = <_ as timed_salsa::storage::HasIngredientsFor<Self>>::ingredient_mut(jar);
                                        &mut ingredients.#all_field_indices
                                    },
                                );
                                timed_salsa::input_field::InputFieldIngredient::new(index, #debug_name_fields)
                            },
                        )*
                        {
                            let index = routes.push(
                            ...
                            timed_salsa::input::InputIngredient::new(index, #debug_name_struct)
                        },
                    )
                }
            }
