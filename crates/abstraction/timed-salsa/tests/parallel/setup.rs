...
impl timed_salsa::Database for Database {
    fn salsa_event(&self, event: timed_salsa::Event) {
        if let timed_salsa::EventKind::WillBlockOn { .. } = event.kind {
            self.signal(self.knobs().signal_on_will_block.get());
        }
    }
}

impl timed_salsa::ParallelDatabase for Database {
    fn snapshot(&self) -> timed_salsa::Snapshot<Self> {
        timed_salsa::Snapshot::new(Database {
