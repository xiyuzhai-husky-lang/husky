...
pub(crate) trait Db: timed_salsa::DbWithJar<Jar> + Knobs {}

impl<T: timed_salsa::DbWithJar<Jar> + Knobs> Db for T {}
...
#[timed_salsa::tracked(jar = Jar, recovery_fn=recover)]
pub(crate) fn a2(db: &dyn Db, input: MyInput) -> i32 {
    b1(db, input)
}

fn recover(db: &dyn Db, _cycle: &timed_salsa::Cycle, key: MyInput) -> i32 {
    dbg!("recover");
    key.field(db) * 20 + 2
}
