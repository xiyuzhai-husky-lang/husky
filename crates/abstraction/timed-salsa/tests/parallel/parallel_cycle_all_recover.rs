...
#[timed_salsa::tracked(jar = Jar, recovery_fn=recover_a1)]
...
fn recover_a1(db: &dyn Db, _cycle: &timed_salsa::Cycle, key: MyInput) -> i32 {
    dbg!("recover_a1");
    key.field(db) * 10 + 1
}

#[timed_salsa::tracked(jar = Jar, recovery_fn=recover_a2)]
pub(crate) fn a2(db: &dyn Db, input: MyInput) -> i32 {
    b1(db, input)
}

fn recover_a2(db: &dyn Db, _cycle: &timed_salsa::Cycle, key: MyInput) -> i32 {
    dbg!("recover_a2");
    key.field(db) * 10 + 2
}

#[timed_salsa::tracked(jar = Jar, recovery_fn=recover_b1)]
...
fn recover_b1(db: &dyn Db, _cycle: &timed_salsa::Cycle, key: MyInput) -> i32 {
    dbg!("recover_b1");
    key.field(db) * 20 + 1
}

#[timed_salsa::tracked(jar = Jar, recovery_fn=recover_b2)]
pub(crate) fn b2(db: &dyn Db, input: MyInput) -> i32 {
    a1(db, input)
}

fn recover_b2(db: &dyn Db, _cycle: &timed_salsa::Cycle, key: MyInput) -> i32 {
    dbg!("recover_b2");
    key.field(db) * 20 + 2
}
