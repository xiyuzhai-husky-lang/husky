...
impl SearchScope {
    ...
    fn reverse_dependencies(db: &RootDatabase, of: semantics::Crate) -> SearchScope {
        let mut entries = FxHashMap::default();
    ...
    fn krate(db: &RootDatabase, of: semantics::Crate) -> SearchScope {
    ...
    fn module(db: &RootDatabase, module: semantics::Module) -> SearchScope {
        let mut entries = FxHashMap::default();
...
impl Definition {
    fn search_scope(&self, db: &RootDatabase) -> SearchScope {
        ...
        if let Definition::GenericParam(semantics::GenericParam::LifetimeParam(param)) = self {
            let def = match param.parent(db) {
                semantics::GenericDef::Function(it) => it.source(db).map(|src| src.syntax().cloned()),
                semantics::GenericDef::Adt(it) => it.source(db).map(|src| src.syntax().cloned()),
                semantics::GenericDef::Trait(it) => it.source(db).map(|src| src.syntax().cloned()),
                semantics::GenericDef::TypeAlias(it) => it.source(db).map(|src| src.syntax().cloned()),
                semantics::GenericDef::Impl(it) => it.source(db).map(|src| src.syntax().cloned()),
                semantics::GenericDef::Variant(it) => it.source(db).map(|src| src.syntax().cloned()),
                semantics::GenericDef::Const(it) => it.source(db).map(|src| src.syntax().cloned()),
            };
            return match def {
                Some(def) => SearchScope::file_range(def.as_ref().original_file_range(db)),
                None => SearchScope::single_file(file_id),
        ...
        if let Definition::Macro(macro_def) = self {
            return match macro_def.kind() {
                semantics::MacroKind::Declarative => {
                ...
                semantics::MacroKind::BuiltIn => SearchScope::crate_graph(db),
                ...
                semantics::MacroKind::Derive | semantics::MacroKind::Attr | semantics::MacroKind::ProcMacro => {
                    SearchScope::reverse_dependencies(db, module.krate())
                }
            };
        }
...
pub struct FindUsages<'a> {
    ...
    include_self_kw_refs: Option<semantics::Type>,
...
fn def_to_ty(sema: &Semantics<RootDatabase>, def: &Definition) -> Option<semantics::Type> {
    match def {
