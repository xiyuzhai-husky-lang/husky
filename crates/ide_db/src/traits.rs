...
pub fn resolve_target_trait(
    ...
    impl_def: &ast::Impl,
) -> Option<semantics::Trait> {
    let ast_path =
        impl_def.trait_().map(|it| it.syntax().clone()).and_then(ast::PathType::cast)?.path()?;

    match sema.resolve_path(&ast_path) {
        Some(semantics::PathResolution::Def(semantics::ModuleDef::Trait(def))) => Some(def),
        _ => None,
    }
}
...
pub fn get_missing_assoc_items(
    ...
    resolve_target_trait(sema, impl_def).map_or(vec![], |target_trait| {
        target_trait
            ...
            .into_iter()
            .filter(|i| match i {
                semantics::AssocItem::Function(f) => {
                    !impl_fns_consts.contains(&f.name(sema.db).to_string())
                }
                semantics::AssocItem::TypeAlias(t) => !impl_type.contains(&t.name(sema.db).to_string()),
                semantics::AssocItem::Const(c) => c
                    .name(sema.db)
