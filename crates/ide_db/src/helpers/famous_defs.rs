//! See [`FamousDefs`].
use semantics::{Crate, Enum, MacroDef, Module, ScopeDef, Semantics, Trait};
...
impl FamousDefs<'_, '_> {
    ...
    fn find_trait(&self, path: &str) -> Option<Trait> {
        match self.find_def(path)? {
            semantics::ScopeDef::ModuleDef(semantics::ModuleDef::Trait(it)) => Some(it),
            _ => None,
        }
    }
    ...
    fn find_macro(&self, path: &str) -> Option<MacroDef> {
        match self.find_def(path)? {
            semantics::ScopeDef::MacroDef(it) => Some(it),
            _ => None,
        }
    }
    ...
    fn find_enum(&self, path: &str) -> Option<Enum> {
        match self.find_def(path)? {
            semantics::ScopeDef::ModuleDef(semantics::ModuleDef::Adt(semantics::Adt::Enum(it))) => Some(it),
            _ => None,
        }
    }
    ...
    fn find_module(&self, path: &str) -> Option<Module> {
        match self.find_def(path)? {
            semantics::ScopeDef::ModuleDef(semantics::ModuleDef::Module(it)) => Some(it),
            _ => None,
        }
    }
