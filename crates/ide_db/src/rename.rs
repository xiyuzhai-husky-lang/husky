...
use either::Either;
use semantics::{AsAssocItem, FieldSource, HasSource, InFile, ModuleSource, Semantics};
use stdx::never;
use syntax::{
    ast::{self, HasName},
    ...
    pub fn range_for_rename(self, sema: &Semantics<RootDatabase>) -> Option<FileRange> {
        let res = match self {
            ...
            Definition::Adt(adt) => match adt {
                semantics::Adt::Struct(it) => name_range(it, sema),
                semantics::Adt::Union(it) => name_range(it, sema),
                semantics::Adt::Enum(it) => name_range(it, sema),
            },
            ...
            Definition::GenericParam(generic_param) => match generic_param {
                semantics::GenericParam::TypeParam(type_param) => {
                ...
                semantics::GenericParam::LifetimeParam(lifetime_param) => {
                    let src = lifetime_param.source(sema.db)?;
                    let lifetime = src.value.lifetime()?;
                    src.with_value(lifetime.syntax()).original_file_range_opt(sema.db)
                }
                semantics::GenericParam::ConstParam(it) => name_range(it, sema),
            },
...
fn rename_reference(
    ...
    if matches!(
        def,
        Definition::GenericParam(semantics::GenericParam::LifetimeParam(_)) | Definition::Label(_)
    ) {
        match ident_kind {
        ...
        Some(assoc) => assoc
            ...
            .and_then(|trait_| {
                trait_.items(sema.db).into_iter().find_map(|it| match (it, assoc) {
                    (semantics::AssocItem::Function(trait_func), semantics::AssocItem::Function(func))
                        if trait_func.name(sema.db) == func.name(sema.db) =>
                    {
                        Some(Definition::Function(trait_func))
                    }
                    (semantics::AssocItem::Const(trait_konst), semantics::AssocItem::Const(konst))
                        if trait_konst.name(sema.db) == konst.name(sema.db) =>
                    {
                        Some(Definition::Const(trait_konst))
                    }
                    (
                        semantics::AssocItem::TypeAlias(trait_type_alias),
                        semantics::AssocItem::TypeAlias(type_alias),
