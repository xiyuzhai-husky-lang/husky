    ...
    pub fn classify(sema: &Semantics<RootDatabase>, name: &ast::Name) -> Option<NameClass> {
        ...
        match_ast! {
            match parent {
                ...
                ast::RecordField(it) => {
                    let field: semantics::Field = sema.to_def(&it)?;
                ...
                ast::Struct(it) => {
                    let def: semantics::Struct = sema.to_def(&it)?;
                    Some(NameClass::Definition(Definition::Adt(def.into())))
                },
                ast::Union(it) => {
                    let def: semantics::Union = sema.to_def(&it)?;
                    Some(NameClass::Definition(Definition::Adt(def.into())))
                },
                ast::Enum(it) => {
                    let def: semantics::Enum = sema.to_def(&it)?;
                    Some(NameClass::Definition(Definition::Adt(def.into())))
                },
                ast::Trait(it) => {
                    let def: semantics::Trait = sema.to_def(&it)?;
                    Some(NameClass::Definition(Definition::Trait(def)))
                },
                ast::Static(it) => {
                    let def: semantics::Static = sema.to_def(&it)?;
                    Some(NameClass::Definition(Definition::Static(def)))
                },
                ast::Variant(it) => {
                    let def: semantics::Variant = sema.to_def(&it)?;
                    Some(NameClass::Definition(Definition::Variant(def)))
                },
                ast::Fn(it) => {
                    let def: semantics::Function = sema.to_def(&it)?;
                    Some(NameClass::Definition(Definition::Function(def)))
                },
                ast::Const(it) => {
                    let def: semantics::Const = sema.to_def(&it)?;
                    Some(NameClass::Definition(Definition::Const(def)))
                },
                ast::TypeAlias(it) => {
                    let def: semantics::TypeAlias = sema.to_def(&it)?;
                ...
                if let PathResolution::Def(ModuleDef::Trait(tr)) = resolved {
                    ...
                    if let Some(ty) = tr
                        ...
                        .filter_map(|assoc| match assoc {
                            semantics::AssocItem::TypeAlias(it) => Some(*it),
                            _ => None,
                        })
        ...
        if let Some(path) = name_ref.syntax().ancestors().find_map(ast::Path::cast) {
            ...
            return match is_attribute_path {
                Some(true) if path == top_path => sema
                    ...
                    .filter(|mac| mac.kind() == semantics::MacroKind::Attr)
...
impl AsAssocItem for Definition {
    fn as_assoc_item(self, db: &dyn semantics::db::HirDatabase) -> Option<AssocItem> {
        match self {
        ...
        match path_resolution {
            ...
            PathResolution::AssocItem(item) => {
                let def: ModuleDef = match item {
                    semantics::AssocItem::Function(it) => it.into(),
                    semantics::AssocItem::Const(it) => it.into(),
                    semantics::AssocItem::TypeAlias(it) => it.into(),
                };
                def.into()
            }
