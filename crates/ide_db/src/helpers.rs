...
use base_db::FileID;
use semantics::{ItemInNs, MacroDef, ModuleDef, Name, PathResolution, Semantics};
...
pub fn try_resolve_derive_input(
    ...
    match scope.speculative_resolve_as_mac(&path) {
        Some(mac) if mac.kind() == semantics::MacroKind::Derive => Some(PathResolution::Macro(mac)),
        Some(_) => return None,
        None => scope
...
pub fn mod_path_to_ast(path: &semantics::ModPath) -> ast::Path {
    ...
    match path.kind {
        ...
        semantics::PathKind::Super(0) => segments.push(make::path_segment_self()),
        semantics::PathKind::Super(n) => segments.extend((0..n).map(|_| make::path_segment_super())),
        semantics::PathKind::DollarCrate(_) | semantics::PathKind::Crate => {
            segments.push(make::path_segment_crate())
        }
        semantics::PathKind::Abs => is_abs = true,
    }
...
pub fn visit_file_defs(
    ...
    while let Some(def) = defs.pop_front() {
        if let ModuleDef::Module(submodule) = def {
            if let semantics::ModuleSource::Module(_) = submodule.definition_source(db).value {
