...
use super::{CCursorRange, CodeEditState, CursorRange, TextEditOutput};
...
impl<'t> WidgetWithState for CodeEdit<'t> {
    type State = CodeEditState;
}

impl<'t> CodeEdit<'t> {
    pub fn load_state(ctx: &Context, id: Id) -> Option<CodeEditState> {
        CodeEditState::load(ctx, id)
    }

    pub fn store_state(ctx: &Context, id: Id, state: CodeEditState) {
        state.store(ctx, id);
    }
}
...
impl<'t> CodeEdit<'t> {
    ...
    fn show_content(self, ui: &mut Ui) -> TextEditOutput {
        ...
        let mut state = CodeEditState::load(ui.ctx(), id).unwrap_or_default();
