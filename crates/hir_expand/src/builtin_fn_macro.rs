...
fn asm_expand(
    ...
    for tt in tt.token_trees.chunks(2) {
        match tt {
            ...
            | [tt::TokenTree::Leaf(tt::Leaf::Literal(lit)), tt::TokenTree::Leaf(tt::Leaf::Punct(tt::Punct {
                char: ',',
                id: _,
                spacing: _,
            }))] => {
...
fn compile_error_expand(
    ...
    ExpandResult {
        value: Some(ExpandedEager::new(quote! {})),
        err: Some(err),
    }
}
...
fn concat_expand(
    ...
    ExpandResult {
        value: Some(ExpandedEager::new(quote!(#text))),
        err,
    }
}
...
fn concat_idents_expand(
    ...
    ExpandResult {
        value: Some(ExpandedEager::new(quote!(#ident))),
        err,
    }
}
...
fn relative_file(
    ...
    if res == call_site && !allow_recursion {
        Err(mbe::ExpandError::Other(format!(
            "recursive inclusion of `{}`",
            path_str
        )))
    } else {
        Ok(res)
...
fn include_expand(
    ...
    match res {
        Ok((subtree, file_id)) => ExpandResult::ok(Some(ExpandedEager {
            subtree,
...
fn env_expand(
    ...
    ExpandResult {
        value: Some(ExpandedEager::new(expanded)),
        err,
    }
}
