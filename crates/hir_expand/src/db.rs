...
use syntax::{
    algo::diff,
    ast::{self, HasAttrs},
    AstNode, GreenNode, ParseResult, SyntaxNode, SyntaxToken, T,
};
...
pub enum TokenExpander {
    ...
    DeclarativeMacro {
        mac: mbe::DeclarativeMacro,
        def_site_token_map: mbe::TokenMap,
    },
...
pub trait AstDatabase: SourceDatabase {
    ...
    ) -> ExpandResult<Option<(ParseResult<SyntaxNode>, Arc<mbe::TokenMap>)>>;
...
fn ast_id_map(db: &dyn AstDatabase, file_id: HirFileID) -> Arc<AstIdMap> {
    let map = db
        ...
        .map(|it| AstIdMap::from_source(&it))
        .unwrap_or_default();
    Arc::new(map)
}
...
fn parse_or_expand(db: &dyn AstDatabase, file_id: HirFileID) -> Option<SyntaxNode> {
    match file_id.0 {
        ...
        HirFileIDRepr::MacroFile(macro_file) => db
            .parse_macro_expansion(macro_file)
            .value
            .map(|(it, _)| it.syntax_node()),
    }
}
...
fn macro_def(db: &dyn AstDatabase, id: MacroDefId) -> Result<Arc<TokenExpander>, mbe::ParseError> {
    match id.kind {
        MacroDefKind::Declarative(ast_id) => {
            ...
            Ok(Arc::new(TokenExpander::DeclarativeMacro {
                mac,
                def_site_token_map,
            }))
        }
        ...
        MacroDefKind::BuiltInEager(..) => {
            ...
            Err(mbe::ParseError::UnexpectedToken(
                "unexpected eager macro".to_string(),
            ))
        }
...
fn is_self_replicating(from: &SyntaxNode, to: &SyntaxNode) -> bool {
    ...
    if let Some(stmts) = ast::MacroStmts::cast(from.clone()) {
        if stmts
            .statements()
            .any(|stmt| diff(stmt.syntax(), to).is_empty())
        {
            return true;
        }
...
fn token_tree_to_syntax_node(
    tt: &tt::Subtree,
    expand_to: ExpandTo,
) -> Result<(ParseResult<SyntaxNode>, mbe::TokenMap), ExpandError> {
