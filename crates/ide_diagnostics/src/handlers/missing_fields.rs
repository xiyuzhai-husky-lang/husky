...
// let a = A { a: 10 };
// ```
pub(crate) fn missing_fields(ctx: &DiagnosticsContext<'_>, d: &semantics::MissingFields) -> Diagnostic {
...
fn fixes(ctx: &DiagnosticsContext<'_>, d: &semantics::MissingFields) -> Option<Vec<Assist>> {
    ...
    ctx.sema.scope(field_list_parent.syntax()).process_all_names(&mut |name, def| {
        if let semantics::ScopeDef::Local(local) = def {
