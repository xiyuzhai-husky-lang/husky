    ...
    F: Fn(ItemPath) -> &'a SemStaticVarDeps,
{
    pub(crate) fn calc_root(&mut self) -> SemStaticVarDeps {
        ...
        deps.merge(
            &self.expr_control_flow_static_var_deps_table[root_body],
            &mut self.counter,
        );
        deps
    }
    ...
    fn calc_expr_value(&mut self, expr: SemExprIdx) -> SemStaticVarDeps {
        match *expr.data(self.sem_expr_region_data.sem_expr_arena()) {
            ...
            SemExprData::Binary {
                ...
                deps.merge(
                    &self.expr_value_static_var_deps_table[ropd],
                    &mut self.counter,
                );
            ...
            SemExprData::FunctionRitchieCall {
                ...
                deps.merge(
                    &self.expr_value_static_var_deps_table[function],
                    &mut self.counter,
                );
            ...
            SemExprData::MethodRitchieCall {
                ...
                if self_contract == Contract::BorrowMut {
                    self.expr_value_static_var_deps_table[self_argument]
                        .merge(&deps, &mut self.counter);
                }
                deps
            }
            ...
            SemExprData::Index {
                ...
                for item in index_sem_list_items {
                    deps.merge(
                        &self.expr_value_static_var_deps_table[item.sem_expr_idx],
                        &mut self.counter,
                    )
                }
            ...
            SemExprData::EmptyHtmlTag {
                ...
                for argument in arguments {
                    deps.merge(
                        &self.expr_value_static_var_deps_table[argument.expr()],
                        &mut self.counter,
                    )
                }
                deps
    ...
    fn calc_expr_control_flow(&mut self, expr: SemExprIdx) -> SemStaticVarDeps {
        match *expr.data(self.sem_expr_region_data.sem_expr_arena()) {
            ...
            SemExprData::Binary {
                ...
                deps.merge(
                    &self.expr_control_flow_static_var_deps_table[ropd],
                    &mut self.counter,
                );
                deps
            }
            ...
            SemExprData::Unveil {
                ...
                deps.merge(
                    &self.expr_value_static_var_deps_table[opd],
                    &mut self.counter,
                );
                let path_deps = self.calc_path(unveil_assoc_fn_path);
                deps.merge(path_deps, &mut self.counter);
            ...
            SemExprData::Index {
                ...
                for item in index_sem_list_items {
                    deps.merge(
                        &self.expr_control_flow_static_var_deps_table[item.sem_expr_idx],
                        &mut self.counter,
                    )
                }
                match index_dynamic_dispatch.signature() {
    ...
    fn calc_stmt_control_flow(&mut self, stmt: SemStmtIdx) -> SemStaticVarDeps {
        match *stmt.data(self.sem_expr_region_data.sem_stmt_arena()) {
            ...
            SemStmtData::Return { result, .. } => {
                ...
                deps.merge(
                    &self.expr_value_static_var_deps_table[result],
                    &mut self.counter,
                );
                deps
            }
            ...
            SemStmtData::ForBetween {
                ...
                for stmt in stmts {
                    deps.merge(
                        &self.stmt_control_flow_static_var_deps_table[stmt],
                        &mut self.counter,
                    );
                }
                deps
        ...
        for variable in variables {
            self.variable_static_var_deps_table
                .insert_new_current_or_merge(variable, deps.clone(), |deps0, deps| {
                    SemStaticVarDeps::merge(deps0, &deps, &mut self.counter)
                })
        }
    }
