...
impl<'a> DependeeMapBuilder<'a> {
    ...
    fn push(&mut self, entity_path: Ty) {
        todo!()
        ...
        // }
        // if !self.map.has(entity_path) {
        //     self.map
        //         .insert_new((entity_path, self.db.entity_defn_uid(entity_path)))
        //         .unwrap();
        // }
    }
...
pub(crate) fn entity_immediate_dependees(
    ...
    todo!()
    // let defn = db.entity_defn(entity_path)?;
...
pub(crate) fn entity_dependees(
    ...
    let mut dependees = (*db.entity_immediate_dependees(entity_path)?).clone();
...
impl EntityDefn {
    fn immediate_dependees(&self, db: &dyn EntityDefnQueryGroup) -> VecPairMap<Ty, EntityDefnUid> {
        ...
        //         ..
        //     } => {
        //         extract_call_head_dependees(parameters, &mut builder);
        ...
        //     //     LazyExprVariant::EntityFeature { entity_path: route } => builder.push(route),
