...
impl ArticleParser<'_> {
    pub fn parse_item(&mut self) -> Result<Option<Item>> {
        ...
        Ok(match self.parse_elem()? {
            e @ (ArticleElem::Item(_)
            | ArticleElem::AuxiliaryItem(_)
            | ArticleElem::Canceled(_)) => Some(e.into()),
    ...
    fn parse_reasoning(
        &mut self,
        diffuse: bool,
    ) -> Result<(Vec<(Item, Option<Thesis>)>, Position)> {
        let mut items = vec![];
        let end = loop {
            let e = self.parse_elem()?;
            match e {
                ArticleElem::AuxiliaryItem(_) | ArticleElem::PerCases(_) => {
                    items.push((e.into(), None))
                }
                ArticleElem::Proposition(prop) => {
                    items.push((self.finish_proposition(prop)?, None))
                }
                ArticleElem::Let(_)
                | ArticleElem::Thus(_)
    ...
    fn parse_justification(&mut self) -> Result<Justification> {
        ...
        Ok(match e.local_name().as_ref() {
            b"By" => {
                let linked = e
                    .try_get_attribute(b"linked")
                    .unwrap()
                    .map_or(false, |attr| &*attr.value == b"true");
            ...
            b"From" => {
                ...
                Justification::Simple(Inference {
                    kind: InferenceKind::From {
                        sch: (ArticleId(article_nr), SchId(nr)),
                    },
                    pos: self.r.get_pos(&e)?,
                    refs: self.parse_references()?,
                })
    ...
    fn parse_case_block(
        &mut self,
        (start, label): (Position, Option<LabelId>),
    ) -> Result<CaseBlock> {
        ...
        Ok(if let Event::Start(e) = self.r.read_event(&mut self.buf)? {
            match e.local_name().as_ref() {
                b"DefinitionBlock" => {
                    ...
                    let end = loop {
                        ...
                        match e {
                            ...
                            ArticleElem::Proposition(prop) => {
                                items.push(self.finish_proposition(prop)?)
                            }
                            ArticleElem::EndPosition(pos) => break pos,
                ...
                b"NotationBlock" => {
                    ...
                    let end = loop {
                        ...
                        match e {
                            ArticleElem::AuxiliaryItem(_)
                            | ArticleElem::Let(_)
                            | ArticleElem::Pattern(_) => items.push(e.into()),
                            ArticleElem::Proposition(prop) => {
                                items.push(self.finish_proposition(prop)?)
                            }
                            ArticleElem::EndPosition(pos) => break pos,
                ...
                b"Reservation" => {
                    ...
                    let ty = loop {
                        match self.r.parse_elem(&mut self.buf)? {
                            MizElem::Ident(id) => ids.push(id),
                            MizElem::Type(ty) => break Box::new(ty),
                ...
                b"SchemeBlock" => {
                    ...
                    let nr = self.r.get_attr::<u32>(
                        &e.try_get_attribute(b"schemenr").unwrap().unwrap().value,
                    )? - 1;
                    let mut decls = vec![];
                    loop {
                ...
                b"IterEquality" => {
                    ...
                    while self
                        .r
                        .try_read_start(&mut self.buf, Some("IterStep"))?
                        .is_ok()
                    {
                ...
                b"Consider" => {
                    ...
                    loop {
                        match self.r.parse_elem(&mut self.buf)? {
                            MizElem::Type(ty) if parsing_fixed => fixed.push(ty),
                            MizElem::Proposition(prop) => {
                                parsing_fixed = false;
                                intro.push(prop)
                            }
                    ...
                    ArticleElem::AuxiliaryItem(AuxiliaryItem::Consider {
                        prop,
                        just,
                        fixed,
                        intro,
                ...
                b"Reconsider" => {
                    ...
                    let prop = loop {
                        match self.r.parse_elem(&mut self.buf)? {
                            MizElem::Type(ty) => {
                                terms.push((ty, self.r.parse_term(&mut self.buf)?.unwrap()))
                            }
                            MizElem::Proposition(prop) => break prop,
                            _ => panic!("expected proposition"),
                        }
                    };
                    ...
                    if kind == b'G' {
                        ...
                        let cl = loop {
                            match self.parse_elem()? {
                                ...
                                ArticleElem::Registration(Registration::Cluster(decl)) => {
                                    break decl
                                }
                                _ => panic!("expected cluster"),
                            }
                        };
                        let (mut conds, mut corr, mut patts, mut state) =
                            (vec![], None, vec![], false);
                        loop {
                            match self.parse_elem()? {
                        ...
                        ArticleElem::DefStruct(DefStruct {
                            pos,
                            label,
                            constrs,
                            cl,
                ...
                b"Conclusion" => {
                    let s = match self.parse_elem()? {
                        ...
                        ArticleElem::Proposition(prop) => Statement::Proposition {
                            prop,
                            just: self.parse_justification()?,
                        },
                    ...
                    let (prop, just) = loop {
                        match self.parse_elem()? {
                            ArticleElem::BlockThesis(f) => {
                                assert!(block_thesis.replace(f).is_none())
                            }
                ...
                b"Registration" => {
                    ...
                    let kind = match self.r.parse_cluster_attrs(&e)? {
                        (aid, ClusterKind::R) => {
                            ClusterDeclKind::R(self.r.parse_rcluster(&mut self.buf, aid)?)
                        }
                        (aid, ClusterKind::F) => {
                            ClusterDeclKind::F(self.r.parse_fcluster(&mut self.buf, aid)?)
                        }
                        (aid, ClusterKind::C) => {
                            ClusterDeclKind::C(self.r.parse_ccluster(&mut self.buf, aid)?)
                        }
                    };
                    let (conds, corr) = self.parse_cond_and_correctness()?;
                    ArticleElem::Registration(Registration::Cluster(ClusterDecl {
                        kind,
                        conds,
                        corr,
                    }))
