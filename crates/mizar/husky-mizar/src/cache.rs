...
pub fn init_cache<'a>(articles: impl Iterator<Item = (&'a str, bool)>) {
    let articles = ["hidden", "tarski_0", "tarski_a"]
        ...
        .map(|(s, wait)| {
            (
                Article::from_lower(s.as_bytes()).unwrap(),
                Cache {
                    wait,
                    ..Cache::default()
                },
...
impl MizPath {
    ...
    fn get_cache<A, T, R, E>(
        ...
        no: bool,
        args: &mut A,
        get: impl FnOnce(&Cache) -> &OnceCell<T>,
        read: impl FnOnce(&mut A, bool) -> Result<R, E>,
        take: impl FnOnce(&mut A, R) -> T,
    ...
    fn get_cache_basic<A: Default + Clone, R, E>(
        ...
        no: bool,
        args: &mut A,
        get: impl FnOnce(&Cache) -> &OnceCell<A>,
        read: impl FnOnce(&mut A) -> Result<R, E>,
        result: impl FnOnce(&A) -> Result<R, E>,
    ) -> Result<R, E> {
        self.get_cache(
            no,
            args,
    ...
    pub fn read_def(
        ...
        self.get_cache(
            ...
            |c| &c.def,
            |(sig, defs), _| {
                self.read_definitions(MaybeMut::None, new_prel, "def", Some(sig), defs)
            },
    ...
    pub fn read_did(
        ...
        self.get_cache(
            ...
            |c| &c.did,
            |(sig, did), _| {
                self.read_identify_regs(MaybeMut::None, new_prel, "did", Some(sig), did)
            },
    ...
    pub fn read_drd(
        ...
        self.get_cache(
            ...
            |c| &c.drd,
            |(sig, drd), _| {
                self.read_reduction_regs(MaybeMut::None, new_prel, "drd", Some(sig), drd)
            },
