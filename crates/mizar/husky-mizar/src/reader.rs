...
impl MizPath {
    pub fn with_reader(
        ...
        if let Some(accom) = &mut v.accom {
            ...
            accom
                .accom_requirements(&v.g.constrs, &mut v.g.reqs)
                .unwrap();
        ...
        if needs_formats && !cfg.parser_enabled {
            ...
            v.formats
                .extend(fmts.formats.enum_iter().map(|(id, f)| (*f, id)));
        ...
        if let Some(accom) = &mut v.accom {
            if let Some(parser) = &mut parser {
                ...
                accom
                    .accom_notations(&mut fmt_map, Some(&mut fmts), &mut notations)
                    .unwrap();
                fmts.formats
                    .0
                    .iter()
                    .for_each(|fmt| parser.read_format(fmt));
                v.formats
                    .extend(fmts.formats.enum_iter().map(|(id, f)| (*f, id)));
                *v.lc.formatter.formats = fmts.formats;
            } else {
                accom
                    .accom_notations(&mut v.formats, None, &mut notations)
                    .unwrap();
            }
        } else {
        ...
        if let Some(accom) = &mut v.accom {
            accom
                .accom_clusters(&v.g.constrs, &mut v.g.clusters)
                .unwrap();
        } else {
        ...
        v.g.clusters
            .functor
            .sort_all(|a, b| FunctorCluster::cmp_term(&a.term, &v.g.constrs, &b.term));
        ...
        if let Some(zero) = v.g.reqs.zero() {
            attrs.push(MizAttr::new0(zero, false))
        }
        if has_omega {
            if let Some(positive) = v.g.reqs.positive() {
                attrs.push(MizAttr::new0(positive, true))
            }
        }
        ...
        if cfg.analyzer_enabled {
            if let Some(accom) = &mut v.accom {
                ...
                self.read_definitions(&v.g.constrs, false, "dfs", None, &mut v.definitions.0)
                    .unwrap();
            }
        ...
        if cfg.checker_enabled {
            if let Some(accom) = &mut v.accom {
                ...
                self.read_definitions(&v.g.constrs, false, "dfe", None, &mut v.equalities)
                    .unwrap();
            }
            if cfg.dump.definitions {
        ...
        if cfg.checker_enabled {
            if let Some(accom) = &mut v.accom {
                ...
                self.read_definitions(&v.g.constrs, false, "dfx", None, &mut v.expansions)
                    .unwrap();
            }
            if cfg.dump.definitions {
        ...
        if let Some(accom) = &mut v.accom {
            accom
                .accom_properties(&v.g.constrs, &mut v.properties)
                .unwrap();
        } else {
            self.read_properties(&v.g.constrs, false, "epr", None, &mut v.properties)
                .unwrap();
        }
        ...
        if cfg.checker_enabled || cfg.exporter_enabled {
            if let Some(accom) = &mut v.accom {
                accom
                    .accom_identify_regs(&v.g.constrs, &mut v.identify)
                    .unwrap();
                accom
                    .accom_reduction_regs(&v.g.constrs, &mut v.reductions)
                    .unwrap();
            } else {
                self.read_identify_regs(&v.g.constrs, false, "eid", None, &mut v.identify)
                    .unwrap();
                self.read_reduction_regs(&v.g.constrs, false, "erd", None, &mut v.reductions)
                    .unwrap();
            }
        }

        // in mizar this was done inside the parser
        MizRoundUpTypes::with(&v.g, &mut v.lc, |rr| {
        ...
        for (i, id) in v.identify.iter().enumerate() {
            let Term::Functor { nr, .. } = id.lhs else {
                unreachable!()
            };
        ...
        clusters
            .registered
            .iter_mut()
            .for_each(|c| c.consequent.1.visit(cc));
        clusters
            .conditional
            .iter_mut()
            .for_each(|c| c.consequent.1.visit(cc));
        ...
        if cfg.checker_enabled {
            if let Some(accom) = &mut v.accom {
                accom
                    .accom_theorems(cfg.xml_internals, &v.g.constrs, &mut v.def_map, &mut v.libs)
                    .unwrap();
            } else {
            ...
            if let Some(accom) = &mut v.accom {
                accom
                    .accom_schemes(cfg.xml_internals, &v.g.constrs, &mut v.libs)
                    .unwrap();
            } else {
                self.read_esh(&v.g.constrs, refs, &mut v.libs).unwrap();
            }
            MizRoundUpTypes::with(&v.g, &mut v.lc, |rr| v.libs.visit(rr));
...
impl Reader {
    ...
    pub(crate) fn close_scope(&mut self, sc: Scope, check_for_local_const: bool) -> Descope {
        ...
        let mut descope =
            self.lc
                .truncate_infer_const(&self.g.constrs, check_for_local_const, sc.infer_const);
        ...
        if descope.remap.is_empty() {
            ...
            for x in self.pending_defs.drain(sc.pending_defs..) {
                match x {
                    ...
                    PendingDef::Cluster(ClusterKind::F, i) => {
                        self.g.clusters.functor[i].visit(&mut descope)
                    }
    ...
    pub fn read_item(&mut self, it: &Item) {
        ...
        if self.g.cfg.item_header {
            ...
            if self.g.cfg.always_verbose_item || verbose() {
                ...
                match it {
                    ...
                    Item::AuxiliaryItem(it) => match it {
                        AuxiliaryItem::Statement(it) => match it {
                            Statement::Proposition { .. } => {
                                eprintln!("Proposition @ {:?}", it.pos())
                            }
                            Statement::IterEquality { .. } => {
                                eprintln!("IterEquality @ {:?}", it.pos())
                            }
                            Statement::Now { .. } => eprintln!("Now @ {:?}", it.pos()),
                        },
                        AuxiliaryItem::Consider { prop, .. } => {
                            eprintln!("Consider @ {:?}", prop.pos)
                        }
                        AuxiliaryItem::Set { .. } => eprintln!("Set"),
                        AuxiliaryItem::Reconsider { prop, .. } => {
                            eprintln!("Reconsider @ {:?}", prop.pos)
                        }
                    ...
                    Item::DefTheorem { kind, prop } => {
                        eprintln!("DefTheorem {kind:?} @ {:?}", prop.pos)
                    }
        ...
        match it {
            ...
            Item::AuxiliaryItem(AuxiliaryItem::Statement(it)) | Item::Thus(it) => {
                self.read_stmt(it)
            }
            Item::AuxiliaryItem(AuxiliaryItem::Consider {
                prop,
                just,
                fixed,
                intro,
                    ...
                    loop {
                        ...
                        if let Term::Functor { nr, args } = &asgn.def {
                            ...
                            if f == nr {
                                ...
                                drop(ic);
                                if let Some(mut t) =
                                    func.expand_if_equal(&self.g, &self.lc, args, 0)
                                {
                                    t.visit(&mut self.intern_const());
    ...
    fn read_justification(&mut self, thesis: &Formula, just: &Justification) {
        match just {
            ...
            Justification::Proof {
                ...
                assert!(
                    self.g.eq(&self.lc, thesis, &block_thesis),
                    "\n{thesis:?}\n !=\n{block_thesis:?}"
                );
                self.scope(*label, false, |this| {
                    for it in items {
    ...
    fn read_corr_conds(&mut self, conds: &[CorrCond], corr: &Option<Correctness>) {
        conds
            .iter()
            .for_each(|c| self.read_just_prop(&c.prop, &c.just, false));
        if let Some(c) = corr {
            self.read_just_prop(&c.prop, &c.just, false)
        }
    }

    pub fn push_constr(&mut self, id: ConstrKind) {
        self.pending_defs.push(PendingDef::Constr(id))
    }
        ...
        while let Some(asgn) = ic.get(i) {
            match &asgn.def {
                ...
                | Term::SchFunc { args, .. } => {
                    if args
                        .iter()
                        .any(|t| matches!(t, Term::Infer(i) if rounded_up.contains_key(i)))
                    {
    ...
    pub fn read_registered_cluster(&mut self, mut cl: RegisteredCluster) {
        ...
        self.pending_defs
            .push(PendingDef::Cluster(ClusterKind::R, i))
    }
    ...
    pub fn read_conditional_cluster(&mut self, mut cl: ConditionalCluster) {
        ...
        self.pending_defs
            .push(PendingDef::Cluster(ClusterKind::C, i))
    }
    ...
    pub fn read_functor_cluster(&mut self, mut cl: FunctorCluster) {
        ...
        self.round_up_further(rounded_up);
        self.pending_defs
            .push(PendingDef::Cluster(ClusterKind::F, i))
    }
