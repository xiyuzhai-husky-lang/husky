...
impl Reader {
    fn push_parse_item(
        &mut self,
        parser: &mut Result<&mut MizParser<'_>, MsmParser>,
        buf: &mut Vec<ast::Item>,
    ) -> PathResult<bool> {
        match parser {
            Ok(parser) => {
    ...
    pub fn run_analyzer(&mut self, path: &MizPath, parser: Option<&mut MizParser<'_>>) {
        let mut parser = match parser {
            Some(v) => Ok(v),
            None => {
                let result = if self.g.cfg.nameck_enabled {
                    path.open_wsx()
                } else {
                    path.open_msx()
                };
...
/// Agree
fn agrees(g: &MizGlobal, lc: &LocalContext, tms: &[TermQua], tys: &[Type]) -> bool {
    let mut subst = Subst {
        subst_term: tms.iter().map(|tm| Some(CowBox::Borrowed(tm))).collect(),
    };
...
impl Analyzer<'_> {
    ...
    fn open_scope(&mut self, push_label: bool, copy_thesis: bool) -> Scope {
        self.thesis_stack.push(if copy_thesis {
            self.thesis.clone()
        } else {
            self.thesis.take()
        });
        ...
        match &mut it.kind {
            ...
            ast::ItemKind::Theorem { prop, just } => {
                ...
                if self.g.cfg.exporter_enabled {
                    self.export.theorems.push(Theorem {
                        pos: it.pos,
                        kind: TheoremKind::Thm,
                        stmt: f,
                    })
                }
            }
    ...
    fn elab_scheme(&mut self, ast::SchemeBlock { end, head, items }: &mut ast::SchemeBlock) {
        ...
        for group in groups {
            match group {
                ast::SchemeBinderGroup::Func { vars, tys, ret, .. } => {
                    ...
                    for _ in 1..vars.len() {
                        self.sch_func_args
                            .push(self.r.lc.locus_ty.0.to_vec().into());
                        self.r.lc.sch_func_ty.push(ret.clone());
                    }
                    self.sch_func_args
                        .push(self.r.lc.locus_ty.0.drain(..).collect());
        ...
        sch.sch_funcs
            .iter()
            .for_each(|ty| Exportable.visit_type(ty));
        ...
        match &mut it.kind {
            ast::ItemKind::Set(decls) => {
                for ast::SetDecl { var, value } in decls {
                    ...
                    let c = self.lc.fixed_var.push(FixedVar {
                        ty,
                        def: Some((Box::new(term), true)),
                    });
                    if self.g.cfg.nameck_enabled {
                        Rc::make_mut(&mut self.lookup)
                            .var
                            .insert(var.spelling.clone(), VarKind::Const(c));
                    }
                }
            }
            ast::ItemKind::DefFunc { var, tys, value } => {
                ...
                self.lc.term_cache.get_mut().close_scope();
                let i = self.r.lc.priv_func.push(FuncDef {
                    primary,
                    ty: Box::new(ty),
                    value: Box::new(value),
                });
                if self.g.cfg.nameck_enabled {
                    (Rc::make_mut(&mut self.lookup).func).insert(
                        (var.spelling.clone(), tys.len() as u32),
                        PrivFuncKind::PrivFunc(i),
                    );
                }
            }
            ast::ItemKind::DefPred { var, tys, value } => {
                ...
                if self.g.cfg.nameck_enabled {
                    (Rc::make_mut(&mut self.lookup).pred).insert(
                        (var.spelling.clone(), tys.len() as u32),
                        PrivPredKind::PrivPred(i),
                    );
                }
            }
            ast::ItemKind::Reconsider { vars, ty, just } => {
                ...
                for var in vars {
                    if let ast::ReconsiderVar::Var(v) = var {
                        *var = ast::ReconsiderVar::Equality {
                            var: v.clone(),
                            tm: v.to_term(),
                        }
                    }
                    let ast::ReconsiderVar::Equality { var, tm } = var else {
                        unreachable!()
                    };
                    let tm = Box::new(self.elab_intern_term_no_reserve(tm));
                    if let Some(conjs) = &mut conjs {
                        conjs.push(Formula::Is {
                            term: tm.clone(),
                            ty: Box::new(ty.clone()),
                        });
                    }
    ...
    fn elab_stmt(&mut self, stmt: &mut ast::Statement) -> Formula {
        ...
        match stmt {
            ...
            ast::Statement::IterEquality { prop, just, steps } => {
                ...
                if let Formula::Pred { nr, ref args } = f {
                    if let (nr, [lhs, rhs]) = Formula::adjust_pred(nr, args, Some(&self.g.constrs))
                    {
                        if self.g.reqs.equals_to() == Some(nr) {
                            self.elab_justification(
                                false,
                                &self.g.reqs.mk_eq(lhs.clone(), rhs.clone()),
                                just,
                            );
                            ...
                            for ast::IterStep { rhs, just, .. } in steps {
                                ...
                                self.elab_justification(
                                    false,
                                    &self.g.reqs.mk_eq(mid, rhs.clone()),
                                    just,
                                );
                                mid = rhs;
                            }
                ...
                self.err(
                    prop.f.pos(),
                    MizError::IterEqualityNotAnEquality(Box::new(f.clone())),
                );
                f
            }
            ast::Statement::Now { end, label, items } => {
                ...
                let f = self.scope(label.is_some(), false, true, |this| {
                    ReconstructThesis {
                        stack: vec![ProofStep::Break(true)],
                    }
                    .elab_proof(this, items, *end)
                });
                self.push_prop(label, f.clone());
                f
            }
    ...
    fn elab_fixed_vars(&mut self, ast::BinderGroup { vars, ty }: &mut ast::BinderGroup) {
        ...
        if self.g.cfg.nameck_enabled {
            ...
            for v in vars {
                lookup
                    .var
                    .insert(v.spelling.clone(), VarKind::Const(base.fresh()));
            }
        }
    }
            ...
            let kind = match r.kind {
                ...
                ast::ReferenceKind::UnresolvedPriv(ref name) => {
                    let lab = self
                        ...
                        .enum_iter()
                        .rev()
                        .find(|p| p.1.as_deref() == Some(name));
                    ReferenceKind::Priv(lab.expect("label not found").0)
                }
                ast::ReferenceKind::Global(art, ref frags) => {
                    for frag in frags {
                        out.push(match *frag {
                            ast::RefFragment::Thm { pos, id } => Reference {
                                pos,
                                kind: ReferenceKind::Thm((art, id)),
                            },
                            ast::RefFragment::Def { pos, id } => Reference {
                                pos,
                                kind: ReferenceKind::Def((art, id)),
                            },
                        })
                    }
                    continue;
    ...
    fn elab_justification_intro_reserved(
        ...
        debug_assert!(self.g.cfg.analyzer_full);
        match just {
            &mut ast::Justification::Inference {
                pos,
                ref kind,
                ref refs,
            } => {
                let it = Inference {
                    kind: match kind {
                        ast::InferenceKind::By { link } => InferenceKind::By {
                            linked: link.is_some(),
                        },
                        &ast::InferenceKind::From {
                            sch: ast::SchRef::Resolved(art, id),
                        } => InferenceKind::From { sch: (art, id) },
                        ast::InferenceKind::From {
                            sch: ast::SchRef::UnresolvedPriv(name),
                        } => {
                            let sch = *(self.sch_names.0.get(&**name))
                                .unwrap_or_else(|| panic!("local scheme '{}' not found", name));
                            InferenceKind::From {
                                sch: (ArticleId::SELF, sch),
                            }
                        }
                    },
                    pos,
    ...
    fn elab_def_case<T, U>(
        &mut self,
        def: &mut ast::DefCase<T>,
        f: impl FnOnce(&mut Self, &mut T) -> U + Copy,
    ) -> DefCase<U> {
    ...
    fn elab_def_value(&mut self, value: &mut ast::DefValue, pos: bool) -> DefValue {
        match value {
            ast::DefValue::Term(body) => {
                ...
                let res = DefValue::Term(
                    self.elab_def_body(body, |this, t| this.elab_term_no_reserve(t)),
                );
                self.lc.it_type = it_type;
                res
            }
        ...
        for pat in self.notations[PKC::Func].iter().rev() {
            if pat.fmt == fmt {
                if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
                    let PatternKind::Func(nr) = pat.kind else {
                        unreachable!()
                    };
    ...
    fn elab_pred(&mut self, fmt: FormatPred, args: Vec<TermQua>, pos: bool) -> Formula {
        ...
        for pat in self.notations[PKC::Pred].iter().rev() {
            if pat.fmt == fmt {
                if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
                    let PatternKind::Pred(nr) = pat.kind else {
                        unreachable!()
                    };
    ...
    fn elab_fraenkel(
        &mut self,
        vars: &[ast::BinderGroup],
        scope: &ast::Term,
        compr: Option<&ast::Formula>,
        ...
        let scope_info = (
            self.lookup.clone(),
            self.reserved_lookup.clone(),
            self.reserved_extra_depth,
        );
        ...
        let res = (|| match *tm {
            ...
            ast::Term::PrivFunc {
                ...
                match kind.or_else(|| {
                    self.lookup
                        .func
                        .get(&(spelling.clone(), args.len() as u32))
                        .copied()
                }) {
            ...
            ast::Term::Aggregate {
                ...
                let args = self.elab_terms_qua(args);
                let fmt = self.formats[&Format::Aggr(FormatAggr {
                    sym: sym.0,
                    args: args.len() as u8,
                })];
                for pat in self.notations[PKC::Aggr].iter().rev() {
                    if pat.fmt == fmt {
                        if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
                            let PatternKind::Aggr(nr) = pat.kind else {
                                unreachable!()
                            };
                            let mut args =
                                subst.trim_to(self.g.constrs.aggregate[nr].primary.len());
                            args.iter_mut().for_each(|t| t.strip_qua_mut());
            ...
            ast::Term::SubAggr {
                ...
                for pat in self.notations[PKC::SubAggr].iter().rev() {
                    if pat.fmt == fmt
                        && pat
                            .check_types(&self.g, &self.lc, std::slice::from_ref(&arg))
                            .is_some()
                    {
                        let PatternKind::SubAggr(nr) = pat.kind else {
                            unreachable!()
                        };
            ...
            ast::Term::Selector {
                ...
                for pat in self.notations[PKC::Sel].iter().rev() {
                    if pat.fmt == fmt {
                        if let Some(subst) =
                            pat.check_types(&self.g, &self.lc, std::slice::from_ref(&arg))
                        {
                            let PatternKind::Sel(nr) = pat.kind else {
                                unreachable!()
                            };
            ...
            ast::Term::The { ref ty, .. } => Term::The {
                ty: Box::new(self.elab_type(ty)),
            },
            ast::Term::Fraenkel {
                ref vars,
                ref scope,
            ...
            ast::Term::Qua {
                ...
                value: Box::new(self.elab_term(term)),
                ty: Box::new(self.elab_type(ty)),
            },
            ast::Term::It { .. } => {
    ...
    fn elab_term(&mut self, tm: &ast::Term) -> Term {
        self.elab_term_qua(tm).strip_qua()
    }
        ...
        match attr {
            ...
            ast::Attr::Attr { sym, args, .. } => {
                let args = (args.iter().map(|t| self.elab_term_qua(t)))
                    ...
                    .collect_vec();
                let fmt = self.formats[&Format::Attr(FormatAttr {
                    sym: sym.0,
                    args: args.len() as u8,
                })];
                for pat in self.notations[PKC::Attr].iter().rev() {
                    if pat.fmt == fmt {
                        if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
                            let PatternKind::Attr(nr) = pat.kind else {
                                unreachable!()
                            };
                            let mut args =
                                subst.trim_to(self.g.constrs.attribute[nr].primary.len());
                            args.iter_mut().for_each(|t| t.strip_qua_mut());
                            let (nr, args) = Formula::adjust_attr(nr, &args, Some(&self.g.constrs));
                            return Formula::Attr {
                                nr,
                                args: args.to_owned().into(),
                            }
                            .maybe_neg(pat.pos == pos);
                        }
                    }
                }
                panic!("type error")
    ...
    fn elab_type(&mut self, ty: &ast::Type) -> Type {
        ...
        let res = (|| match ty {
            ast::Type::Mode { sym, args, .. } => {
                let args = self.elab_terms_qua(args);
                let fmt = self.formats[&Format::Mode(FormatMode {
                    sym: sym.0,
                    args: args.len() as u8,
                })];
                    ...
                    if pat.fmt == fmt {
                        if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
                            ...
                            match pat.kind {
                                PatternKind::Mode(nr) => {
                                    let mut args = subst
                                        .trim_to(self.g.constrs.mode[nr].primary.len())
                                        .to_vec();
                                    args.iter_mut().for_each(|t| t.strip_qua_mut());
                                    ty = Type {
            ...
            ast::Type::Struct { sym, args, .. } => {
                let args = self.elab_terms_qua(args);
                let fmt = self.formats[&Format::Struct(FormatStruct {
                    sym: sym.0,
                    args: args.len() as u8,
                })];
                for pat in self.notations[PKC::Struct].iter().rev() {
                    if pat.fmt == fmt {
                        if let Some(subst) = pat.check_types(&self.g, &self.lc, &args) {
                            let PatternKind::Struct(nr) = pat.kind else {
                                unreachable!()
                            };
                            let mut args = subst
                                .trim_to(self.g.constrs.struct_mode[nr].primary.len())
                                .to_vec();
                            args.iter_mut().for_each(|t| t.strip_qua_mut());
                            return Type {
                                kind: TypeKind::Struct(nr),
                                attrs: Default::default(),
                                args,
                            };
                        }
            ...
            ast::Type::Cluster { attrs, ty, .. } => {
                ...
                attrs.iter().rev().for_each(|attr| {
                    ...
                    ty2.attrs
                        .0
                        .insert(Some(&self.g.constrs), &self.lc, attr.clone());
                ...
                for cl in self.g.clusters.registered.iter().rev() {
                    ...
                    if subst.eq_radices(&mut subst.eq_ctx(&self.g, &self.lc), &cl.ty, &ty)
                        && (ty2.attrs.0).is_subset_of(&cl.consequent.1, |a2, a1| {
                            subst.eq(&self.g, &self.lc, a1, a2)
                        })
    ...
    fn elab_flex_and(&mut self, f1: &ast::Formula, f2: &ast::Formula, pos: bool) -> Formula {
        ...
        impl<'a> OneDiff<'a> {
            ...
            fn eq_term(
                ...
                let res = match (t1, t2) {
                    ...
                    ) if n1 == n2 => Self::eq_terms(ctx, ic, args1, args2),
                    ...
                    (
                        ...
                        Self::merge_many(
                            ...
                            args1
                                .iter()
                                .zip(&**args2)
                                .map(|(ty1, ty2)| Self::eq_type(ctx, ic, ty1, ty2)),
                        )
            ...
            fn eq_terms(
                ...
                Self::then(t1.len() == t2.len(), || {
                    Self::merge_many(
                        ...
                        t1.iter()
                            .zip(t2)
                            .map(|(t1, t2)| Self::eq_term(ctx, ic, t1, t2)),
                    )
                })
            }
                        ...
                        Self::then(args1.len() == args2.len(), || {
                            Self::merge_many(
                                ...
                                args1
                                    .iter()
                                    .zip(args2)
                                    .map(|(f1, f2)| Self::eq_formula(ctx, ic, f1, f2)),
                            )
                        })
                    }
                    (
        ...
        impl<'a> Apply<'a> {
            fn fail() -> ! {
                panic!("flex-and construction failed")
            }
            fn term(&mut self, ctx: &mut EqCtx<'a>, t1: &Term, t2: &Term) -> Term {
                ...
                match (t1, t2) {
                    ...
                    (The { ty: ty1 }, The { ty: ty2 }) => The {
                        ty: Box::new(self.ty(ctx, ty1, ty2)),
                    },
                    (
                        Fraenkel {
                            args: args1,
            ...
            fn ty(&mut self, ctx: &mut EqCtx<'a>, ty1: &Type, ty2: &Type) -> Type {
                ...
                Type {
                    attrs,
                    kind: ty1.kind,
                    args: self.terms(ctx, &ty1.args, &ty2.args).into(),
                }
            }
            ...
            fn formula(&mut self, ctx: &mut EqCtx<'a>, f1: &Formula, f2: &Formula) -> Formula {
                ...
                match (f1.skip_priv_pred(), f2.skip_priv_pred()) {
                    (Neg { f: f1 }, Neg { f: f2 }) => Neg {
                        f: Box::new(self.formula(ctx, f1, f2)),
                    },
                    (Is { term: t1, ty: ty1 }, Is { term: t2, ty: ty2 }) => Is {
                        term: Box::new(self.term(ctx, t1, t2)),
                        ty: Box::new(self.ty(ctx, ty1, ty2)),
                    },
                    (And { args: args1 }, And { args: args2 }) => And {
                        args: args1
                            .iter()
                            .zip(args2)
                            .map(|(f1, f2)| self.formula(ctx, f1, f2))
                            .collect(),
                    },
                    (
                        SchPred {
                    ...
                    (
                        ...
                        let terms = Box::new([
                            self.term(ctx, &t1[0], &t2[0]),
                            self.term(ctx, &t1[1], &t2[1]),
                        ]);
                        let scope = ctx.enter(1, |ctx| Box::new(self.formula(ctx, sc1, sc2)));
                        FlexAnd {
                            terms,
        ...
        let Some(natural) = self.g.reqs.natural() else {
            panic!("requirement NUMERALS missing")
        };
        let Some(le) = self.g.reqs.less_or_equal() else {
            panic!("requirement REAL missing")
        };
        ...
        assert!(
            only_constant,
            "can't abstract flex-and term, contains bound variables"
        );
        let natural = Attrs::Consistent(vec![MizAttr::new0(natural, true)]);
        ...
        let mut nat = Box::new(Type {
            kind: ModeId::SET.into(),
            attrs: (natural.clone(), natural),
            args: vec![],
        });
        nat.round_up_with_self(&self.g, &self.lc, false);
        let scope = self.g.with_eq(&self.lc, |ctx| {
            Apply {
                t1: &t1,
                t2: &t2,
                base,
    ...
    fn push_many_bound(&mut self, mut dom: Type, vars: &[ast::Variable]) {
        for v in vars {
            ...
            if self.g.cfg.nameck_enabled {
                Rc::make_mut(&mut self.lookup)
                    .var
                    .insert(v.spelling.clone(), VarKind::Bound(i));
            }
        }
    }
    ...
    fn elab_formula(&mut self, f: &ast::Formula, positive: bool) -> Formula {
        ...
        let res = match f {
            ...
            ast::Formula::Binop {
                ...
                .elab_formula(f1, true)
                .mk_iff(self.elab_formula(f2, true))
                .maybe_neg(positive),
            ...
            ast::Formula::ChainPred { first, rest, .. } => {
                ...
                Formula::mk_and_with(|conjs| {
                    for rhs in rest {
                        ...
                        self.elab_pred(fmt, args, rhs.positive)
                    ...
                    let fmt = FormatPred { sym, left, right };
                    self.elab_pred(fmt, args, first.positive)
                        .append_conjuncts_to(conjs);
                })
                .maybe_neg(positive)
            }
            &ast::Formula::PrivPred {
                pos,
                kind,
                ref spelling,
                ref args,
                ...
                match kind.or_else(|| {
                    ...
                    Some(PrivPredKind::PrivPred(nr)) => {
                        ...
                        args.iter_mut().for_each(|t| t.strip_qua_mut());
                        let base = self.lc.bound_var.len() as u32;
                        let value = value.visit_cloned(&mut Inst::new(
                            &self.g.constrs,
                            &self.lc,
                            &args,
                            base,
            ...
            ast::Formula::Attr {
                ...
                Formula::mk_and_with(|conjs| {
                    for attr in attrs {
                        self.elab_is_attr(attr, true, &term)
                            .append_conjuncts_to(conjs)
                    }
                })
            ...
            ast::Formula::Is {
                ...
                term: Box::new(self.elab_term(term)),
                ty: Box::new(self.elab_type(ty)),
            }
            .maybe_neg(*positive2 == positive),
            ast::Formula::Binder {
                ...
                .elab_forall(vars, st.as_deref(), scope, true)
                .maybe_neg(positive),
            ast::Formula::Binder {
                ...
                .elab_forall(vars, st.as_deref(), scope, false)
                .maybe_neg(!positive),
    ...
    fn try_unfold(
        ...
        let DefValue::Formula(value) = &def.value else {
            unreachable!()
        };
        let (pos2, f2) = if value.cases.is_empty() {
            (
                pos,
                value.otherwise.as_ref().unwrap().visit_cloned(&mut inst),
            )
        } else {
            let f = Formula::mk_and_with(|disjs| {
                ...
                for case in &*value.cases {
                    ...
                    let f = Formula::mk_and_with(|conj| {
                        ...
                        case.case
                            .visit_cloned(&mut inst)
                            .maybe_neg(pos)
                            .append_conjuncts_to(conj);
                    });
                    f.mk_neg().append_conjuncts_to(disjs)
                }
                if let Some(ow) = &value.otherwise {
                    ow.visit_cloned(&mut inst)
                        .maybe_neg(pos)
                        .append_conjuncts_to(&mut otherwise);
                    Formula::mk_and(otherwise)
                        .mk_neg()
                        .append_conjuncts_to(disjs)
                }
            });
            (false, f)
        };
            ...
            let (kind, args) = match &mut *f.1 {
                ...
                Formula::FlexAnd { .. } => {
                    let Formula::FlexAnd {
                        ...
                        terms,
                        scope,
                    } = std::mem::take(&mut *f.1)
                    else {
                        unreachable!()
                    };
                    *f.1 = MizGlobal::expand_flex_and(nat, le, *terms, scope, 0);
                    continue 'start;
                }
    ...
    fn inst_forall(&self, term: &Term, widenable: bool, up: bool, f: &mut (bool, Box<Formula>)) {
        ...
        let (true, Formula::ForAll { dom, scope }) = (f.0, &mut *f.1) else {
            panic!("not a forall")
        };
    ...
    fn register_cluster(&mut self, mut attrs: Attrs, primary: Box<[Type]>, mut ty: Type) {
        ...
        attrs.enlarge_by(&self.g.constrs, &self.lc, &ty.attrs.1);
        attrs.round_up_with(&self.g, &self.lc, &ty, false);
        let Attrs::Consistent(_) = attrs else {
            panic!("inconsistent existential cluster")
        };
        ty.attrs = (Attrs::EMPTY, Attrs::EMPTY);
        self.read_registered_cluster(RegisteredCluster {
            cl: Cluster {
                primary,
                consequent: (attrs1, attrs),
            },
            ty: Box::new(ty),
        });
    }
...
impl UnfoldConjIter {
    ...
    fn into_vec(self) -> Vec<Formula> {
        self.iter
            .chain(self.stack.into_iter().rev().flatten())
            .collect()
    }
    fn is_empty(&self) -> bool {
        self.iter.as_slice().is_empty() && self.stack.is_empty()
    }
}
...
impl<'a> PropertiesBuilder<'a> {
    ...
    fn reflexivity(&self, lc: &LocalContext, args: [ConstId; 2], pos: bool) -> Formula {
        ...
        Formula::forall(
            ty.clone(),
            self.the_formula(args, 1, 0, [0, 0]).maybe_neg(pos),
        )
    }
    ...
    fn asymmetry(&self, lc: &LocalContext, args: [ConstId; 2], pos1: bool, pos2: bool) -> Formula {
        let ty = &lc.fixed_var[args[0]].ty;
        let f = Formula::mk_and_with(|conj| {
            self.the_formula(args, 2, 0, [0, 1])
                .maybe_neg(pos1)
                .append_conjuncts_to(conj);
            self.the_formula(args, 2, 0, [1, 0])
                .maybe_neg(pos2)
                .append_conjuncts_to(conj);
        });
            ...
            if g.cfg.analyzer_full {
                let thesis = match (prop.kind, &self.kind) {
                    ...
                    (
                        ...
                        &PropertyDeclKind::Pred(Args::Binary(args), pos),
                    ...
                    (
                        PropertyKind::Commutativity,
                        &PropertyDeclKind::Func(t, Args::Binary(args)),
                    ) => {
                        let ty = &lc.fixed_var[args[0]].ty;
                        if let Some((nr, args2)) = t {
                            let term = |tgt| {
                                ...
                                let args = args2
                                    .iter()
                                    .map(|&c| Term::Const(c).visit_cloned(inst))
                                    .collect();
                            ...
                            Formula::forall(
                                ...
                                Formula::forall(
                                    ty.clone(),
                                    g.reqs.mk_eq(term([0, 1]), term([1, 0])),
                                ),
                            )
                        } else {
                            let f = Formula::mk_and_with(|conj| {
                                self.the_formula(args, 3, 0, [1, 2])
                                    .append_conjuncts_to(conj);
                                self.the_formula(args, 3, 0, [2, 1])
                                    .mk_neg()
                                    .append_conjuncts_to(conj);
                            });
                            Formula::forall(
                    ...
                    (
                        PropertyKind::Involutiveness,
                        &PropertyDeclKind::Func(_, Args::Unary(args)),
                    ) => {
                        let ty = &lc.fixed_var[args[0]].ty;
                    ...
                    (PropertyKind::Projectivity, &PropertyDeclKind::Func(_, Args::Unary(args))) => {
                        ...
                        assert!(ty
                            .attrs
                            .0
                            .is_subset_of(&wty.attrs.1, |a1, a2| g.eq(lc, a1, a2)));
                        let f = Formula::mk_and_with(|conj| {
                            self.the_formula(args, 2, 0, [1]).append_conjuncts_to(conj);
                            self.the_formula(args, 2, 0, [0])
                                .mk_neg()
                                .append_conjuncts_to(conj);
                        });
                    ...
                    (PropertyKind::Sethood, PropertyDeclKind::Mode(value)) => {
                        ...
                        let f = value.by_cases(0, |case| {
                            let f = Formula::mk_and_with(|conj| {
                                case.visit_cloned(&mut AbstractIt(1, 2))
                                    .append_conjuncts_to(conj);
                                let f = Formula::Pred {
                                    nr: g.reqs.belongs_to().unwrap(),
                                    args: Box::new([
                                        Term::Bound(BoundId(1)),
                                        Term::Bound(BoundId(0)),
                                    ]),
                                };
                                conj.push(f.mk_neg())
                            });
                            Formula::forall(
                                Type::SET,
                                Formula::forall(it_ty.clone(), f.mk_neg()).mk_neg(),
                            )
                            .mk_neg()
                        });
                        f.mk_neg()
...
impl ReserveBlock {
    fn intro(self, elab: &mut Analyzer<'_>) {
        if !self.0.is_empty() {
            ...
            let mut inst = InstConst {
                depth: 0,
                base: elab.lc.fixed_var.len() as u32,
            };
            let lookup = Rc::make_mut(&mut elab.lookup);
            for &nr in &self.0 {
                let Formula::ForAll { mut dom, scope } = *thesis else {
                    unreachable!()
                };
                if inst.depth != 0 {
                    inst.visit_type(&mut dom);
                }
                let c = elab.r.lc.fixed_var.push(FixedVar {
                    ty: *dom,
                    def: None,
                });
                ...
                lookup
                    .var
                    .insert(elab.reserved[nr].0.clone(), VarKind::Const(c));
                thesis = scope;
                inst.depth += 1;
            }
...
impl Analyzer<'_> {
    ...
    fn push_bound_reserved(
        ...
        fvars: &[ReservedId],
        inner: bool,
        mut check: impl FnMut(&mut Self, &Type),
    ) {
        if !fvars.is_empty() {
            for &v in fvars {
            ...
            if inner {
                self.reserved_more_insertion
                    .push((self.lc.bound_var.len() as u32 - len, len))
            } else {
...
impl CollectReserved<'_> {
    fn use_reserved(&mut self, pos: Position, n: ReservedId) {
        for &m in &self.res_groups[self.reserved[n].1]
            .fvars
            .as_ref()
            .unwrap()
            .0
    ...
    fn merge_bound_uses(
        uses: &mut im::OrdMap<ReservedId, bool>,
        bound: &im::OrdMap<ReservedId, bool>,
    ) {
        for (&k, _) in bound {
            match uses.entry(k) {
        ...
        for ast::BinderGroup { vars, ty } in vars {
            if let Some(ty) = ty {
                ...
                let subst = (self.res_groups[group].fvars.as_ref().unwrap().0.iter())
                    ...
                    .collect_vec();
                *ty = Some(Box::new(ast::Type::Reservation {
                    pos: vars[0].pos,
                    group,
                    subst,
                }));
                ...
                Rc::make_mut(&mut self.lookup)
                    .var
                    .insert(vars[0].spelling.clone(), VarKind::Bound(v));
    ...
    fn visit_term(&mut self, tm: &mut ast::Term) {
        match tm {
            ast::Term::Var {
                ...
                if kind.is_none() {
                    let Some(kind2) = (self.lookup.var.get(&**spelling).copied()).or_else(|| {
                        Some(VarKind::Reserved(*self.reserved_by_name.get(&**spelling)?))
                    }) else {
            ...
            ast::Term::SubAggr { arg, .. } | ast::Term::Selector { arg, .. } => {
                self.visit_term(arg)
            }
            ...
            ast::Term::Fraenkel {
                ...
                if !scope_uses.is_empty() {
                    *nameck = Some(Box::new(FraenkelNameckResult {
                        scope: NameckScope {
                            uses: scope_uses,
                            reserved: scope_reserved,
                        },
                        compr: NameckScope {
                            uses: compr_uses,
                            reserved: compr_reserved,
                        },
                    }))
                }
            }),
    ...
    fn visit_formula(&mut self, f: &mut ast::Formula) {
        match f {
            ...
            ast::Formula::ChainPred { first, rest, .. } => {
                ...
                rest.iter_mut()
                    .for_each(|rhs| self.visit_terms(&mut rhs.right));
            }
        ...
        match &mut it.kind {
            ast::ItemKind::Let { vars, conds } => {
                ...
                if !conds.is_empty() {
                    let mut conjs = vec![];
                    for prop in conds {
                        elab.elab_proposition_forall_reserved(prop, true)
            ...
            ast::ItemKind::Take(its) => {
                for ast::TakeDecl { var, term } in its {
                    ...
                    if let Some(var) = var {
                        ...
                        let v = elab.lc.fixed_var.push(FixedVar {
                            ty,
                            def: Some((Box::new(term), false)),
                        });
                        if elab.g.cfg.nameck_enabled {
                            Rc::make_mut(&mut elab.lookup)
                                .var
                                .insert(var.spelling.clone(), VarKind::Const(v));
                        }
...
impl CorrConds {
    const fn new() -> Self {
        Self(EnumMap::from_array([None, None, None, None, None, None]))
    }
}
...
pub trait BodyKind {
    fn it_eq(&self, g: &MizGlobal) -> Formula;
    fn mk_eq(&self, g: &MizGlobal, other: &Self) -> Formula;
}
impl BodyKind for Term {
    fn it_eq(&self, g: &MizGlobal) -> Formula {
        g.reqs.mk_eq(Term::It, self.clone())
    }
    fn mk_eq(&self, g: &MizGlobal, other: &Self) -> Formula {
        g.reqs.mk_eq(self.clone(), other.clone())
    }
}
impl BodyKind for Formula {
    fn it_eq(&self, _: &MizGlobal) -> Formula {
        self.clone()
    }
    fn mk_eq(&self, _: &MizGlobal, other: &Self) -> Formula {
        self.clone().mk_iff(other.clone())
    }
}

impl<T: BodyKind> DefBody<T> {
    fn mk_consistency(&self, g: &MizGlobal, it_type: Option<&Type>) -> Option<Box<Formula>> {
        ...
        let f = Formula::mk_and_with(|conjs| {
            for (i, j) in self.cases.iter().tuple_combinations() {
                let f = Formula::mk_and_with(|disj| {
                    ...
                    i.case
                        .it_eq(g)
                        .mk_iff(j.case.it_eq(g))
                        .mk_neg()
                        .append_conjuncts_to(disj);
                });
    ...
    fn iffthm_for(&self, g: &MizGlobal, defines: &Formula) -> Box<Formula> {
        self.by_cases(0, |case| defines.clone().mk_iff(case.it_eq(g)).mk_neg())
    }

    fn defthm_for(&self, g: &MizGlobal, defines: &T) -> Box<Formula> {
...
impl DefBody<Formula> {
    ...
    fn mk_uniqueness(&self, g: &MizGlobal, it_type: &Type) -> Box<Formula> {
        let scope = self.by_cases(2, |case| {
            Formula::mk_and_with(|conjs| {
                case.visit_cloned(&mut AbstractIt(0, 2))
                    .append_conjuncts_to(conjs);
                case.visit_cloned(&mut AbstractIt(1, 2))
                    .append_conjuncts_to(conjs);
                conjs.push(
                    g.reqs
                        .mk_eq(Term::Bound(BoundId(0)), Term::Bound(BoundId(1)))
                        .mk_neg(),
                )
            })
        });
...
fn mk_mode_coherence(
    ...
    Box::new(Formula::forall(
        Type {
            ...
            attrs: (
                Attrs::EMPTY,
                attrs.visit_cloned(&mut Inst::new(ctx, lc, &args, 0)),
            ),
            args,
        },
        Formula::Is {
...
impl DefValue {
    ...
    fn as_formula(&self, g: &MizGlobal) -> Box<Formula> {
        match self {
...
impl ReadProof for WithThesis {
    ...
    fn assume(&mut self, elab: &mut Analyzer, conjs: Vec<Formula>) {
        ...
        let args = elab
            .and_telescope(conjs, true, thesis)
    ...
    fn unfold(&mut self, elab: &mut Analyzer, refs: &[ast::Reference]) {
        ...
        for r in elab.elab_references(refs) {
            ...
            let Some((pos, f2)) = elab.try_unfold(false, def, true, kind, args) else {
                fail()
            };
            f.0 = pos;
            *f.1 = f2;
    ...
    fn reconstruct(&mut self, elab: &mut Analyzer, pos: bool) -> Formula {
        ...
        impl Reconstruction {
            fn as_pos(&mut self, pos: bool) -> &mut Vec<Formula> {
                if self.pos != pos {
                    self.pos = pos;
                    self.conjs = Formula::mk_and(std::mem::take(&mut self.conjs))
                        .mk_neg()
                        .into_conjuncts();
                }
                &mut self.conjs
        ...
        let conjs = elab
            .thesis
            .take()
            .map_or_else(Vec::new, |f| f.into_conjuncts());
...
impl ReadProof for ReconstructThesis {
    ...
    fn intro(&mut self, elab: &mut Analyzer, start: usize, istart: u32) {
        match self.stack.last_mut() {
            ...
            _ => self.stack.push(ProofStep::Let {
    ...
    fn take(&mut self, _: &mut Analyzer, _: Term) {
        panic!("take steps are not reconstructible")
    }
    ...
    fn new_cases(&mut self, _: &mut Analyzer) {
        self.stack.push(ProofStep::Break(false))
    }
    fn new_case(&mut self, _: &mut Analyzer, _: &mut (), conjs: &[Formula]) {
        self.stack.push(ProofStep::Break(true));
        self.stack.push(ProofStep::Thus {
            conjs: conjs.to_vec(),
        })
    }
    ...
    fn new_supposes(&mut self, _: &mut Analyzer) -> Self::SupposeRecv {
        None
    }
...
impl ToLocus<'_> {
    fn get(&self, c: ConstId) -> LocusId {
        self.to_locus
            .get(c)
            .and_then(|l| *l)
            .expect("local constant in exported item")
    }
}
            ...
            if let Some(i) = c.0.checked_sub(self.fixed_vars) {
                *tm = match self.terms[i as usize] {
                    ...
                    Err(nr) => Term::Selector {
                        nr,
                        args: (0..=self.base)
                            .map(|i| Term::Const(self.to_const[LocusId(i)]))
                            .collect(),
                    },
                }
            }
...
impl CheckAccess {
    ...
    const fn get(&self, n: LocusId) -> bool {
        self.0 & (1 << n.0 as u64) != 0
    }
    fn set(&mut self, n: LocusId) {
        self.0 |= 1 << n.0 as u64
    }
...
impl<F> Pattern<F> {
    fn check_access(&self) {
        CheckAccess::with(&self.primary, |occ| {
            self.visible.iter().for_each(|&v| occ.set(v))
        })
    }
}
...
impl BlockReader {
    ...
    fn has_assums(&self) -> bool {
        self.assums
            .iter()
            .any(|f| matches!(f, ReconstructAssum::Assum(_)))
    }
                ...
                match assum {
                    ReconstructAssum::Let { start } => {
                        while al.0 as u8 > start.0 {
                            al.0 -= 1;
                            let mut ty = elab.lc.fixed_var[self.to_const[LocusId(al.0 as u8)]]
                                .ty
                                .visit_cloned(l);
                            ty.visit(&mut al);
    ...
    fn check_compatible_args(&self, lc: &LocalContext, subst: &Subst<'_>) {
        let n = self
            .primary
            .len()
            .checked_sub(subst.subst_term.len())
    ...
    fn elab_func_def(
        ...
        elab: &mut Analyzer,
        loc: Position,
        pat: &mut ast::PatternFunc,
        it: &mut ast::DefinitionBody,
        spec: Option<&ast::Type>,
        ...
        let visible: Box<[_]> = args
            .iter_mut()
            .map(|v| self.locus(elab.resolve_const(v)))
            .collect();
        ...
        if it.redef {
            ...
            let args2: Box<[_]> = self.to_const.0[superfluous as usize..]
                .iter()
                .map(|c| Term::Const(*c))
                .collect();
        ...
        let value = def
            .as_mut()
            .map(|def| elab.elab_def_value(&mut def.kind, true));
        ...
        if let Some(value) = &value {
            ...
            if let Some(nr) = redefines {
                let args = self.to_const.0[superfluous as usize..]
                    .iter()
                    .map(|c| Term::Const(*c))
                    .collect();
        ...
        if let Some(mut value) = value {
            ...
            let formals: Box<[_]> = self
                .primary
                .enum_iter()
                .map(|(i, _)| Term::Locus(i))
                .collect();
    ...
    fn elab_pred_def(
        ...
        if it.redef {
            ...
            (redefines, superfluous, pos) = (
                Some(nr),
                (self.primary.len() - c.primary.len()) as u8,
                pat.pos,
            );
            properties.props = c.properties.offset(superfluous)
        } else {
        ...
        let value = def
            .as_deref_mut()
            .map(|def| elab.elab_def_value(&mut def.kind, pos));
        if let Some(value) = &value {
        ...
        if !self.has_assums() {
            properties.load_args(&elab.g, &elab.lc, &self.to_const, |args| {
                PropertyDeclKind::Pred(args, pos)
            });
        }
        properties.elab_properties(elab, &mut it.props);
        let n;
        ...
        if let Some(value) = value {
            let DefValue::Formula(mut value) = value else {
    ...
    fn elab_mode_def(
        ...
        elab: &mut Analyzer,
        loc: Position,
        pat: &mut ast::PatternMode,
        kind: &mut ast::DefModeKind,
        it: &mut ast::DefinitionBody,
    ) {
        ...
        match kind {
            ...
            ast::DefModeKind::Standard { spec, def } => {
              ...
              Some(subst) if { self.check_compatible_args(&elab.lc, &subst); true })
                    ...
                    let pat = pat.expect("type error");
                    let PatternKind::Mode(nr) = pat.kind else {
                        panic!("redefining expandable mode")
                    };
                    let c = &elab.g.constrs.mode[nr];
                    (redefines, superfluous) =
                        (Some(nr), (self.primary.len() - c.primary.len()) as u8);
                    if c.properties.get(PropertyKind::Sethood) {
                        properties.props.set(PropertyKind::Sethood)
                    }
                    let args2: Vec<_> = self.to_const.0[superfluous as usize..]
                        .iter()
                        .map(|c| Term::Const(*c))
                        .collect();
                    let tgt =
                        c.ty.visit_cloned(&mut Inst::new(&elab.g.constrs, &elab.lc, &args2, 0));
                    it_type = elab.elab_spec(spec.as_deref(), &tgt);
                    if spec.is_some() {
                        cc.0[CorrCondKind::Coherence] = Some(mk_mode_coherence(
                            &elab.g.constrs,
                            &elab.lc,
                            nr,
                            &tgt.attrs.1,
                ...
                if let Some(value) = &value {
                    ...
                    if let Some(nr) = redefines {
                        ...
                        let defined = Formula::Is {
                            term: Box::new(Term::It),
                            ty,
                        };
                        cc.0[CorrCondKind::Compatibility] =
                            Some(value.mk_compatibility(&elab.g, Some(&it_type), &defined));
                ...
                if !it.redef {
                    let Some(DefValue::Formula(value)) = &value else {
                        panic!("expected formula")
                    };
                    cc.0[CorrCondKind::Existence] = Some(value.mk_existence(&it_type));
                ...
                if let TypeKind::Mode(nr) = it_type.kind {
                    if elab.g.constrs.mode[nr]
                        .properties
                        .get(PropertyKind::Sethood)
                    {
                        properties.props.set(PropertyKind::Sethood)
                    }
                }
                ...
                if let Some(value) = value {
                    ...
                    it_type2.adjust_mut(&elab.g.constrs);
                    let primary: Box<[_]> =
                        self.primary.0.iter().cloned().chain([it_type2]).collect();
                    it_type.visit(&mut Inst::new(&elab.g.constrs, &elab.lc, &formals, 0));
                    let ty = Box::new(Type {
                    ...
                    let depth = self.primary.len() as u32;
                    let defined = Formula::Is {
                        term: Box::new(Term::Locus(LocusId(depth as _))),
                        ty,
                    };
    ...
    fn elab_attr_def(
        ...
        if let Some(value) = value {
            ...
            self.to_locus(elab, |l| value.visit(l));
            let formals = (superfluous..self.primary.len() as u8)
                .map(LocusId)
                .map(Term::Locus)
                .collect();
            let mut f = value.defthm_for(
    ...
    fn elab_struct_def(&mut self, elab: &mut Analyzer, it: &mut ast::DefStruct) {
        ...
        for group in &it.fields {
            ...
            for field in &group.vars {
                let c = elab.lc.fixed_var.push(FixedVar {
                    ty: ty.clone(),
                    def: None,
                });
        ...
        self.to_locus
            .push(Some(self.to_const.push(ConstId::from_usize(fixed_vars))));
        let mut mk_sel = MakeSelector {
            base,
            fixed_vars: fixed_vars as u32,
            to_const: &self.to_const,
            terms: vec![],
        };
        let mut fields = vec![];
            ...
            .zip(field_tys)
        {
            ...
            let mut iter = parents
                .iter()
                .zip(&mut prefixes)
                .filter_map(|(parent, fields)| {
                    let arg = Term::Const(elab.lc.fixed_var.push(FixedVar {
                        ty: parent.clone(),
                        def: None,
                    }));
                    for pat in elab.notations[PKC::Sel].iter().rev() {
                        if pat.fmt == fmt {
                            if let Some(subst) =
                                pat.check_types(&elab.g, &elab.lc, std::slice::from_ref(&arg))
                            {
                                let PatternKind::Sel(nr) = pat.kind else {
                                    unreachable!()
                                };
            ...
            if let Some((sel_id, args)) = iter.next() {
                assert!(
                    iter.all(|(nr, _)| sel_id == nr),
                    "overlapping parent fields"
                );
                mk_sel
                    .terms
                    .push(Ok(Box::new(Term::Selector { nr: sel_id, args })));
                fields.push(sel_id);
            } else {
                self.to_locus(elab, |l| ty.visit(l));
                let mut sel = TyConstructor {
        ...
        assert!(
            ...
            "structure does not extend parent"
        );
        ...
        new_fields
            .into_iter()
            .for_each(|sel_id| elab.push_constr(ConstrKind::Sel(sel_id)));

        let attrs = Attrs::Consistent(vec![MizAttr {
            nr: attr_id,
            pos: true,
            args: formals.into(),
        }]);
        ...
        let mut c = Aggregate {
            c: TyConstructor {
                c: Constructor::new(aggr_primary),
                ty: aggr_ty,
            },
            base,
            fields: fields.into(),
    ...
    fn elab_cond_reg(
        ...
        let Attrs::Consistent(_) = attrs1 else {
            panic!("inconsistent cluster antecedent")
        };
        let Attrs::Consistent(_) = attrs2 else {
            panic!("inconsistent cluster consequent")
        };
        elab.read_conditional_cluster(ConditionalCluster {
            cl: Cluster {
                primary,
                consequent: (attrs2.clone(), attrs2),
            },
            ty: Box::new(ty2),
            antecedent: attrs1,
        });
    ...
    fn elab_func_reg(
        ...
        let concl = concl
            .iter()
            .map(|attr| elab.elab_attr(attr, true, &mut ty))
            .collect_vec();
        let mut cc = CorrConds::new();
        ...
        let Attrs::Consistent(_) = attrs else {
            panic!("inconsistent functor registration")
        };
        self.to_locus(elab, |l| {
            term2.visit(l);
            attrs1.visit(l);
    ...
    fn elab_identify_func(&mut self, elab: &mut Analyzer, it: &mut ast::IdentifyFunc) {
        ...
        for (v1, v2) in &mut it.eqs {
            ...
            eq_args.push((c1, c2));
            occ.set(v1);
            assert!(elab.lc.fixed_var[c2].ty.is_wider_than(
                &elab.g,
                &elab.lc,
                &elab.lc.fixed_var[c1].ty
            ));
        }
        ...
        let mut cc = CorrConds::new();
        let (lhs_args, rhs_args) = self.to_locus(elab, |l| {
            (lhs_pat.args.visit_cloned(l), rhs_pat.args.visit_cloned(l))
        });
        let f = Formula::mk_and_with(|conjs| {
            conjs.extend(
                eq_args
                    .iter()
                    .map(|&(c1, c2)| elab.g.reqs.mk_eq(Term::Const(c1), Term::Const(c2))),
            );
        ...
        elab.r.push_identify(&IdentifyFunc {
            ...
            eq_args: eq_args
                .into_iter()
                .map(|(c1, c2)| (self.locus(c1), self.locus(c2)))
                .collect(),
        });
    }
    ...
    fn elab_reduction(&mut self, elab: &mut Analyzer, it: &mut ast::Reduction) {
        ...
        fn subterm_list(ctx: &mut EqCtx<'_>, args: &[Term], sub: &Term) -> bool {
            args.iter()
                .any(|t| ().eq_term(ctx, t, sub) || is_ssubterm(ctx, t, sub))
        }
        ...
        let mut cc = CorrConds::new();
        cc.0[CorrCondKind::Reducibility] =
            Some(Box::new(elab.g.reqs.mk_eq(from.clone(), to.clone())));
        elab.elab_corr_conds(cc, &mut it.conds, &mut it.corr);
        self.to_locus(elab, |l| {
            from.visit(l);
            to.visit(l)
        ...
        elab.r.reductions.push(Reduction {
            primary,
            terms: [from, to],
        });
    }
    ...
    fn elab_sethood_registration(
        ...
        elab.properties.push(Property {
            primary,
            ty,
            kind: PropertyKind::Sethood,
        })
    }
    ...
    fn elab_pred_notation(
        &mut self,
        elab: &mut Analyzer,
        new: &mut ast::PatternPred,
        orig: &mut ast::PatternPred,
        pos: bool,
    ) {
        ...
        CheckAccess::with(&primary, |occ| {
            orig.args
                .iter_mut()
                .for_each(|v| occ.set(self.locus(elab.resolve_const(v))))
        });
    ...
    fn elab_func_notation(
        &mut self,
        elab: &mut Analyzer,
        new: &mut ast::PatternFunc,
        orig: &mut ast::PatternFunc,
    ) {
    ...
    fn elab_mode_notation(
        &mut self,
        elab: &mut Analyzer,
        new: &mut ast::PatternMode,
        orig: &mut ast::PatternMode,
    ) {
    ...
    fn elab_attr_notation(
        &mut self,
        elab: &mut Analyzer,
        new: &mut ast::PatternAttr,
        orig: &mut ast::PatternAttr,
        pos: bool,
    ) {
...
impl ReadProof for BlockReader {
    ...
    fn intro(&mut self, elab: &mut Analyzer, start: usize, _: u32) {
        ...
        if !matches!(self.assums.last(), Some(ReconstructAssum::Let { .. })) {
            self.assums.push(ReconstructAssum::Let {
                start: self.primary.peek(),
            })
        }
    ...
    fn thus(&mut self, _: &mut Analyzer, _: Vec<Formula>) {
        panic!("invalid item")
    }
    fn unfold(&mut self, _: &mut Analyzer, _: &[ast::Reference]) {
        panic!("invalid item")
    }
    fn new_cases(&mut self, _: &mut Analyzer) -> Self::CaseIter {
        panic!("invalid item")
    }
    ...
    fn elab_item(&mut self, elab: &mut Analyzer, it: &mut ast::Item) -> bool {
        match &it.kind {
            ...
            ast::ItemKind::SethoodRegistration { .. } => {
                elab.item_header(it, "SethoodRegistration")
            }
            _ => {}
        }
        match (self.kind, &mut it.kind) {
            ...
            (BlockKind::Registration, ast::ItemKind::Cluster(it)) => match &mut it.kind {
                ...
                ast::ClusterDeclKind::Func { term, concl, ty } => self.elab_func_reg(
                    elab,
                    term,
                    concl,
                    ty.as_deref(),
            ...
            (BlockKind::Registration, ast::ItemKind::Reduction(it)) => {
                self.elab_reduction(elab, it)
            }
            ...
            (
                ...
                &mut ast::ItemKind::Pragma(Pragma::Canceled(CancelKind::Def, n)),
