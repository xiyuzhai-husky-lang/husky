...
impl<I: MizIdx> Monomial<I> {
    ...
    pub fn coeff_atom(coeff: Complex, var: I) -> Self {
        Self {
            coeff,
            powers: std::iter::once((var, 1)).collect(),
        }
    }
    pub fn atom(var: I) -> Self {
        Self::coeff_atom(Complex::ONE, var)
    }
    pub fn degree(&self) -> u32 {
        self.powers.iter().map(|p| p.1).sum()
    }
    ...
    fn mul(&self, other: &Self) -> Self {
        ...
        for item in self
            .powers
            .iter()
            .merge_join_by(&other.powers, |a, b| a.0.cmp(b.0))
        {
            match item {
                EitherOrBoth::Left((&et, &n)) | EitherOrBoth::Right((&et, &n)) => {
                    powers.insert(et, n)
                }
                EitherOrBoth::Both((&et, &n1), (_, &n2)) => {
                    powers.insert(et, n1.checked_add(n2).unwrap())
                }
            };
        }
    ...
    fn lex_powers(a: &BTreeMap<I, u32>, b: &BTreeMap<I, u32>) -> Ordering {
        a.iter()
            .map(|p| (p.0, !*p.1))
            .cmp(b.iter().map(|p| (p.0, !*p.1)))
    }

    fn lex(&self, other: &Self) -> Ordering {
        Self::lex_powers(&self.powers, &other.powers)
    }

    pub fn contains(&self, v: I) -> bool {
        self.powers.contains_key(&v)
    }
...
impl<I: MizIdx> Polynomial<I> {
    ...
    pub fn contains(&self, v: I) -> bool {
        self.0.iter().any(|mon| mon.contains(v))
    }
...
impl<I> LinPoly<I> {
    const fn cnst(cnst: Complex) -> Self {
        Self {
            cnst,
            terms: vec![],
        }
...
impl<I: MizIdx> Polynomial<I> {
    pub(super) fn reduce<J: MizIdx>(
        ...
        let mut it = eqs.iter();
        'next: loop {
            let [v1, ..] = &*eq.terms else {
                return eq.cnst == Complex::ZERO;
            };
