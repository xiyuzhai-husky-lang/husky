...
fn assert_eq_iter<T: Debug + PartialEq<U>, U: Debug>(
    header: &str,
    mut it1: impl Iterator<Item = T> + Clone,
    mut it2: impl Iterator<Item = U> + Clone,
) {
...
impl VisitMut for ExportPrep<'_> {
    ...
    fn visit_attrs(&mut self, attrs: &mut Attrs) {
        attrs.reinsert_all(self.ctx, self.lc, true, |attr| {
...
impl ApplyMarkConstr {
    fn apply(&mut self, n: &mut u32, key: impl Fn(&ConstructorsBase) -> u32) {
        if let Some(i) = self
            .0
            .partition_point(|(base, _)| key(base) <= *n)
            .checked_sub(1)
        {
            *n -= key(&self.0[i].1)
        }
...
impl Analyzer<'_> {
    pub fn export(&mut self) {
        ...
        // validating .dfr
        {
            ...
            if self.g.cfg.verify_export {
                assert_eq_nonempty!(
                    self,
                    nonempty,
                    self.path.read_dfr(false, &mut vocs2, &mut dfr2)
                );
            }
            if nonempty {
        ...
        {
            let mut def1 =
                self.definitions.0[self.export.definitions_base.into_usize()..].to_owned();
            ...
            if self.g.cfg.verify_export {
                assert_eq_nonempty!(
                    self,
                    nonempty,
                    self.path.read_def(false, &mut sig, &mut def2)
                );
            }
            if nonempty {
                def1.visit(ep);
        ...
        // validating .did
        {
            ...
            if self.g.cfg.verify_export {
                assert_eq_nonempty!(
                    self,
                    nonempty,
                    self.path.read_did(false, &mut sig, &mut did2)
                );
            }
            if nonempty {
                did1.visit(ep);
        ...
        // validating .drd
        {
            ...
            if self.g.cfg.verify_export {
                assert_eq_nonempty!(
                    self,
                    nonempty,
                    self.path.read_drd(false, &mut sig, &mut drd2)
                );
            }
            if nonempty {
                drd1.visit(ep);
        ...
        // validating .dpr
        {
            ...
            if self.g.cfg.verify_export {
                assert_eq_nonempty!(
                    self,
                    nonempty,
                    self.path.read_dpr(false, &mut sig, &mut dpr2)
                );
                dpr2.visit(ep);
            }
            if nonempty {
