...
use crate::types::*;
use crate::{
    vprintln, CheckBound, CmpStyle, EqCtx, Equate, ExpandPrivFunc, Inst, LocalContext, MizGlobal,
...
impl<I: MizIdx> ConstrMap<I> {
    ...
    fn find(&self, g: &MizGlobal, lc: &LocalContext, nr: I, args: &[Term]) -> Option<EqMarkId> {
        let entry = self.0.get(&nr)?;
        entry
            .iter()
            .copied()
            .find(|&m| g.eq(lc, args, lc.marks[m].0.args().unwrap()))
    }
}

impl Attrs {
    fn try_attrs(&self) -> OrUnsat<&[MizAttr]> {
        match self {
        ...
        loop {
                ...
                .any(|old| old.kind == new.kind && self.g.eq(self.lc, &*old.args, &new.args))
            {
                if !new
                    .attrs
                    .1
                    .is_subset_of(&eq_term.supercluster, |a1, a2| self.g.eq(self.lc, a1, a2))
                {
                    for attr in new.attrs.1.try_attrs().unwrap() {
                        added |= eq_term.supercluster.try_insert(
                            &self.g.constrs,
                            self.lc,
                            attr.clone(),
                        )?;
            ...
            let Attrs::Consistent(attrs) = std::mem::take(&mut new.attrs).1 else {
                unreachable!()
            };
            ...
            for attr in attrs {
                eq_term
                    .supercluster
                    .try_insert(&self.g.constrs, self.lc, attr)?;
            }
            ...
            loop {
                ...
                for mut ty in parents.into_vec() {
                    ...
                    if !(eq_term.ty_class.iter()).any(|old| {
                        old.kind == ty.kind && EqMarks.eq(self.g, self.lc, &*old.args, &ty.args)
                    }) {
                        eq_term.ty_class.push(ty)
                    }
                }
                i += 1;
                let Some(new) = eq_term.ty_class.get(i) else {
                    return Ok(true);
                };
                let TypeKind::Struct(m2) = new.kind else {
                    unreachable!()
                };
                m = m2;
            }
...
impl<'a> Equalizer<'a> {
    ...
    fn prep_binder(
        ...
        tm: &mut Term,
        depth: u32,
        coll: fn(&mut ConstrMaps) -> &mut Vec<EqMarkId>,
        ...
        match vec.binary_search_by(|&m| {
            self.lc.marks[m]
                .0
                .cmp(Some(&self.g.constrs), Some(self.lc), tm, CmpStyle::Red)
        }) {
        ...
        let et = match *tm {
                ...
                if let Some(&Some(em)) = self.infers.get(nr) {
                    ...
                    *self.eq.infers.get_mut_extending(nr) = Some(self.eq.terms[et].mark);
                    let ty = {
                        ...
                        ic[nr]
                            .ty
                            .visit_cloned(&mut ExpandPrivFunc(&self.eq.g.constrs, self.eq.lc))
                    };
            ...
            Term::Functor { nr, ref mut args } => {
                ...
                let ty = if CheckE::with(&self.lc.marks, |ce| ce.visit_terms(&orig)) {
                    ...
                    *Term::Functor { nr, args: orig }
                        .round_up_type(self.g, self.lc, true)
                        .to_owned()
                };
                ...
                *tm = Term::Functor {
                ...
                if constr.properties.get(PropertyKind::Commutativity) {
                    args1.swap(
                        constr.properties.arg1 as usize,
                        constr.properties.arg2 as usize,
                    );
                    let (nr3, comm_args) = Term::adjust(nr, &args1, Some(&self.g.constrs));
                    let m = self.lc.marks.push((
                        Term::Functor {
                            nr: nr3,
                            args: comm_args.to_vec().into(),
                        },
                        et,
                    ));
                    self.terms[et].eq_class.push(m);
            ...
            Term::Fraenkel {
                ...
                let Some(et) = self.add_binder_into(tm, |c| &mut c.fraenkel) else {
                    return;
                };
                et
            }
            Term::The { ref mut ty } => {
                self.visit_type(ty);
                let Some(et) = self.add_binder_into(tm, |c| &mut c.choice) else {
                    return;
                };
                et
            }
        ...
        match term {
            ...
            Term::Functor { nr, ref mut args } => {
                ...
                match self.g.reqs.rev.get(nr1) {
                    Some(Some(Requirement::ZeroNumber)) => {
                        self.terms[et].number = Some(Complex::ZERO)
                    }
                    Some(Some(Requirement::ImaginaryUnit)) => {
                        self.terms[et].number = Some(Complex::I)
                    }
                    _ => {}
                }
                let m = self.lc.marks.push((
...
impl MizAttr {
    fn is_strict(&self, ctx: &Constructors) -> bool {
        self.pos
            && ctx.attribute[self.nr]
                .properties
                .get(PropertyKind::Abstractness)
    }
}
...
impl Instantiate<'_> {
            ...
            (Term::EqClass(_), _) => {
                ...
                match src {
                    &Term::Locus(v) => {
                        ...
                        z.mk_and_then(|| Ok(Dnf::single(Conjunct::single(v, self.terms[et].id))))
                            .unwrap();
                        z
                    }
                    ...
                    Term::Functor {
                        ...
                        for &m in &self.terms[et].eq_class {
                            ...
                            } = &self.lc.marks[m].0
                            {
                                let (n2, args2) = Term::adjust(*n2, args2, Some(&self.g.constrs));
                                if n1 == n2 {
                                    res.mk_or_else(|| Ok(self.inst_terms(args1, args2)))
                                        .unwrap()
                                }
                            }
                        }
    ...
    fn and_inst_attrs(&self, attrs: &Attrs, et: EqTermId, res: &mut Dnf<LocusId, EqClassId>) {
        ...
        let Attrs::Consistent(sc) = &self.terms[et].supercluster else {
            unreachable!()
        };
...
fn is_empty_set(g: &MizGlobal, lc: &LocalContext, terms: &[EqMarkId]) -> bool {
    let empty = g.reqs.empty_set().unwrap();
    terms
        .iter()
        .any(|&m| matches!(lc.marks[m].0, Term::Functor { nr, .. } if nr == empty))
}
...
impl<'a> Equalizer<'a> {
    ...
    fn filter_allowed(&self, attrs: &Attrs) -> Attrs {
        match attrs {
            ...
            Attrs::Consistent(attrs) => {
                let attrs = attrs
                    .iter()
                    .filter(|a| !HasInfer::get(&self.infers, |ci| ci.visit_terms(&a.args)));
    ...
    fn drain_pending(
        &mut self,
        to_y_term: &mut Vec<(EqTermId, Term)>,
        eq_pendings: &mut Equals,
    ) -> OrUnsat<()> {
    ...
    fn union_terms(&mut self, x: EqTermId, y: EqTermId) -> OrUnsat<()> {
        let (x, y) = (
            self.lc.marks[self.terms[x].mark].1,
            self.lc.marks[self.terms[y].mark].1,
        );
    ...
    fn locate_terms(
        &self,
        inst: &Conjunct<LocusId, EqClassId>,
        args1: &[Term],
        args2: &[Term],
    ) -> Option<()> {
        assert!(args1.len() == args2.len());
        ...
        for (t1, t2) in args1.iter().zip(args2) {
            ...
            matches!(*t2, Term::EqMark(m2) if self.lc.marks[m1].1 == self.lc.marks[m2].1)
                .then_some(())?;
        }
        Some(())
    }
        ...
        match *tm {
            Term::Locus(n) => {
                ...
                Some(
                    self.terms
                        .0
                        .iter()
                        .find(|&et| et.id == id && !et.eq_class.is_empty())?
                        .mark,
                )
            }
            Term::Infer(n) => (self.terms.0.iter())
                .find(|et| {
                    et.eq_class
                        .iter()
                        .any(|&m| matches!(self.lc.marks[m].0, Term::Infer(n2) if n == n2))
                })
                .map(|et| et.mark),
            Term::Numeral(nr) => {
                let c = nr.into();
                self.terms
                    .0
                    .iter()
                    .find(|et| et.number.as_ref() == Some(&c))
                    .map(|et| et.mark)
            }
            ...
            Attrs::Consistent(attrs) => {
                ...
                for attr in attrs {
                    if let Some(args) = attr
                        .args
                        .iter()
                        .map(|tm| self.locate_term(inst, tm).map(Term::EqMark))
                        .collect()
                    {
                        res.push(MizAttr {
                            nr: attr.nr,
                ...
                res.sort_by(|a1, a2| {
                    a1.cmp_abs(Some(&self.g.constrs), Some(self.lc), a2, CmpStyle::Attr)
                });
    ...
    fn set_number(&mut self, et: EqTermId, val: Complex) -> OrUnsat<()> {
        if let Some(n) = &self.terms[et].number {
            ...
            for et2 in (0..self.terms.len()).map(EqTermId::from_usize) {
                if !self.terms[et2].eq_class.is_empty()
                    && self.terms[et2].number.as_ref() == Some(&val)
                {
                    vprintln!("[{et:?}] = [{et2:?}] = {val}");
        ...
        loop {
            for marks in self.constrs.aggregate.0.values() {
                ...
                while let Some(m1) = iter.next() {
                    ...
                    if let Some(m2) = iter
                        .clone()
                        .find(|&m| self.lc.marks[self.terms[self.lc.marks[m].1].mark].1 == et1)
                    {
                        let Term::Aggregate { nr, args: args1 } = &self.lc.marks[m1].0 else {
                            unreachable!()
                        };
                        let Term::Aggregate { args: args2, .. } = &self.lc.marks[m2].0 else {
                            unreachable!()
                        };
                        let base = self.g.constrs.aggregate[*nr].base as usize;
                        assert!(args1.len() == args2.len());
            ...
            for (&i, marks) in &self.constrs.functor.0 {
                ...
                if props.get(PropertyKind::Idempotence) {
                    for &m in marks {
                        let (Term::Functor { ref args, .. }, et) = self.lc.marks[m] else {
                            unreachable!()
                        };
                        let et1 = self.lc.marks[args[props.arg1 as usize].mark().unwrap()].1;
                        let et2 = self.lc.marks[args[props.arg2 as usize].mark().unwrap()].1;
                        if self.lc.marks[self.terms[et1].mark].1
                            == self.lc.marks[self.terms[et2].mark].1
                        {
                            ...
                            } = self.lc.marks[m2].0
                            {
                                if nr == i
                                    && EqMarks.eq(
                                        ...
                                        self.lc,
                                        args1,
                                        &args2[..props.arg1 as usize],
                                    )
                                {
                                    let et2 =
                                        self.lc.marks[args2[props.arg1 as usize].mark().unwrap()].1;
                    ...
                    (|$x:ident, $y:ident| $e:expr) => {
                        for &m in marks {
                            ...
                            if let (Some($x), Some($y)) =
                                (&self.terms[et1].number, &self.terms[et2].number)
                            {
                                to_number.push((self.lc.marks[self.terms[et].mark].1, $e))
                            }
                        }
                    ...
                    for (m1, m2) in vec.iter().copied().tuple_combinations() {
                        ...
                        if et1 != et2 {
                            match (tm1, tm2) {
                                ...
                                (
                                    Term::SchFunc { args: args1, .. },
                                    Term::SchFunc { args: args2, .. },
                                )
                                | (
                                    Term::PrivFunc { args: args1, .. },
                                    Term::PrivFunc { args: args2, .. },
                                ) => {
                                    if EqMarks.eq(self.g, self.lc, args1, args2) {
                                        to_union.push((et1, et2))
                                    }
                                ...
                                (
                                    Term::Aggregate { args: args1, .. },
                                    Term::Aggregate { nr, args: args2 },
                                ) => {
                                    let base = self.g.constrs.aggregate[*nr].base as usize;
                                ...
                                (
                                    Term::Selector { args: args1, .. },
                                    Term::Selector { args: args2, .. },
                                ) => {
                                    if EqMarks.eq(
                                        self.g,
                                        self.lc,
        ...
        loop {
                ...
                if !etm.eq_class.is_empty() {
                    if let Some(p) = etm.eq_polys.first() {
                        if let Some(v2) = p.is_var() {
                            if v2 != v1 {
                                if let Some(q) = self.terms[self.lc.marks[self.terms[v2].mark].1]
                                    .eq_polys
                                    .first()
                                {
                                        ...
                                        if v3 == v2 {
                                            if v1 > v2 {
                                                ...
                                                to_subst.push((
                                                    v2,
                                                    v1,
                                                    Polynomial::single(Monomial::atom(v1)),
                                                ));
                                            }
                                        }
                                    }
            ...
            'next: for (x, y) in std::mem::take(&mut eqs.0) {
                ...
                if let (Some(q1), Some(q2)) = (
                    self.terms[et1].eq_polys.first(),
                    self.terms[et2].eq_polys.first(),
                ) {
                    ...
                    } else if self.terms[et1].number.is_some()
                        && self.terms[et1].eq_polys.len() == 1
                    {
                        if u2 == Some(y) {
        ...
        loop {
            ...
            while let Some(et) = pending.pop_first() {
                ...
                if etm.eq_polys.len() == 1 {
                    progress |= self.subst_var(
                        et,
                        &etm.eq_polys.first().unwrap().clone(),
                        Some(&mut pending),
                    )?;
                }
            }
    ...
    fn insert_non_attr0(&mut self, et1: EqTermId, et2: EqTermId, nr: AttrId) -> OrUnsat<()> {
        if self.terms[et1]
            .supercluster
            .find0(&self.g.constrs, nr, true)
        {
            self.terms[et2].supercluster.try_insert(
                &self.g.constrs,
                self.lc,
                MizAttr::new0(nr, false),
            )?;
        }
        Ok(())
    }
    ...
    fn round_up_one_supercluster(
        ...
        match inst {
            Dnf::True => {
                ...
                self.terms[et]
                    .supercluster
                    .try_enlarge_by(&self.g.constrs, self.lc, &attrs)
            }
            Dnf::Or(conjs) => {
                let mut added = false;
                for conj in conjs {
                    let attrs = self.locate_attrs(conj, attrs);
                    added |= self.terms[et].supercluster.try_enlarge_by(
                        &self.g.constrs,
                        self.lc,
                        &attrs,
                    )?;
    ...
    pub fn run(
            ...
            for &m in &ets.eq_class {
                if let Term::Infer(id) = self.lc.marks[m].0 {
                    ...
                    to_yy_term.push((
                        i,
                        asgn.def
                            .visit_cloned(&mut ExpandPrivFunc(&self.g.constrs, self.lc)),
                    ))
                }
            }
            ...
            if !ets.eq_class.is_empty() {
                ...
                for attr in ets.supercluster.try_attrs().unwrap() {
                    if attr.is_strict(&self.g.constrs) {
                        let TypeKind::Struct(s) = self.g.constrs.attribute[attr.nr].ty.kind else {
                            panic!()
                        };
                ...
                if let Some(s) = strict_struct {
                    for ty in &ets.ty_class {
                        if ty.kind == TypeKind::Struct(s) {
                            to_y_term.push((
                                ei,
                                Term::mk_aggr(self.g, self.lc, s, &Term::EqMark(ets.mark), ty),
                            ))
                        }
                    }
                }
        ...
        if let Some(complex) = self.g.reqs.complex() {
            ...
            for et in to_complex {
                let attr = MizAttr::new0(complex, true);
                self.terms[et]
                    .supercluster
                    .try_insert(&self.g.constrs, self.lc, attr)?;
            }
        ...
        if let Some(complex) = self.g.reqs.complex() {
            ...
            eqs.0.retain(|&(x, y)| {
                ...
                if self.terms[et1]
                    .supercluster
                    .find0(&self.g.constrs, complex, true)
                    || self.terms[et2]
                        .supercluster
                        .find0(&self.g.constrs, complex, true)
                {
                        ...
                        match self.lc.marks[m].0 {
                            ...
                            Term::Infer(_) => {
                                if let Some(n) = &etm.number {
                                    etm.eq_polys
                                        .insert(Polynomial::single(Monomial::cnst(n.clone())));
                                    pending.insert(et);
                    ...
                    pending.insert(et);
                                ...
                                match self.g.reqs.rev.get(nr).copied().flatten() {
                                    Some(Requirement::ImaginaryUnit) => {
                                        ...
                                        etm.eq_polys
                                            .insert(Polynomial::single(Monomial::cnst(Complex::I)));
                                        pending.insert(et);
                                    }
                                    ...
                                    Some(Requirement::RealDiv) => {
                                        let [Term::EqMark(m1), Term::EqMark(m2)] = **args else {
                                            unreachable!()
                                        };
                                        let et1 =
                                            self.lc.marks[self.terms[self.lc.marks[m1].1].mark].1;
                                        let et2 =
                                            self.lc.marks[self.terms[self.lc.marks[m2].1].mark].1;
                                        if let Some(p) = self.terms[et2]
                                            .number
                                            .clone()
                                            .filter(|p| *p != Complex::ZERO)
                                        {
                                            let q =
                                                Polynomial::single(Monomial::atom(et1)) * &p.inv();
                                            self.terms[et].eq_polys.insert(q);
                                            pending.insert(et);
                                        } else {
                                            self.terms[et]
                                                .eq_polys
                                                .insert(Polynomial::single(Monomial::atom(et)));
                                        }
                                    }
                                    _ => {
                                        if let Some(n) = &etm.number {
                                            etm.eq_polys.insert(Polynomial::single(
                                                Monomial::cnst(n.clone()),
                                            ));
                                        } else if etm.supercluster.find0(
                                            &self.g.constrs,
                                            complex,
                                            true,
                                        ) && etm.eq_polys.is_empty()
        ...
        for etm in &self.terms.0 {
            ...
            let Term::EqClass(ec) = self.lc.marks[self.terms[et].mark].0 else {
                unreachable!()
            };
        ...
        for i in 0..self.terms.0.len() {
            ...
            if !etm.eq_class.is_empty() {
                let Attrs::Consistent(sc) = std::mem::take(&mut etm.supercluster) else {
                    unreachable!()
                };
                for mut a in sc {
                    ...
                    self.terms.0[i]
                        .supercluster
                        .try_insert(&self.g.constrs, self.lc, a)?;
                }
            }
        }
                ...
                | Formula::Pred { .. } => {
                    if pos_bas
                        .0
                         .0
                        .iter()
                        .any(|pos| EqMarks.eq(self.g, self.lc, pos, neg))
                    {
                        return Err(Unsat);
                    }
                }
                    ...
                    if self.g.reqs.less_or_equal() == Some(nr) {
                            ...
                            (self.g.reqs.positive(), self.g.reqs.negative())
                        {
                            ...
                            let pos1 =
                                self.terms[et1]
                                    .supercluster
                                    .find0(&self.g.constrs, positive, true);
                            added |= pos1
                                && self.terms[et2].supercluster.try_insert(
                                    &self.g.constrs,
                                    self.lc,
                                    MizAttr::new0(positive, true),
                                )?;
                            // a <= b, b is negative => a is negative
                            let neg2 =
                                self.terms[et2]
                                    .supercluster
                                    .find0(&self.g.constrs, negative, true);
                            added |= neg2
                                && self.terms[et1].supercluster.try_insert(
                                    &self.g.constrs,
                                    self.lc,
                                    MizAttr::new0(negative, true),
                                )?;
                            // a <= b, a is non negative => b is non negative
                            let nonneg1 = self.terms[et1].supercluster.find0(
                                &self.g.constrs,
                                negative,
                                false,
                            );
                            added |= nonneg1
                                && self.terms[et2].supercluster.try_insert(
                                    &self.g.constrs,
                                    self.lc,
                                    MizAttr::new0(negative, false),
                                )?;
                            // a <= b, b is non positive => a is non positive
                            let nonpos2 = self.terms[et2].supercluster.find0(
                                &self.g.constrs,
                                positive,
                                false,
                            );
                            added |= nonpos2
                                && self.terms[et1].supercluster.try_insert(
                                    &self.g.constrs,
                                    self.lc,
                                    MizAttr::new0(positive, false),
                                )?;
                            if let Some(zero) = self.g.reqs.zero() {
                                // a <= b, a is non negative, b is non zero => b is positive
                                if nonneg1
                                    && self.terms[et2].supercluster.find0(
                                        &self.g.constrs,
                                        zero,
                                        false,
                                    )
                                ...
                                if nonpos2
                                    && self.terms[et1].supercluster.find0(
                                        &self.g.constrs,
                                        zero,
                                        false,
                                    )
                        ...
                        if let (Some(element), Some(pw)) =
                            (self.g.reqs.element(), self.g.reqs.power_set())
                        {
                            let [arg1, arg2] = args else { unreachable!() };
                            // A c= B => A: Element of bool B
                            let mut tm = Term::Functor {
                                nr: pw,
                        ...
                        for ty in &self.terms[et2].ty_class {
                            if let TypeKind::Mode(n) = ty.kind {
                                ...
                                if self.g.reqs.element() == Some(n) {
                                    ...
                                    for &m in
                                        &self.terms[self.lc.marks[arg3.mark().unwrap()].1].eq_class
                                    {
                    ...
                    Formula::Pred { nr, args } => {
                        ...
                        if self.g.reqs.less_or_equal() == Some(nr) {
                                ...
                                (self.g.reqs.positive(), self.g.reqs.negative())
                            {
                                ...
                                added |= self.terms[et1].supercluster.find0(
                                    &self.g.constrs,
                                    positive,
                                    false,
                                ) && self.terms[et2].supercluster.try_insert(
                                ...
                                added |= self.terms[et2].supercluster.find0(
                                    &self.g.constrs,
                                    negative,
                                    false,
                                ) && self.terms[et1].supercluster.try_insert(
                            ...
                            if let (Some(element), Some(empty)) =
                                (self.g.reqs.element(), self.g.reqs.empty())
                            {
                                let [arg1, arg2] = args else { unreachable!() };
                                let et1 = self.lc.marks[arg1.mark().unwrap()].1;
                                let et2 = self.lc.marks[arg2.mark().unwrap()].1;
                                if self.terms[et2]
                                    .supercluster
                                    .find0(&self.g.constrs, empty, false)
                                {
                                    let ty = Type {
                                        args: vec![arg2.clone()],
                                        ..Type::new(element.into())
                                    ...
                                    if self.terms[et1].ty_class.iter().any(|ty2| {
                                        ty2.decreasing_attrs(&ty, |a1, a2| {
                                            EqMarks.eq(self.g, self.lc, a1, a2)
                    ...
                    Formula::Is { term, ty } => {
                        ...
                        if self.with_eq(|ctx| {
                            self.terms[et]
                                .ty_class
                                .iter()
                                .any(|ty2| EqMarks.eq_radices(ctx, ty, ty2))
                        }) {
                            return Err(Unsat);
                        }
                    }
        ...
        let mut eq_stack: BTreeSet<EqTermId> = self
            .terms
            .enum_iter()
            .filter(|p| !p.1.eq_class.is_empty())
            .map(|p| p.0)
            .collect();
            ...
            .tuple_combinations()
        {
            if etm1
                .supercluster
                .contradicts(&self.g.constrs, self.lc, &etm2.supercluster)
            {
                ...
                Formula::Pred { nr, args } => {
                    ...
                    if self.g.reqs.equals_to() != Some(nr) {
                        for f2 in &pos_bas.0 .0 {
                            ...
                            {
                                let (nr2, args2) =
                                    Formula::adjust_pred(*nr2, args2, Some(&self.g.constrs));
                                if nr == nr2 {
