...
use husky_typst_syntax::TypstSynSpan;
...
impl TimingScope {
    ...
    pub fn new(name: &'static str, span: Option<TypstSynSpan>) -> Option<Self> {
        if !is_enabled() {
            return None;
        }
...
pub fn export_json<W: Write>(
    writer: W,
    mut source: impl FnMut(TypstSynSpan) -> (String, u32),
) -> Result<(), String> {
    #[derive(Serialize)]
    struct Entry {
        name: &'static str,
