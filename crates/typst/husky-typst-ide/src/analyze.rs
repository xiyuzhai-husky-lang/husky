...
use husky_typst::syntax::{ast, LinkedNode, TypstSynSpan, TypstSyntaxKind};
use husky_typst::IsTypstWorld;

/// Try to determine a set of possible values for an expression.
pub fn analyze_expr(world: &dyn IsTypstWorld, node: &LinkedNode) -> EcoVec<TypstValue> {
    match node.cast::<ast::Expr>() {
        ...
        Some(_) => {
            if let Some(parent) = node.parent() {
                if parent.kind() == TypstSyntaxKind::FieldAccess && node.index() > 0 {
            ...
            husky_typst::compile(world, &mut tracer).ok();
            tracer.values()
        }
...
pub fn analyze_import(world: &dyn IsTypstWorld, source: &LinkedNode) -> Option<TypstValue> {
    ...
    let mut vm = Vm::new(
        engine,
        TypstValueAssignmentGroups::new(Some(world.library())),
        TypstSynSpan::detached(),
    );
    husky_typst::eval::import(&mut vm, source, TypstSynSpan::detached(), true)
        .ok()
