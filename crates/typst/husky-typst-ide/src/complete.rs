...
use husky_typst::syntax::{
    ast, is_id_continue, is_id_start, is_ident, LinkedNode, Source, TypstSyntaxKind,
};
...
fn complete_comments(ctx: &mut CompletionContext) -> bool {
    matches!(
        ctx.leaf.kind(),
        TypstSyntaxKind::LineComment | TypstSyntaxKind::BlockComment
    )
}
...
fn complete_markup(ctx: &mut CompletionContext) -> bool {
    ...
    if !matches!(
        ...
        None | Some(TypstSyntaxKind::TypstMarkup) | Some(TypstSyntaxKind::Ref)
    ) {
        return false;
    }

    // Start of an interpolated identifier: "#|".
    if ctx.leaf.kind() == TypstSyntaxKind::Hash {
        ctx.from = ctx.cursor;
    ...
    if ctx.leaf.kind() == TypstSyntaxKind::Ident {
    ...
    if ctx.leaf.kind() == TypstSyntaxKind::RefMarker {
    ...
    if_chain! {
        ...
        if prev.kind() == TypstSyntaxKind::Eq;
        if prev.parent_kind() == Some(TypstSyntaxKind::LetBinding);
        then {
            ctx.from = ctx.cursor;
...
fn complete_math(ctx: &mut CompletionContext) -> bool {
    if !matches!(
        ctx.leaf.parent_kind(),
        Some(TypstSyntaxKind::Equation)
            | Some(TypstSyntaxKind::Math)
            | Some(TypstSyntaxKind::MathFrac)
            | Some(TypstSyntaxKind::MathAttach)
    ) {
        return false;
    }
    ...
    if matches!(
        ctx.leaf.kind(),
        TypstSyntaxKind::Text | TypstSyntaxKind::MathIdent
    ) {
        ctx.from = ctx.leaf.offset();
...
fn complete_field_accesses(ctx: &mut CompletionContext) -> bool {
    ...
    if_chain! {
        if ctx.leaf.kind() == TypstSyntaxKind::Dot
            || (ctx.leaf.kind() == TypstSyntaxKind::Text
                && ctx.leaf.text() == ".");
        if ctx.leaf.range().end == ctx.cursor;
        if let Some(prev) = ctx.leaf.prev_sibling();
        if prev.is::<ast::Expr>();
        if prev.parent_kind() != Some(TypstSyntaxKind::TypstMarkup) ||
           prev.prev_sibling_kind() == Some(TypstSyntaxKind::Hash);
...
fn complete_imports(ctx: &mut CompletionContext) -> bool {
    ...
    if_chain! {
        if matches!(
            ctx.leaf.parent_kind(),
            Some(TypstSyntaxKind::ModuleImport | TypstSyntaxKind::ModuleInclude)
        );
    ...
    if_chain! {
        ...
        if parent.kind() == TypstSyntaxKind::ImportItems;
...
fn complete_rules(ctx: &mut CompletionContext) -> bool {
    ...
    if matches!(prev.kind(), TypstSyntaxKind::Set) {
        ctx.from = ctx.cursor;
    ...
    if matches!(prev.kind(), TypstSyntaxKind::Show) {
        ctx.from = ctx.cursor;
    ...
    if_chain! {
        if let Some(prev) = ctx.leaf.prev_leaf();
        if matches!(prev.kind(), TypstSyntaxKind::Colon);
        if matches!(prev.parent_kind(), Some(TypstSyntaxKind::ShowRule));
        then {
            ctx.from = ctx.cursor;
...
fn complete_params(ctx: &mut CompletionContext) -> bool {
    ...
    let (callee, set, args) = if_chain! {
        ...
        if let Some(parent) = match parent.kind() {
            TypstSyntaxKind::Named => parent.parent(),
            _ => Some(parent),
        };
    ...
    while !matches!(
        deciding.kind(),
        TypstSyntaxKind::LeftParen | TypstSyntaxKind::Comma | TypstSyntaxKind::Colon
    ) {
    ...
    if_chain! {
        if deciding.kind() == TypstSyntaxKind::Colon;
    ...
    if_chain! {
        if matches!(deciding.kind(), TypstSyntaxKind::LeftParen | TypstSyntaxKind::Comma);
        if deciding.kind() != TypstSyntaxKind::Comma || deciding.range().end < ctx.cursor;
        then {
            if let Some(next) = deciding.next_leaf() {
                ctx.from = ctx.cursor.min(next.offset());
            }
...
fn complete_code(ctx: &mut CompletionContext) -> bool {
    if matches!(
        ctx.leaf.parent_kind(),
        None | Some(TypstSyntaxKind::TypstMarkup)
            | Some(TypstSyntaxKind::Math)
            | Some(TypstSyntaxKind::MathFrac)
            | Some(TypstSyntaxKind::MathAttach)
            | Some(TypstSyntaxKind::MathRoot)
    ) {
        return false;
    }
    ...
    if ctx.explicit
        && (ctx.leaf.kind().is_trivia()
            || matches!(
                ctx.leaf.kind(),
                TypstSyntaxKind::LeftParen | TypstSyntaxKind::LeftBrace
            ))
    {
        ctx.from = ctx.cursor;
        code_completions(ctx, false);
...
struct CompletionContext<'a> {
    world: &'a (dyn IsTypstWorld + 'a),
    document: Option<&'a TypstDocument>,
    global: &'a TypstValueAssignmentGroup,
    math: &'a TypstValueAssignmentGroup,
    text: &'a str,
    before: &'a str,
    after: &'a str,
    leaf: LinkedNode<'a>,
...
impl<'a> CompletionContext<'a> {
    ...
    fn cast_completions(&mut self, cast: &'a CastInfo) {
        ...
        if !self.seen_casts.insert(husky_typst::util::hash128(cast)) {
            return;
        }
        ...
        match cast {
            ...
            CastInfo::Type(ty) => {
                if *ty == Type::of::<NoneTypstValue>() {
                    self.snippet_completion("none", "none", "Nothing.")
                } else if *ty == Type::of::<AutoTypstValue>() {
        ...
        while let Some(node) = &ancestor {
            ...
            if let Some(parent) = node.parent() {
                if let Some(v) = parent.cast::<ast::ForLoop>() {
                    if node.prev_sibling_kind() != Some(TypstSyntaxKind::In) {
