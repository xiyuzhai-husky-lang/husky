...
fn expr_tooltip(world: &dyn IsTypstWorld, leaf: &LinkedNode) -> Option<Tooltip> {
...
fn closure_tooltip(leaf: &LinkedNode) -> Option<Tooltip> {
    ...
    if !matches!(leaf.kind(), TypstSyntaxKind::Eq | TypstSyntaxKind::Arrow) {
        return None;
    }

    // Find the closure to analyze.
    let parent = leaf.parent()?;
    if parent.kind() != TypstSyntaxKind::Closure {
        return None;
    }
...
fn label_tooltip(document: &TypstDocument, leaf: &LinkedNode) -> Option<Tooltip> {
    let target = match leaf.kind() {
        TypstSyntaxKind::RefMarker => leaf.text().trim_start_matches('@'),
        TypstSyntaxKind::Label => leaf.text().trim_start_matches('<').trim_end_matches('>'),
        _ => return None,
    };
...
fn named_param_tooltip(world: &dyn IsTypstWorld, leaf: &LinkedNode) -> Option<Tooltip> {
    let (func, named) = if_chain! {
        ...
        if matches!(grand.kind(), TypstSyntaxKind::Args);
...
/// Tooltip for font.
fn font_tooltip(world: &dyn IsTypstWorld, leaf: &LinkedNode) -> Option<Tooltip> {
    if_chain! {
