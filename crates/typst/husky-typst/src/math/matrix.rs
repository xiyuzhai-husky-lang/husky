...
const DEFAULT_COL_GAP: TypstEmLength = TypstEmLength::new(0.5);
const VERTICAL_PADDING: Ratio = Ratio::new(0.1);
const DEFAULT_STROKE_THICKNESS: TypstEmLength = TypstEmLength::new(0.05);
...
///     = a + 2b + 3c $
/// ```
#[elem(title = "Vector", TypstLayoutMath)]
pub struct VecElem {
...
/// ) $
/// ```
#[elem(title = "Matrix", TypstLayoutMath)]
pub struct MatElem {
    /// The delimiter to use.
    ///
    /// ```example
...
fn layout_mat_body(
    ...
    let nrows = rows.len();
    if ncols == 0 || nrows == 0 {
        return Ok(TypstFrame::soft(Size::zero()));
    }
    ...
    let mut heights = vec![(TypstAbsLength::zero(), TypstAbsLength::zero()); nrows];
    ...
    let total_height =
        heights.iter().map(|&(a, b)| a + b).sum::<TypstAbsLength>() + gap.y * (nrows - 1) as f64;

    // Width starts at zero because it can't be calculated until later
    let mut frame = TypstFrame::soft(Size::new(TypstAbsLength::zero(), total_height));

    let mut x = TypstAbsLength::zero();

    for (index, col) in cols.into_iter().enumerate() {
        let TypstAlignmentResult {
            points,
            width: rcol,
        } = alignments(&col);
