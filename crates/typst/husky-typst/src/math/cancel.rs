...
use crate::layout::{
    Angle, Length, Point, Ratio, Rel, Size, Transform, TypstAbsLength, TypstFrame, TypstFrameItem,
};
...
pub struct CancelLineTypstElem {
    ...
    ///     - cancel(x, length: #200%) $
    /// ```
    #[default(Rel::new(Ratio::one(), TypstAbsLength::pt(3.0).into()))]
...
impl TypstLayoutMath for TypstContentRefined<CancelLineTypstElem> {
    ...
    fn layout_math(&self, ctx: &mut MathContext, styles: StyleChain) -> SourceResult<()> {
        ...
        if cross {
            ...
            let second_line = draw_cancel_line(ctx, length, stroke, true, &angle, body_size, span)?;
...
fn draw_cancel_line(
    ...
    let mut angle = match angle {
        ...
        Smart::Custom(angle) => match angle {
            ...
            CancelAngle::Func(func) => func.call(ctx.engine, [default])?.cast().at(span)?,
        },
    };
    ...
    frame.push(
        start,
        TypstFrameItem::Shape(TypstGeometry::Line(delta).stroked(stroke), span),
    );
