...
pub fn module() -> TypstModuleEvaluation {
    ...
    math.define_elem::<ClassTypstElem>();
    ...
    TypstModuleEvaluation::new("math", math)
}
...
impl TypstLayoutMath for TypstContent {
    ...
    fn layout_math(
        ...
        if let Some(boxed) = self.to_packed::<BoxTypstElem>() {
            let frame = ctx.layout_box(boxed, styles)?;
            ctx.push(FrameFragment::new(ctx, styles, frame).with_spaced(true));
            return Ok(());
        }

        if let Some(elem) = self.with::<dyn TypstLayoutMath>() {
            return elem.layout_math(ctx, styles);
        }
