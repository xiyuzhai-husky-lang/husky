...
use crate::layout::{TypstFrame, TypstFrameItem};
...
impl<'a> Locator<'a> {
    ...
    pub fn visit_frame(&mut self, frame: &TypstFrame) {
        for (_, item) in frame.items() {
            match item {
                TypstFrameItem::Group(group) => self.visit_frame(&group.frame),
                TypstFrameItem::Meta(TypstMeta::Elem(elem), _) => {
    ...
    pub fn visit_frames<'b>(&mut self, frames: impl IntoIterator<Item = &'b TypstFrame>) {
        for frame in frames {
