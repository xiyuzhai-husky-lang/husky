...
pub struct Introspector {
    ...
    elems: IndexMap<Location, (Prehashed<TexContent>, Position)>,
...
impl Introspector {
    ...
    pub fn all(&self) -> impl Iterator<Item = &Prehashed<TexContent>> + '_ {
        self.elems.values().map(|(c, _)| c)
    }

    /// Get an element by its location.
    fn get(&self, location: &Location) -> Option<&Prehashed<TexContent>> {
        self.elems.get(location).map(|(elem, _)| elem)
    }

    /// Get the index of this element among all.
    fn index(&self, elem: &TexContent) -> usize {
        self.elems
...
impl Introspector {
    ...
    pub fn query(&self, selector: &Selector) -> EcoVec<Prehashed<TexContent>> {
    ...
    pub fn query_first(&self, selector: &Selector) -> Option<Prehashed<TexContent>> {
        match selector {
    ...
    pub fn query_label(&self, label: Label) -> StrResult<&Prehashed<TexContent>> {
...
/// Caches queries.
#[derive(Default)]
struct QueryCache(RwLock<HashMap<u128, EcoVec<Prehashed<TexContent>>>>);

impl QueryCache {
    fn get(&self, hash: u128) -> Option<EcoVec<Prehashed<TexContent>>> {
        self.0.read().unwrap().get(&hash).cloned()
    }

    fn insert(&self, hash: u128, output: EcoVec<Prehashed<TexContent>>) {
