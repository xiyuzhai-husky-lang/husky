...
pub struct EnumElem {
    ...
    /// ```
    #[variadic]
    pub children: Vec<TypstContentRefined<EnumItem>>,
...
impl LayoutMultiple for TypstContentRefined<EnumElem> {
    #[husky_typst_macros::time(name = "enum", span = self.span())]
    fn layout(
        &self,
        ...
        for item in self.children() {
            ...
            cells.push(Cell::from(
                item.body()
                    .clone()
                    .styled(EnumElem::set_parents(smallvec![number])),
            ));
