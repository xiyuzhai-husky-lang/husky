...
pub struct TermsElem {
    ...
    /// ```
    #[default(HElem::new(TypstEmLength::new(0.6).into()).with_weak(true).pack())]
    #[borrowed]
    ...
    /// ```
    #[variadic]
    pub children: Vec<TypstContentRefined<TermItem>>,
}
...
impl LayoutMultiple for TypstContentRefined<TermsElem> {
    #[husky_typst_macros::time(name = "terms", span = self.span())]
    fn layout(
        &self,
        ...
        let unpad =
            (!hanging_indent.is_zero()).then(|| HElem::new((-hanging_indent).into()).pack());
