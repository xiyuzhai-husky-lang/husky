...
impl BibliographyElem {
    ...
    pub fn find(introspector: Tracked<Introspector>) -> StrResult<TypstContentRefined<Self>> {
    ...
    pub fn has(engine: &TypstEngine, key: impl Into<PicoStr>) -> bool {
        let key = key.into();
        engine
            .introspector
    ...
    fn show(
        ...
        if references.iter().any(|(prefix, _)| prefix.is_some()) {
            ...
            for (prefix, reference) in references {
                cells.push(
                    TypstContentRefined::new(GridCell::new(prefix.clone().unwrap_or_default()))
                        .spanned(span),
                );
                cells
                    .push(TypstContentRefined::new(GridCell::new(reference.clone())).spanned(span));
            }

            seq.push(VElem::new(row_gutter).with_weakness(3).pack());
            seq.push(
                GridElem::new(cells)
                    .with_columns(TrackSizings(smallvec![TypstSizing::Auto; 2]))
        ...
        if works.hanging_indent {
            content = content.styled(ParagraphTypstElem::set_hanging_indent(INDENT.into()));
        }
...
impl LocalName for TypstContentRefined<BibliographyElem> {
...
impl Bibliography {
    ...
    #[comemo::memoize]
    #[husky_typst_macros::time(name = "load bibliography")]
...
struct Generator<'a> {
    ...
    world: Tracked<'a, dyn IsTypstWorld + 'a>,
    /// The document's bibliography.
    bibliography: TypstContentRefined<BibliographyElem>,
...
impl<'a> Generator<'a> {
    ...
    fn new(
        ...
        let groups = introspector.query(&TypstCiteGroup::elem().select());
    ...
    fn drive(&mut self) -> hayagriva::Rendered {
        ...
        for elem in &self.groups {
            let group = elem.to_packed::<TypstCiteGroup>().unwrap();
    ...
    fn display_citations(
        ...
        for (info, citation) in self.infos.iter().zip(&rendered.citations) {
            ...
            let content = if info.subinfos.iter().all(|sub| sub.hidden) {
                ...
                if info.footnote {
                    content = FootnoteTypstElem::with_content(content).pack();
                }
        ...
        for (k, item) in rendered.items.iter().enumerate() {
            ...
            let mut prefix = item.first_field.as_ref().map(|elem| {
                ...
                if let Some(location) = first_occurrences.get(item.key.as_str()) {
                    let dest = TypstDestination::Location(*location);
...
impl ElemRenderer<'_> {
    ...
    fn display_elem(
        ...
        if let Some(prefix) = suf_prefix {
            ...
            content = GridElem::new(vec![
                TypstContentRefined::new(GridCell::new(prefix)).spanned(self.span),
                TypstContentRefined::new(GridCell::new(content)).spanned(self.span),
            ])
            .with_columns(TrackSizings(smallvec![TypstSizing::Auto; 2]))
            .with_column_gutter(TrackSizings(smallvec![COLUMN_GUTTER.into()]))
            .pack()
    ...
    fn display_link(&self, text: &hayagriva::Formatted, url: &str) -> TypstContent {
        let dest = TypstDestination::Url(url.into());
        LinkTypstElem::new(dest.into(), self.display_formatted(text))
            .pack()
            .spanned(self.span)
    }
...
fn apply_formatting(mut content: TypstContent, format: &hayagriva::Formatting) -> TypstContent {
    match format.font_style {
        ...
        citationberg::FontStyle::Italic => {
            content = content.styled(TextElem::set_style(TypstFontStyle::Italic));
        }
    }
