...
pub struct LinkTypstElem {
    ...
    #[required]
    #[parse(match &dest {
        LinkTarget::Dest(TypstDestination::Url(url)) => match args.eat()? {
            Some(body) => body,
...
impl LinkTypstElem {
    ...
    pub fn from_url(url: EcoString) -> Self {
        ...
        Self::new(LinkTarget::Dest(TypstDestination::Url(url)), body)
    }
}
    ...
    fn show(&self, engine: &mut TypstEngine, _: StyleChain) -> SourceResult<TypstContent> {
        ...
        let linked = match self.dest() {
            ...
            LinkTarget::Label(label) => {
                ...
                let dest = TypstDestination::Location(elem.location().unwrap());
...
cast! {
    ...
    v: TypstDestination => Self::Dest(v),
    v: Label => Self::Label(v),
}

impl From<TypstDestination> for LinkTarget {
    fn from(dest: TypstDestination) -> Self {
        Self::Dest(dest)
    }
}
