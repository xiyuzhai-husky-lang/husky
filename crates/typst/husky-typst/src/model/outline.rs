...
pub struct OutlineElem {
    ...
    /// )
    /// ```
    #[default(LocatableSelector(select_where!(HeadingTypstElem, Outlined => true)))]
    #[borrowed]
...
impl LocalName for TypstContentRefined<OutlineElem> {
    ...
    fn apply(
        ...
        match indent {
            ...
            Some(Smart::Custom(OutlineIndent::Func(func))) => {
                ...
                let LengthOrContent(content) = func.call(engine, [depth])?.cast().at(span)?;
                if !content.is_empty() {
                    seq.push(content);
                }
            }
...
impl Show for TypstContentRefined<OutlineEntry> {
    ...
    fn show(&self, _: &mut TypstEngine, _: TypstStyleChain) -> TypstSourceResult<TypstContent> {
        ...
        seq.push(
            self.body()
                .clone()
                .linked(TypstDestination::Location(location)),
        );
        ...
        if let Some(filler) = self.fill() {
            ...
            seq.push(
                BoxTypstElem::new()
                    ...
                    .with_width(TypstFraction::one().into())
                    .pack()
                    .spanned(self.span()),
            );
            seq.push(SpaceElem::new().pack());
        } else {
            seq.push(HElem::new(TypstFraction::one().into()).pack());
        }
