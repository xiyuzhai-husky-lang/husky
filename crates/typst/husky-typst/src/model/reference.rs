...
pub struct RefElem {
    ...
    #[synthesized]
    pub citation: Option<TypstContentRefined<CiteTypstElem>>,
...
impl Show for TypstContentRefined<RefElem> {
    ...
    fn show(
        ...
        if elem.func() == FootnoteTypstElem::elem() {
            return Ok(FootnoteTypstElem::with_label(target).pack().spanned(span));
        }
        ...
        Ok(content.linked(TypstDestination::Location(loc)))
    }
}
