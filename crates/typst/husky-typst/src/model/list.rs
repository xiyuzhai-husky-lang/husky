...
pub struct ListElem {
    ...
    /// ```
    #[variadic]
    pub children: Vec<TypstContentRefined<ListItem>>,
...
impl LayoutMultiple for TypstContentRefined<ListElem> {
    #[husky_typst_macros::time(name = "list", span = self.span())]
    fn layout(
        &self,
        ...
        let gutter = if self.tight(styles) {
            ParagraphTypstElem::leading_in(styles).into()
        } else {
            self.spacing(styles)
        ...
        let grid = CellGrid::new(
            Axes::with_x(&[
                TypstSizing::Rel(indent.into()),
                TypstSizing::Auto,
                TypstSizing::Rel(body_indent.into()),
                TypstSizing::Auto,
            ]),
