    ...
    fn show(
        ...
        if let Some(numbering) = (**self).numbering(styles).as_ref() {
            realized = Counter::of(HeadingTypstElem::elem())
                ...
                .spanned(self.span())
                + HElem::new(TypstEmLength::new(0.3).into())
                    .with_weak(true)
                    .pack()
                + realized;
        }
...
impl ShowSet for TypstContentRefined<HeadingTypstElem> {
    fn show_set(&self, styles: TypstStyleChain) -> TypstStyles {
        ...
        let size = TypstEmLength::new(scale);
...
impl Refable for TypstContentRefined<HeadingTypstElem> {
    ...
    fn counter(&self) -> Counter {
        Counter::of(HeadingTypstElem::elem())
    }
...
impl LocalName for TypstContentRefined<HeadingTypstElem> {
