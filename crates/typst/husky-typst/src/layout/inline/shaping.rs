...
use crate::text::{
    decorate, families, features, variant, FontVariant, Lang, Region, TextElem, TypstFont,
    TypstGlyph, TypstTextItem,
};
...
pub(super) struct Adjustability {
    ...
    pub stretchability: (TypstEmLength, TypstEmLength),
    /// The left and right shrinkability
    pub shrinkability: (TypstEmLength, TypstEmLength),
}
...
impl ShapedGlyph {
    ...
    pub fn base_adjustability(&self, gb_style: bool) -> Adjustability {
        ...
        if self.is_space() {
            Adjustability {
                ...
                stretchability: (TypstEmLength::zero(), width / 2.0),
                shrinkability: (TypstEmLength::zero(), width / 3.0),
            }
        } else if self.is_cjk_left_aligned_punctuation(gb_style) {
            Adjustability {
                stretchability: (TypstEmLength::zero(), TypstEmLength::zero()),
                shrinkability: (TypstEmLength::zero(), width / 2.0),
            }
        } else if self.is_cjk_right_aligned_punctuation() {
            Adjustability {
                stretchability: (TypstEmLength::zero(), TypstEmLength::zero()),
                shrinkability: (width / 2.0, TypstEmLength::zero()),
            }
    ...
    pub fn stretchability(&self) -> (TypstEmLength, TypstEmLength) {
        self.adjustability.stretchability
    }

    /// The shrinkability of the character.
    pub fn shrinkability(&self) -> (TypstEmLength, TypstEmLength) {
        self.adjustability.shrinkability
    }

    /// Shrink the width of glyph on the left side.
    pub fn shrink_left(&mut self, amount: TypstEmLength) {
    ...
    pub fn shrink_right(&mut self, amount: TypstEmLength) {
        self.x_advance -= amount;
...
impl<'a> ShapedText<'a> {
    ...
    pub fn build(
        ...
        extra_justification: TypstAbsLength,
    ) -> TypstFrame {
            ...
            .group_by_key(|g| (g.font.clone(), g.y_offset))
        {
            ...
            let glyphs: Vec<TypstGlyph> = group
                ...
                .map(|shaped: &ShapedGlyph| {
                    ...
                    if shaped.is_justifiable() {
                        justification_right +=
                            TypstEmLength::from_length(extra_justification, self.size)
                    }
            ...
            if decos.is_empty() {
                frame.push(pos, TypstFrameItem::Text(item));
            } else {
                // Apply line decorations.
                frame.push(pos, TypstFrameItem::Text(item.clone()));
                for deco in &decos {
    ...
    fn measure(&self, engine: &TypstEngine) -> (TypstAbsLength, TypstAbsLength) {
    ...
    pub fn stretchability(&self) -> TypstAbsLength {
        self.glyphs
            .iter()
    ...
    pub fn shrinkability(&self) -> TypstAbsLength {
        self.glyphs
            .iter()
    ...
    pub fn push_hyphen(&mut self, engine: &TypstEngine, fallback: bool) {
        let world = engine.world;
        let book = world.book();
        ...
        chain.find_map(|id| {
            ...
            self.glyphs.to_mut().push(ShapedGlyph {
                ...
                y_offset: TypstEmLength::zero(),
                adjustability: Adjustability::default(),
                range,
                safe_to_break: true,
                c: '-',
                span: (TypstSynSpan::detached(), 0),
...
fn track_and_space(ctx: &mut ShapingContext) {
    let tracking = TypstEmLength::from_length(TextElem::tracking_in(ctx.styles), ctx.size);
    let spacing =
        TextElem::spacing_in(ctx.styles).map(|abs| TypstEmLength::from_length(abs, ctx.size));
...
fn nbsp_delta(font: &TypstFont) -> Option<TypstEmLength> {
...
fn is_cjk_left_aligned_punctuation(
    c: char,
    x_advance: TypstEmLength,
    stretchability: (TypstEmLength, TypstEmLength),
    gb_style: bool,
) -> bool {
    // CJK quotation marks shares codepoints with latin quotation marks.
    // But only the CJK ones have full width.
    if matches!(c, '”' | '’') && x_advance + stretchability.1 == TypstEmLength::one() {
        return true;
    }
...
fn is_cjk_right_aligned_punctuation(
    ...
    // CJK quotation marks shares codepoints with latin quotation marks.
    // But only the CJK ones have full width.
    if matches!(c, '“' | '‘') && x_advance + stretchability.0 == TypstEmLength::one() {
        return true;
    }
    // See appendix A.3 https://www.w3.org/TR/clreq/#tables_of_chinese_punctuation_marks
    matches!(
