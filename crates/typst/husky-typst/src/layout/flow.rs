...
enum FlowItem {
    ...
    Absolute(TypstAbsLength, bool),
    /// Fractional spacing between other items.
    Fractional(TypstFraction),
...
impl FlowItem {
    ...
    fn height(&self) -> TypstAbsLength {
        match self {
            Self::Absolute(v, _) => *v,
            Self::Fractional(_) | Self::Placed { .. } => TypstAbsLength::zero(),
    ...
    fn is_out_of_flow(&self) -> bool {
        match self {
            ...
            Self::Frame { frame, .. } => frame
                .items()
                .all(|(_, item)| matches!(item, TypstFrameItem::Meta(..))),
            _ => false,
        }
    }
}
...
impl<'a> FlowLayouter<'a> {
    ...
    fn layout_item(
        ...
        match item {
            ...
            FlowItem::Placed {
                ...
                if !self.regions.size.y.fits(frame.height() + clearance) && !self.regions.in_last()
                {
                ...
                if y_align.is_auto() {
                    let ratio = (self.regions.size.y - (frame.height() + clearance) / 2.0)
                        / self.regions.full;
    ...
    fn finish_region(&mut self, engine: &mut TypstEngine, force: bool) -> TypstSourceResult<()> {
        ...
        let mut footnote_height = TypstAbsLength::zero();
        let mut float_top_height = TypstAbsLength::zero();
        let mut float_bottom_height = TypstAbsLength::zero();
        ...
        let mut float_top_offset = TypstAbsLength::zero();
        let mut offset = float_top_height;
        let mut float_bottom_offset = TypstAbsLength::zero();
        let mut footnote_offset = TypstAbsLength::zero();
            ...
            match item {
                ...
                FlowItem::Placed {
                    ...
                    let y = if float {
                        ...
                        match y_align {
                            Smart::Custom(Some(align)) => align.position(size.y - frame.height()),
    ...
    fn finish(mut self, engine: &mut TypstEngine) -> TypstSourceResult<TypstLayoutFragment> {
        ...
        Ok(TypstLayoutFragment::frames(self.finished))
    }
}
...
impl FlowLayouter<'_> {
    ...
    fn handle_footnotes(
        ...
        while k < notes.len() {
            ...
            // together).
            if !force && (k == 0 || movable) && frames.first().map_or(false, TypstFrame::is_empty) {
...
fn find_footnotes(notes: &mut Vec<TypstContentRefined<FootnoteTypstElem>>, frame: &TypstFrame) {
    for (_, item) in frame.items() {
        match item {
            TypstFrameItem::Group(group) => find_footnotes(notes, &group.frame),
            TypstFrameItem::Meta(TypstMeta::Elem(content), _)
                if !notes
                    .iter()
                    .any(|note| note.location() == content.location()) =>
            {
                let Some(footnote) = content.to_packed::<FootnoteTypstElem>() else {
                    continue;
                };
