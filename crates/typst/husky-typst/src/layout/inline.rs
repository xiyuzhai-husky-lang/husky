...
impl<'a> Item<'a> {
    ...
    fn width(&self) -> TypstAbsLength {
        match self {
            Self::Text(shaped) => shaped.width,
            Self::Absolute(v) => *v,
            Self::Frame(frame) => frame.width(),
            Self::Fractional(_, _) | Self::Meta(_) => TypstAbsLength::zero(),
        }
    }
}
...
#[derive(Default)]
struct SpanMapper(Vec<(usize, TypstSynSpan)>);
...
impl SpanMapper {
    ...
    fn push(&mut self, len: usize, span: TypstSynSpan) {
    ...
    fn span_at(&self, offset: usize) -> (TypstSynSpan, u16) {
        let mut cursor = 0;
...
impl<'a> Line<'a> {
    ...
    fn stretchability(&self) -> TypstAbsLength {
        self.items()
    ...
    fn shrinkability(&self) -> TypstAbsLength {
        self.items()
            .filter_map(Item::text)
...
fn collect<'a>(
    ...
    let first_line_indent = ParagraphTypstElem::first_line_indent_in(*styles);
...
fn prepare<'a>(
    ...
    Ok(Preparation {
        ...
        justify: ParagraphTypstElem::justify_in(styles),
        hang: ParagraphTypstElem::hanging_indent_in(styles),
        cjk_latin_spacing,
        fallback: TextElem::fallback_in(styles),
        leading: ParagraphTypstElem::leading_in(styles),
        linebreaks: ParagraphTypstElem::linebreaks_in(styles),
    ...
    while let Some(item) = items.next() {
        ...
        while let Some(glyph) = glyphs.next() {
            ...
            if glyph.is_cj_script() && next.map_or(false, |g| g.is_letter_or_number()) {
                ...
                glyph.x_advance += TypstEmLength::new(0.25);
                glyph.adjustability.shrinkability.1 += TypstEmLength::new(0.125);
                text.width += TypstEmLength::new(0.25).at(text.size);
            }

            // Case 2: Latin followed by a CJ character
            if glyph.is_cj_script() && prev.map_or(false, |g| g.is_letter_or_number()) {
                glyph.x_advance += TypstEmLength::new(0.25);
                glyph.x_offset += TypstEmLength::new(0.25);
                glyph.adjustability.shrinkability.0 += TypstEmLength::new(0.125);
                text.width += TypstEmLength::new(0.25).at(text.size);
            }
...
fn linebreak_optimized<'a>(
    ...
    breakpoints(p, |end, breakpoint| {
        ...
        for (i, pred) in table.iter().enumerate().skip(active) {
            ...
            let adjust = if delta >= TypstAbsLength::zero() {
            ...
            if hyphen || start < range.end || before.is_empty() {
                ...
                if let Some(last_glyph) = reshaped.glyphs.last() {
                    if last_glyph.is_cjk_left_aligned_punctuation(gb_style) {
                        ...
                        && (last_glyph.x_advance - last_glyph.x_offset) > TypstEmLength::one()
                    {
                        ...
                        glyph.adjustability.shrinkability.1 = TypstEmLength::zero();
                        reshaped.width -= shrink_amount.at(reshaped.size);
                    }
                }
        ...
        if let Some(reshaped) = reshaped {
            if let Some(first_glyph) = reshaped.glyphs.first() {
                if first_glyph.is_cjk_right_aligned_punctuation() {
                    ...
                    && first_glyph.x_offset > TypstEmLength::zero()
                {
                    ...
                    let glyph = reshaped.glyphs.to_mut().first_mut().unwrap();
                    glyph.x_advance -= shrink_amount;
                    glyph.x_offset = TypstEmLength::zero();
                    glyph.adjustability.shrinkability.0 = TypstEmLength::zero();
                    let amount_abs = shrink_amount.at(reshaped.size);
                    reshaped.width -= amount_abs;
                    width -= amount_abs;
                }
...
fn commit(
    ...
    let mut extra_justification = TypstAbsLength::zero();

    let shrink = line.shrinkability();
    let stretch = line.stretchability();
    if remaining < TypstAbsLength::zero() && shrink > TypstAbsLength::zero() {
        // Attempt to reduce the length of the line, using shrinkability.
        justification_ratio = (remaining / shrink).max(-1.0);
        remaining = (remaining + shrink).min(TypstAbsLength::zero());
    } else if line.justify && fr.is_zero() {
        // Attempt to increase the length of the line, using stretchability.
        if stretch > TypstAbsLength::zero() {
            justification_ratio = (remaining / stretch).min(1.0);
            remaining = (remaining - stretch).max(TypstAbsLength::zero());
        }

        let justifiables = line.justifiables();
        if justifiables > 0 && remaining > TypstAbsLength::zero() {
            // Underfull line, distribute the extra space.
            extra_justification = remaining / justifiables as f64;
            remaining = TypstAbsLength::zero();
        }
    }

    let mut top = TypstAbsLength::zero();
    let mut bottom = TypstAbsLength::zero();

    // Build the frames and determine the height and baseline.
    let mut frames = vec![];
    for item in reordered {
        let mut push = |offset: &mut TypstAbsLength, frame: TypstFrame| {
