...
impl Sides<Rel<TypstAbsLength>> {
    /// Evaluate the sides relative to the given `size`.
    pub fn relative_to(self, size: Size) -> Sides<TypstAbsLength> {
        Sides {
...
cast! {
    Side,
    self => TypstAlignment::from(self).into_value(),
    align: TypstAlignment => match align {
        TypstAlignment::LEFT => Self::Left,
        TypstAlignment::RIGHT => Self::Right,
        TypstAlignment::TOP => Self::Top,
        TypstAlignment::BOTTOM => Self::Bottom,
