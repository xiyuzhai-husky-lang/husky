...
use crate::diag::{StrResult, Trace, Tracepoint, TypstSourceResult};
use crate::engine::TypstEngine;
use crate::foundations::{
    cast, elem, scope, Array, Fold, Show, Smart, TypstContent, TypstContentRefined,
    TypstStyleChain, TypstValue,
};
use crate::layout::{
    AlignElem, Axes, LayoutMultiple, Rel, Sides, TypstAlignment, TypstLayoutFragment, TypstLength,
...
pub struct GridElem {
    ...
    /// ```
    #[borrowed]
    pub align: Celled<Smart<TypstAlignment>>,
    ...
    #[variadic]
    pub children: Vec<TypstContentRefined<GridCell>>,
}
...
impl LayoutMultiple for TypstContentRefined<GridElem> {
    #[husky_typst_macros::time(name = "grid", span = self.span())]
    fn layout(
        &self,
...
pub struct TrackSizings(pub SmallVec<[TypstSizing; 4]>);

cast! {
    TrackSizings,
    self => self.0.into_value(),
    sizing: TypstSizing => Self(smallvec![sizing]),
    count: NonZeroUsize => Self(smallvec![TypstSizing::Auto; count.get()]),
...
impl Default for TypstContentRefined<GridCell> {
    fn default() -> Self {
        TypstContentRefined::new(GridCell::new(TypstContent::default()))
    }
}

impl ResolvableCell for TypstContentRefined<GridCell> {
    fn resolve_cell(
        mut self,
        x: usize,
        y: usize,
    ...
    fn span(&self) -> TypstSynSpan {
        TypstContentRefined::span(self)
    }
}

impl Show for TypstContentRefined<GridCell> {
    fn show(
        &self,
