...
pub struct CellGrid {
    ...
    cols: Vec<TypstSizing>,
    /// The row tracks including gutter tracks.
    rows: Vec<TypstSizing>,
...
impl CellGrid {
    ...
    pub fn new(
        tracks: Axes<&[TypstSizing]>,
        gutter: Axes<&[TypstSizing]>,
    ...
    fn new_internal(
        ...
        let auto = TypstSizing::Auto;
        let zero = TypstSizing::Rel(Rel::zero());
...
enum Row {
    ...
    Frame(TypstFrame, usize),
    /// Fractional row with y index.
    Fr(TypstFraction, usize),
}
...
impl<'a> GridLayouter<'a> {
    ...
    pub fn new(
        ...
        Self {
            ...
            regions,
            styles,
            rcols: vec![TypstAbsLength::zero(); grid.cols.len()],
    ...
    pub fn layout(mut self, engine: &mut TypstEngine) -> TypstSourceResult<TypstLayoutFragment> {
        ...
        for y in 0..self.grid.rows.len() {
            ...
            match self.grid.rows[y] {
                TypstSizing::Auto => self.layout_auto_row(engine, y)?,
                TypstSizing::Rel(v) => self.layout_relative_row(engine, v, y)?,
                TypstSizing::Fr(v) => self.lrows.push(Row::Fr(v, y)),
            }
        }
    ...
    fn render_fills_strokes(mut self) -> TypstSourceResult<TypstLayoutFragment> {
        ...
        for (frame, rows) in finished.iter_mut().zip(&self.rrows) {
            ...
            let mut dx = TypstAbsLength::zero();
            for (x, &col) in self.rcols.iter().enumerate().rev_if(self.is_rtl) {
                let mut dy = TypstAbsLength::zero();
                for row in rows {
        ...
        Ok(TypstLayoutFragment::frames(finished))
    }
    ...
    fn measure_columns(&mut self, engine: &mut TypstEngine) -> TypstSourceResult<()> {
        ...
        for (&col, rcol) in self.grid.cols.iter().zip(&mut self.rcols) {
            match col {
                ...
                TypstSizing::Fr(v) => fr += v,
            }
        }

        // Size that is not used by fixed-size columns.
        let available = self.regions.size.x - rel;
        if available >= TypstAbsLength::zero() {
            ...
            if remaining >= TypstAbsLength::zero() {
    ...
    fn cell_spanned_width(&self, x: usize, colspan: usize) -> TypstAbsLength {
        self.rcols
            .iter()
            .skip(x)
    ...
    fn measure_auto_columns(
        ...
        for (x, &col) in self.grid.cols.iter().enumerate() {
            if col != TypstSizing::Auto {
                continue;
            }

            let mut resolved = TypstAbsLength::zero();
            for y in 0..self.grid.rows.len() {
                ...
                if colspan > 1 {
                    let last_spanned_auto_col = self
                        ...
                        .rev()
                        .find(|(_, col)| **col == TypstSizing::Auto)
                        .map(|(x, _)| x);
    ...
    fn grow_fractional_columns(&mut self, remaining: TypstAbsLength, fr: TypstFraction) {
        if fr.is_zero() {
            return;
        }

        for (&col, rcol) in self.grid.cols.iter().zip(&mut self.rcols) {
            if let TypstSizing::Fr(v) = col {
    ...
    fn shrink_auto_columns(&mut self, available: TypstAbsLength, count: usize) {
        let mut last;
        let mut fair = -TypstAbsLength::inf();
        ...
        while changed && overlarge > 0 {
            ...
            for (&col, &rcol) in self.grid.cols.iter().zip(&self.rcols) {
                ...
                if col == TypstSizing::Auto && rcol <= fair && rcol > last {
                    redistribute -= rcol;
                    overlarge -= 1;
                    changed = true;
                }
        ...
        for (&col, rcol) in self.grid.cols.iter().zip(&mut self.rcols) {
            if col == TypstSizing::Auto && *rcol > fair {
                *rcol = fair;
            }
        }
    }
    ...
    fn layout_multi_row(
        ...
        let mut outputs: Vec<_> = heights
            .iter()
            .map(|&h| TypstFrame::soft(Size::new(self.width, h)))
            .collect();
        ...
        Ok(TypstLayoutFragment::frames(outputs))
    }

    /// Push a row frame into the current region.
    fn push_row(&mut self, frame: TypstFrame, y: usize) {
...
fn points(
    extents: impl IntoIterator<Item = TypstAbsLength>,
) -> impl Iterator<Item = TypstAbsLength> {
    let mut offset = TypstAbsLength::zero();
    std::iter::once(TypstAbsLength::zero())
        .chain(extents)
        .map(move |extent| {
            offset += extent;
            offset
...
) -> impl IntoIterator<Item = (TypstAbsLength, TypstAbsLength)> {
...
mod test {
    ...
    fn sample_grid(gutters: bool) -> CellGrid {
        ...
        CellGrid::new_internal(
            Axes::with_x(&[TypstSizing::Auto; COLS]),
            if gutters {
                Axes::new(
                    &[TypstSizing::Auto; COLS - 1],
                    &[TypstSizing::Auto; ROWS - 1],
                )
            } else {
                Axes::default()
            },
    ...
    fn test_vline_splitting_without_gutter() {
        ...
        let rows = &[
            RowPiece {
                height: TypstAbsLength::pt(1.0),
                y: 0,
            },
            RowPiece {
                height: TypstAbsLength::pt(2.0),
                y: 1,
            },
            RowPiece {
                height: TypstAbsLength::pt(4.0),
                y: 2,
            },
            RowPiece {
                height: TypstAbsLength::pt(8.0),
                y: 3,
            },
            RowPiece {
                height: TypstAbsLength::pt(16.0),
                y: 4,
            },
            RowPiece {
                height: TypstAbsLength::pt(32.0),
                y: 5,
            },
        ];
        let expected_vline_splits = &[
            vec![(
                TypstAbsLength::pt(0.),
                TypstAbsLength::pt(1. + 2. + 4. + 8. + 16. + 32.),
            )],
            vec![(
                TypstAbsLength::pt(0.),
                TypstAbsLength::pt(1. + 2. + 4. + 8. + 16. + 32.),
            ...
            vec![
                (TypstAbsLength::pt(0.), TypstAbsLength::pt(1.)),
                (TypstAbsLength::pt(1. + 2.), TypstAbsLength::pt(4.)),
                (
                    TypstAbsLength::pt(1. + 2. + 4. + 8. + 16.),
                    TypstAbsLength::pt(32.),
                ),
            ],
    ...
    fn test_vline_splitting_with_gutter() {
        ...
        let rows = &[
            ...
            RowPiece {
                height: TypstAbsLength::pt(64.0),
                y: 6,
            },
            RowPiece {
                height: TypstAbsLength::pt(128.0),
                y: 7,
            },
            RowPiece {
                height: TypstAbsLength::pt(256.0),
                y: 8,
            },
            RowPiece {
                height: TypstAbsLength::pt(512.0),
                y: 9,
            },
            RowPiece {
                height: TypstAbsLength::pt(1024.0),
                y: 10,
            },
        ];
        let expected_vline_splits = &[
            ...
            vec![
                (TypstAbsLength::pt(0.), TypstAbsLength::pt(1. + 2.)),
                (
                    TypstAbsLength::pt(1. + 2. + 4.),
                    TypstAbsLength::pt(8. + 16. + 32.),
                ),
                (
                    TypstAbsLength::pt(1. + 2. + 4. + 8. + 16. + 32. + 64. + 128. + 256.),
                    TypstAbsLength::pt(512. + 1024.),
                ),
            ],
            vec![
                (TypstAbsLength::pt(0.), TypstAbsLength::pt(1. + 2.)),
            ...
            vec![
                (TypstAbsLength::pt(1.), TypstAbsLength::pt(2.)),
                (TypstAbsLength::pt(1. + 2. + 4.), TypstAbsLength::pt(8.)),
                (
                    TypstAbsLength::pt(1. + 2. + 4. + 8. + 16.),
                    TypstAbsLength::pt(32.),
                ),
