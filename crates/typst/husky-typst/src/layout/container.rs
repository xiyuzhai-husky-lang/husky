...
impl TypstContentRefined<BoxTypstElem> {
    #[husky_typst_macros::time(name = "box", span = self.span())]
    pub fn layout(
        &self,
        ...
        let width = match self.width(styles) {
            ...
            TypstSizing::Fr(_) => Smart::Custom(Ratio::one().into()),
        };
...
pub struct BlockElem {
    ...
    /// ```
    #[external]
    #[default(TypstEmLength::new(1.2).into())]
    pub spacing: Spacing,
    ...
    #[default(VElem::block_spacing(TypstEmLength::new(1.2).into()))]
    pub above: VElem,
...
impl LayoutMultiple for TypstContentRefined<BlockElem> {
    #[husky_typst_macros::time(name = "block", span = self.span())]
    fn layout(
        &self,
        engine: &mut TypstEngine,
        styles: StyleChain,
        ...
        let mut frames = if self.breakable(styles) {
            ...
            if sizing.y.is_custom() {
                ...
                for region in regions.iter() {
                    ...
                    remaining -= limited;
                    if TypstAbsLength::zero().fits(remaining) {
                        break;
                    }
                }
        ...
        Ok(TypstLayoutFragment::frames(frames))
    }
}

/// Defines how to size a grid cell along an axis.
#[derive(Debug, Copy, Clone, Eq, PartialEq, Hash)]
pub enum TypstSizing {
...
impl Default for TypstSizing {
    fn default() -> Self {
        Self::Auto
    }
}

impl<T: Into<Spacing>> From<T> for TypstSizing {
