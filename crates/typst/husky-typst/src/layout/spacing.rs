...
impl HElem {
    ...
    pub fn hole() -> Self {
        Self::new(TypstAbsLength::zero().into()).with_weak(true)
    }
}

impl Behave for TypstContentRefined<HElem> {
...
impl Behave for TypstContentRefined<VElem> {
    fn behaviour(&self) -> Behaviour {
        if self.amount().is_fractional() {
            Behaviour::Destructive
...
impl From<TypstAbsLength> for Spacing {
    fn from(abs: TypstAbsLength) -> Self {
        Self::Rel(abs.into())
    }
}

impl From<TypstEmLength> for Spacing {
...
impl From<TypstFraction> for Spacing {
    fn from(fr: TypstFraction) -> Self {
        Self::Fr(fr)
    }
}
