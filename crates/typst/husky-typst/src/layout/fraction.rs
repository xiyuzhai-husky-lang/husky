...
pub struct TypstFraction(Scalar);
...
impl TypstFraction {
    ...
    pub fn share(self, total: Self, remaining: TypstAbsLength) -> TypstAbsLength {
        let ratio = self / total;
        if ratio.is_finite() && remaining.is_finite() {
            (ratio * remaining).max(TypstAbsLength::zero())
        } else {
            TypstAbsLength::zero()
        }
    }
...
sub_impl!(TypstFraction - TypstFraction -> TypstFraction);

impl Mul<f64> for TypstFraction {
    type Output = Self;
...
impl Mul<TypstFraction> for f64 {
    type Output = TypstFraction;

    fn mul(self, other: TypstFraction) -> TypstFraction {
        other * self
    }
}

impl Div for TypstFraction {
    type Output = f64;
...
impl Div<f64> for TypstFraction {
    type Output = Self;
...
assign_impl!(TypstFraction += TypstFraction);
assign_impl!(TypstFraction -= TypstFraction);
assign_impl!(TypstFraction *= f64);
assign_impl!(TypstFraction /= f64);

impl Sum for TypstFraction {
