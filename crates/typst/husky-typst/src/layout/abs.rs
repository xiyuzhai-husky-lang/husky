...
pub struct TypstAbsLength(Scalar);
...
sub_impl!(TypstAbsLength - TypstAbsLength -> TypstAbsLength);

impl Mul<f64> for TypstAbsLength {
    type Output = Self;
...
impl Mul<TypstAbsLength> for f64 {
    type Output = TypstAbsLength;

    fn mul(self, other: TypstAbsLength) -> TypstAbsLength {
        other * self
    }
}

impl Div<f64> for TypstAbsLength {
    type Output = Self;
...
assign_impl!(TypstAbsLength += TypstAbsLength);
assign_impl!(TypstAbsLength -= TypstAbsLength);
assign_impl!(TypstAbsLength *= f64);
assign_impl!(TypstAbsLength /= f64);

impl Rem for TypstAbsLength {
    type Output = Self;
...
impl Sum for TypstAbsLength {
    fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
        Self(iter.map(|s| s.0).sum())
    }
}

impl<'a> Sum<&'a Self> for TypstAbsLength {
...
mod tests {
    ...
    fn test_length_unit_conversion() {
        assert!((TypstAbsLength::mm(150.0).to_cm() - 15.0) < 1e-4);
    }
}
