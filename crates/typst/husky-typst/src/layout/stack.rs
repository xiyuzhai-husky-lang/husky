...
impl LayoutMultiple for TypstContentRefined<StackElem> {
    #[husky_typst_macros::time(name = "stack", span = self.span())]
    fn layout(
        &self,
...
enum StackItem {
    ...
    Frame(TypstFrame, Axes<FixedAlignment>),
}
...
impl<'a> StackLayouter<'a> {
    ...
    fn finish_region(&mut self) {
        ...
        let mut output = TypstFrame::hard(size);
        ...
        self.used = Gen::zero();
        self.fr = TypstFraction::zero();
        self.finished.push(output);
    }

    /// Finish layouting and return the resulting frames.
    fn finish(mut self) -> TypstLayoutFragment {
        self.finish_region();
        TypstLayoutFragment::frames(self.finished)
    }
}
...
impl Gen<TypstAbsLength> {
    /// The zero value.
    fn zero() -> Self {
        Self {
            cross: TypstAbsLength::zero(),
            main: TypstAbsLength::zero(),
        }
    }
