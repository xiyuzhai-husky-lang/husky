...
impl LayoutMultiple for TypstContentRefined<ColumnsElem> {
    #[husky_typst_macros::time(name = "columns", span = self.span())]
    fn layout(
        &self,
        ...
        for region in regions.iter().take(total_regions) {
            ...
            let mut output = TypstFrame::hard(Size::new(regions.size.x, height));
            let mut cursor = TypstAbsLength::zero();
