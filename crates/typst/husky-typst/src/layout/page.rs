...
pub struct PageElem {
    ...
    pub number_align: TypstAlignment,
...
impl TypstContentRefined<PageElem> {
    ...
    /// page run.
    #[husky_typst_macros::time(name = "page", span = self.span())]
    pub fn layout(
        &self,
        ...
        let height = self.height(styles).unwrap_or(TypstAbsLength::inf());
        ...
        if extend_to.is_some_and(|p| !p.matches(page_counter.physical().get() + frames.len())) {
            ...
            let size = area
                .map(TypstAbsLength::is_finite)
                .select(area, Size::zero());
            frames.push(TypstFrame::hard(size));
        }
...
cast! {
    Binding,
    self => match self {
        Self::Left => TypstAlignment::LEFT.into_value(),
        Self::Right => TypstAlignment::RIGHT.into_value(),
    },
    v: TypstAlignment => match v {
        TypstAlignment::LEFT => Self::Left,
        TypstAlignment::RIGHT => Self::Right,
...
impl Paper {
    ...
    pub fn height(self) -> TypstAbsLength {
        TypstAbsLength::mm(self.height.get())
    }
}
