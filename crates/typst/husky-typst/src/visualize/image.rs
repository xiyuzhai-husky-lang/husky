...
impl TypstLayoutSingle for TypstContentRefined<ImageElem> {
    ...
    fn layout(
        ...
        let image = TypstImage::with_fonts(
            data.clone().into(),
            format,
            self.alt(styles),
            engine.world,
        ...
        let target = if expand.x && expand.y {
            ...
            Size::new(TypstAbsLength::pt(pxw), TypstAbsLength::pt(pxh))
        };
...
impl LocalName for TypstContentRefined<ImageElem> {
...
impl Figurable for TypstContentRefined<ImageElem> {}
...
pub struct TypstImage(Arc<Prehashed<Repr>>);
...
impl TypstImage {
    ...
    #[comemo::memoize]
    #[husky_typst_macros::time(name = "load image")]
    pub fn new(data: Bytes, format: ImageFormat, alt: Option<EcoString>) -> StrResult<TypstImage> {
