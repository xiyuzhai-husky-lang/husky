...
impl LayoutMultiple for TypstContentRefined<PathElem> {
    #[husky_typst_macros::time(name = "path", span = self.span())]
    fn layout(
        &self,
        _: &mut TypstEngine,
        styles: StyleChain,
        ...
        if points.is_empty() {
            return Ok(TypstLayoutFragment::frame(TypstFrame::soft(size)));
        }
        ...
        let mut add_cubic =
            |from_point: Point, to_point: Point, from: PathVertex, to: PathVertex| {
                ...
                let p1 =
                    kurbo::Point::new(from_control_point.x.to_raw(), from_control_point.y.to_raw());
                let p2 =
                ...
                size.x.set_max(TypstAbsLength::raw(extrema.x1));
                size.y.set_max(TypstAbsLength::raw(extrema.y1));
            };
        ...
        for item in self.0.iter() {
            match item {
                ...
                PathItem::CubicTo(c0, c1, end) => {
                    ...
                    min_x = min_x
                        .min(TypstAbsLength::pt(bbox.x0))
                        .min(TypstAbsLength::pt(bbox.x1));
                    min_y = min_y
                        .min(TypstAbsLength::pt(bbox.y0))
                        .min(TypstAbsLength::pt(bbox.y1));
                    max_x = max_x
                        .max(TypstAbsLength::pt(bbox.x0))
                        .max(TypstAbsLength::pt(bbox.x1));
                    max_y = max_y
                        .max(TypstAbsLength::pt(bbox.y0))
                        .max(TypstAbsLength::pt(bbox.y1));
                    cursor = *end;
                }
                PathItem::ClosePath => (),
            }
