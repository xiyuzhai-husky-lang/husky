...
impl TypstStroke<TypstAbsLength> {
        ...
        if cap.is_auto() && join.is_auto() && dash.is_auto() && miter_limit.is_auto() {
            ...
            if let Smart::Custom(dash) = &dash {
                ...
                if let Some(dash) = dash {
                    ...
                    r.push_str(&NoneTypstValue.repr());
                }
                sep = ", ";
            }
...
// Same names as tikz:
// https://typst.stackexchange.com/questions/45275/tikz-get-values-for-predefined-dash-patterns
cast! {
    DashPattern,
    self => dict! { "array" => self.array, "phase" => self.phase }.into_value(),

    "solid" => Vec::new().into(),
    "dotted" => vec![DashLength::LineWidth, TypstAbsLength::pt(2.0).into()].into(),
    "densely-dotted" => vec![DashLength::LineWidth, TypstAbsLength::pt(1.0).into()].into(),
    "loosely-dotted" => vec![DashLength::LineWidth, TypstAbsLength::pt(4.0).into()].into(),
    "dashed" => vec![TypstAbsLength::pt(3.0).into(), TypstAbsLength::pt(3.0).into()].into(),
    "densely-dashed" => vec![TypstAbsLength::pt(3.0).into(), TypstAbsLength::pt(2.0).into()].into(),
    "loosely-dashed" => vec![TypstAbsLength::pt(3.0).into(), TypstAbsLength::pt(6.0).into()].into(),
    "dash-dotted" => vec![TypstAbsLength::pt(3.0).into(), TypstAbsLength::pt(2.0).into(), DashLength::LineWidth, TypstAbsLength::pt(2.0).into()].into(),
...
impl From<TypstAbsLength> for DashLength {
    fn from(l: TypstAbsLength) -> Self {
...
pub struct TypstFixedStroke {
    ...
    /// The stroke's thickness.
    pub thickness: TypstAbsLength,
    /// The stroke's line cap.
    pub cap: LineCap,
    /// The stroke's line join.
    pub join: LineJoin,
    /// The stroke's line dash pattern.
    pub dash: Option<DashPattern<TypstAbsLength, TypstAbsLength>>,
...
impl TypstFixedStroke {
    /// Create a stroke from a paint and a thickness.
    pub fn from_pair(paint: impl Into<TypstPaint>, thickness: TypstAbsLength) -> Self {
        Self {
            paint: paint.into(),
            thickness,
            ..Default::default()
...
impl Default for TypstFixedStroke {
    fn default() -> Self {
        Self {
            ...
            thickness: TypstAbsLength::pt(1.0),
            cap: LineCap::Butt,
            join: LineJoin::Miter,
            dash: None,
