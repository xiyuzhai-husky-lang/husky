...
impl TypstColor {
    ...
    pub const MAP: fn() -> TypstModuleEvaluation = || {
        // Lazy to avoid re-allocating.
        static MODULE: Lazy<TypstModuleEvaluation> = Lazy::new(map);
        MODULE.clone()
    };
...
fn map() -> TypstModuleEvaluation {
    ...
    TypstModuleEvaluation::new("map", scope)
}
