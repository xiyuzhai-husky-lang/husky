...
pub use self::variant::{FontStretch, FontVariant, FontWeight, TypstFontStyle};
...
impl TypstFont {
    ...
    pub fn to_em(&self, units: impl Into<f64>) -> TypstEmLength {
        TypstEmLength::from_units(units, self.units_per_em())
    }

    /// Look up the horizontal advance width of a glyph.
    pub fn advance(&self, glyph: u16) -> Option<TypstEmLength> {
        self.0
            .ttf
...
pub struct FontMetrics {
    ...
    pub ascender: TypstEmLength,
    /// The approximate height of uppercase letters.
    pub cap_height: TypstEmLength,
    /// The approximate height of non-ascending lowercase letters.
    pub x_height: TypstEmLength,
    /// The distance from the baseline to the typographic descender.
    pub descender: TypstEmLength,
...
impl FontMetrics {
    ...
    pub fn from_ttf(ttf: &ttf_parser::Face) -> Self {
        ...
        let to_em = |units| TypstEmLength::from_units(units, units_per_em);
        ...
        let strikethrough = LineMetrics {
            position: strikeout.map_or(TypstEmLength::new(0.25), |s| to_em(s.position)),
            thickness: strikeout
                .or(underline)
                .map_or(TypstEmLength::new(0.06), |s| to_em(s.thickness)),
        };

        let underline = LineMetrics {
            position: underline.map_or(TypstEmLength::new(-0.2), |s| to_em(s.position)),
            thickness: underline
                .or(strikeout)
                .map_or(TypstEmLength::new(0.06), |s| to_em(s.thickness)),
        };
        ...
        let overline = LineMetrics {
            position: cap_height + TypstEmLength::new(0.1),
    ...
    pub fn vertical(&self, metric: VerticalFontMetric) -> TypstEmLength {
        match metric {
            ...
            VerticalFontMetric::Baseline => TypstEmLength::zero(),
...
pub struct LineMetrics {
    ...
    pub position: TypstEmLength,
    /// The thickness of the line.
    pub thickness: TypstEmLength,
}
