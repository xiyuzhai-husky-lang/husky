...
enum DecoLine {
    Underline {
        stroke: TypstStroke<TypstAbsLength>,
        offset: Smart<TypstAbsLength>,
        evade: bool,
        background: bool,
    },
    Strikethrough {
    ...
    let mut push_segment = |from: TypstAbsLength, to: TypstAbsLength, prepend: bool| {
        ...
        if target.x >= min_width || !evade {
            ...
            if prepend {
                frame.prepend(
                    origin,
                    TypstFrameItem::Shape(shape, TypstSynSpan::detached()),
                );
            } else {
                frame.push(
                    origin,
        ...
        if intersect {
            ...
            intersections.extend(
                path.segments()
                    ...
                    .map(|is| TypstAbsLength::raw(line.eval(is.line_t).x)),
            );
        }
    }
...
impl BezPathBuilder {
    fn new(units_per_em: f64, font_size: TypstAbsLength, x_offset: f64) -> Self {
        Self {
            path: BezPath::new(),
            units_per_em,
            font_size,
    ...
    fn s(&self, v: f32) -> f64 {
        TypstEmLength::from_units(v, self.units_per_em)
            .at(self.font_size)
            .to_raw()
    }
}
