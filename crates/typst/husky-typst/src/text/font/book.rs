...
use crate::text::{FontStretch, FontVariant, FontWeight, TypstFont, TypstFontStyle};
        ...
        let variant = {
            let style = exception.and_then(|c| c.style).unwrap_or_else(|| {
                ...
                match (italic, oblique) {
                    (false, false) => TypstFontStyle::Normal,
                    (true, _) => TypstFontStyle::Italic,
                    (_, true) => TypstFontStyle::Oblique,
                }
            });
