...
impl<T: IsTypstElem + Reflect> Reflect for TypstContentRefined<T> {
    fn input() -> CastInfo {
        T::input()
    }
...
impl<T: IsTypstElem + IntoTypstValue> IntoTypstValue for TypstContentRefined<T> {
    fn into_value(self) -> TypstValue {
...
impl<T: IsTypstElem + FromTypstValue> FromTypstValue for TypstContentRefined<T> {
    fn from_value(mut value: TypstValue) -> StrResult<Self> {
        ...
        Ok(TypstContentRefined::new(val))
    }
}
