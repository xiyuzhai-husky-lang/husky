...
impl Tracer {
    ...
    pub fn inspect(&mut self, span: TypstSynSpan) {
...
impl Tracer {
    ...
    pub fn inspected(&self, id: FileId) -> Option<TypstSynSpan> {
        if self.inspected.and_then(TypstSynSpan::id) == Some(id) {
            self.inspected
        } else {
            None
        }
