...
pub(crate) fn call_closure(
    ...
    let mut scopes = TypstValueAssignmentGroups::new(None);
...
pub struct CapturesVisitor<'a> {
    external: Option<&'a TypstValueAssignmentGroups<'a>>,
    internal: TypstValueAssignmentGroups<'a>,
    captures: TypstValueAssignmentGroup,
}

impl<'a> CapturesVisitor<'a> {
    /// Create a new visitor for the given external scopes.
    pub fn new(external: Option<&'a TypstValueAssignmentGroups<'a>>) -> Self {
        Self {
            external,
            internal: TypstValueAssignmentGroups::new(None),
            captures: TypstValueAssignmentGroup::new(),
        }
    }

    /// Return the scope of captured variables.
    pub fn finish(self) -> TypstValueAssignmentGroup {
        self.captures
    }

    /// Visit any node and collect all captured variables.
    pub fn visit(&mut self, node: &TypstSyntaxNode) {
        match node.cast() {
            // Every identifier is a potential variable that we need to capture.
            // Identifiers that shouldn't count as captures because they
            // actually bind a new name are handled below (individually through
            // the expressions that contain them).
            Some(ast::Expr::Ident(ident)) => self.capture(&ident, TypstValueAssignmentGroups::get),
            Some(ast::Expr::MathIdent(ident)) => {
                self.capture(&ident, TypstValueAssignmentGroups::get_in_math)
            }
    ...
    fn capture(
        &mut self,
        ident: &str,
        getter: impl FnOnce(&'a TypstValueAssignmentGroups<'a>, &str) -> HintedStrResult<&'a TypstValue>,
    ) {
