...
#[track_caller]
fn category_page(resolver: &dyn Resolver, category: TypstDefnKind) -> PageModel {
    ...
    let (module, path): (&TypstModuleEvaluation, &[&str]) = if category == MATH {
        (&LIBRARY.math, &["math"])
    } else {
        (&LIBRARY.global, &[])
    };
...
fn param_model(resolver: &dyn Resolver, info: &ParamInfo) -> ParamModel {
    ...
    ParamModel {
        ...
        default: info.default.map(|default| {
            let node = husky_typst::syntax::parse_code(&default().repr());
            Html::new(husky_typst::syntax::highlight_html(&node))
        }),
    ...
    for (name, value) in group.module().scope().iter() {
        ...
        for (variant, c) in symbol.variants() {
            ...
            list.push(SymbolModel {
                ...
                markup_shorthand: shorthand(husky_typst::syntax::ast::Shorthand::MARKUP_LIST),
                math_shorthand: shorthand(husky_typst::syntax::ast::Shorthand::MATH_LIST),
                codepoint: c as u32,
                accent: husky_typst::symbols::Symbol::combining_accent(c).is_some(),
...
#[track_caller]
fn get_module<'a>(
    parent: &'a TypstModuleEvaluation,
    name: &str,
) -> StrResult<&'a TypstModuleEvaluation> {
...
impl GroupData {
    fn module(&self) -> &'static TypstModuleEvaluation {
