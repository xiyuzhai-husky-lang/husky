...
impl<'a> Handler<'a> {
        ...
        match event {
            ...
            md::Event::Code(code) => {
                ...
                let parser = match (chars.next(), chars.next_back()) {
                    (Some('['), Some(']')) => husky_typst::syntax::parse,
                    (Some('{'), Some('}')) => husky_typst::syntax::parse_code,
                    _ => return true,
                };

                let root = parser(&code[1..code.len() - 1]);
                let html = husky_typst::syntax::highlight_html(&root);
            ...
            md::Event::End(md::Tag::CodeBlock(md::CodeBlockKind::Fenced(_))) => {
                let Some(lang) = self.lang.take() else {
                    return false;
                };
    ...
    fn handle_heading(&mut self, id_slot: &mut Option<&'a str>, level: &mut md::HeadingLevel) {
        nest_heading(level, self.nesting());
...
fn code_block(resolver: &dyn Resolver, lang: &str, text: &str) -> Html {
    ...
    let mut zoom: Option<[TypstAbsLength; 4]> = None;
    ...
    if let Some(args) = parts.next() {
        ...
        if !args.contains("single") {
            zoom = args
                .split(',')
                .take(4)
                .map(|s| TypstAbsLength::pt(s.parse().unwrap()))
                .collect::<Vec<_>>()
                .try_into()
                .ok();
        }
    ...
    if lang.is_empty() {
        ...
        let set = &*husky_typst::text::RAW_SYNTAXES;
    ...
    if matches!(lang, "example" | "typ") {
        let root = husky_typst::syntax::parse(&display);
        let html = Html::new(husky_typst::syntax::highlight_html(&root));
        if lang == "typ" {
    ...
    let mut document = match husky_typst::compile(&world, &mut tracer) {
        Ok(doc) => doc,
        Err(err) => {
    ...
    let hash = husky_typst::util::hash128(text);
...
impl IsTypstWorld for DocWorld {
