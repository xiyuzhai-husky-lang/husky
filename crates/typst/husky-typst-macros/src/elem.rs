...
fn create_style_chain_access(field: &Field, borrowed: bool, inherent: TokenStream) -> TokenStream {
    ...
    quote! {
        styles.#getter::<#ty>(
            <Self as #foundations::IsTypstElem>::elem(),
            Fields::#enum_ident as u8,
            #inherent,
            #default,
        )
...
/// Creates the element's `IsTypstElem` implementation.
fn create_native_elem_impl(element: &Elem) -> TokenStream {
    ...
    let local_name = if element.can("LocalName") {
        quote! { Some(<#foundations::TypstContentRefined<#ident> as ::husky_typst::text::LocalName>::local_name) }
    } else {
        quote! { None }
    };
    ...
    let scope = if *scope {
        ...
        quote! { #foundations::TypstValueAssignmentGroup::new() }
    };
    ...
    quote! {
        impl #foundations::IsTypstElem for #ident {
    ...
    let checks = relevant.map(|capability| {
        quote! {
            if capability == ::std::any::TypeId::of::<dyn #capability>() {
                ...
                return Some(unsafe {
                    ::husky_typst::util::fat::vtable(dangling as *const dyn #capability)
                });
            }
        }
    });
    ...
    quote! {
        unsafe impl #foundations::Capable for #ident {
            fn vtable(capability: ::std::any::TypeId) -> ::std::option::Option<*const ()> {
                let dangling = ::std::ptr::NonNull::<#foundations::TypstContentRefined<#ident>>::dangling().as_ptr();
                #(#checks)*
                None
            }
        }
