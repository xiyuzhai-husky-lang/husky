...
/// Whether an attribute list has a specified attribute.
pub fn take_attr(attrs: &mut Vec<syn::Attribute>, target: &str) -> Option<syn::Attribute> {
    attrs
        .iter()
...
pub fn parse_string<K: Token + Default + Parse>(input: ParseStream) -> Result<Option<String>> {
    Ok(parse_key_value::<K, syn::LitStr>(input)?.map(|s| s.value()))
}

/// Parse a metadata key-string pair, separated by `=`.
pub fn parse_string_array<K: Token + Default + Parse>(input: ParseStream) -> Result<Vec<String>> {
...
/// Shorthand for `::husky_typst::foundations`.
#[allow(non_camel_case_types)]
pub struct foundations;

impl quote::ToTokens for foundations {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        quote! { ::husky_typst::foundations }.to_tokens(tokens);
    }
}
