...
pub fn update(command: &UpdateCommand) -> StrResult<()> {
    ...
    let current_exe = env::current_exe()
        .map_err(|err| eco_format!("failed to locate path of the running executable ({err})"))?;
...
fn extract_binary_from_zip(data: &[u8], asset_name: &str) -> StrResult<Vec<u8>> {
    ...
    file.read_to_end(&mut buffer)
        .map_err(|err| eco_format!("failed to read binary data from ZIP archive ({err})"))?;
...
fn extract_binary_from_tar_xz(data: &[u8]) -> StrResult<Vec<u8>> {
    ...
    let mut buffer = vec![];
    file.read_to_end(&mut buffer)
        .map_err(|err| eco_format!("failed to read binary data from tar.xz archive ({err})"))?;
...
fn backup_path() -> StrResult<PathBuf> {
    ...
    let root_backup_dir = dirs::data_dir().ok_or("unable to locate local data directory")?;
    ...
    Ok(backup_dir.join("tex_backup.part"))
}
