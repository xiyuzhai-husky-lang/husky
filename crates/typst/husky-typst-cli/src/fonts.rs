...
use husky_typst::text::{FontInfo, FontVariant, TexFont, TexFontBook};
...
pub struct FontSlot {
    ...
    font: OnceLock<Option<TexFont>>,
}

impl FontSlot {
    /// Get the font for this slot.
    pub fn get(&self) -> Option<TexFont> {
        self.font
            .get_or_init(|| {
                let _scope = TimingScope::new("load font", None);
                let data = fs::read(&self.path).ok()?.into();
                TexFont::new(data, self.index)
            })
            .clone()
    }
}
...
impl FontSearcher {
    ...
    fn add_embedded(&mut self) {
        let mut process = |bytes: &'static [u8]| {
            ...
            for (i, font) in TexFont::iter(buffer).enumerate() {
