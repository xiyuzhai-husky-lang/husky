...
use husky_typst::diag::{bail, At, Severity, SourceDiagnostic, StrResult};
...
use husky_typst::syntax::{FileId, Source, TypstSynSpan};
use husky_typst::visualize::TypstColor;
use husky_typst::{IsTypstWorld, WorldExt};
...
impl CompileCommand {
    ...
    pub fn output(&self) -> PathBuf {
        self.output.clone().unwrap_or_else(|| {
            ...
            path.with_extension(match self.output_format().unwrap_or(OutputFormat::Pdf) {
...
#[husky_typst_macros::time(name = "compile once")]
pub fn compile_once(
    ...
    if let Err(errors) = world.source(world.main()).at(TypstSynSpan::detached()) {
        set_failed();
        if watching {
    ...
    let result = husky_typst::compile(world, &mut tracer);
        ...
        .map(|(i, page)| {
            ...
            match fmt {
                ...
                ImageExportFormat::Svg => {
                    let svg = husky_typst_svg::svg(&page.frame);
...
impl ExportCache {
    ...
    pub fn is_cached(&self, i: usize, frame: &TypstFrame) -> bool {
        let hash = husky_typst::util::hash128(frame);
...
fn label(world: &SystemWorld, span: TypstSynSpan) -> Option<Label<FileId>> {
