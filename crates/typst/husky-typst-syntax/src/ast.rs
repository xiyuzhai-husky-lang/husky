...
//!
//! The AST is rooted in the [`TypstMarkup`] node.
...
use crate::{
    is_id_continue, is_id_start, is_newline, split_newlines, TypstSynSpan, TypstSyntaxKind,
    TypstSyntaxNode,
};

/// A typed AST node.
pub trait TypstAstNode<'a>: Sized {
    /// Convert a node into its typed variant.
    fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self>;

    /// A reference to the underlying syntax node.
    fn to_untyped(self) -> &'a TypstSyntaxNode;
...
/// functionality, it is.
static ARBITRARY: TypstSyntaxNode = TypstSyntaxNode::arbitrary();
...
macro_rules! node {
    ($(#[$attr:meta])* $name:ident) => {
        ...
        #[repr(transparent)]
        $(#[$attr])*
        pub struct $name<'a>(&'a TypstSyntaxNode);

        impl<'a> TypstAstNode<'a> for $name<'a> {
            #[inline]
            fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self> {
                if node.kind() == TypstSyntaxKind::$name {
                    Some(Self(node))
                } else {
                    Option::None
                }
    ...
    pub fn exprs(self) -> impl DoubleEndedIterator<Item = Expr<'a>> {
        ...
        self.0
            ...
            .filter(move |node| {
                ...
                let kind = node.kind();
                let keep = !was_stmt || node.kind() != TypstSyntaxKind::Space;
...
impl<'a> Expr<'a> {
    fn cast_with_space(node: &'a TypstSyntaxNode) -> Option<Self> {
        match node.kind() {
            TypstSyntaxKind::Space => node.cast().map(Self::Space),
            _ => Self::from_untyped(node),
        }
    }
}

impl<'a> TypstAstNode<'a> for Expr<'a> {
    fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self> {
        match node.kind() {
            TypstSyntaxKind::Linebreak => node.cast().map(Self::Linebreak),
            TypstSyntaxKind::Parbreak => node.cast().map(Self::Parbreak),
            TypstSyntaxKind::Text => node.cast().map(Self::Text),
            TypstSyntaxKind::Escape => node.cast().map(Self::Escape),
            TypstSyntaxKind::Shorthand => node.cast().map(Self::Shorthand),
            TypstSyntaxKind::SmartQuote => node.cast().map(Self::SmartQuote),
            TypstSyntaxKind::Strong => node.cast().map(Self::Strong),
            TypstSyntaxKind::Emph => node.cast().map(Self::Emph),
            TypstSyntaxKind::Raw => node.cast().map(Self::Raw),
            TypstSyntaxKind::Link => node.cast().map(Self::Link),
            TypstSyntaxKind::Label => node.cast().map(Self::Label),
            TypstSyntaxKind::Ref => node.cast().map(Self::Ref),
            TypstSyntaxKind::Heading => node.cast().map(Self::Heading),
            TypstSyntaxKind::ListItem => node.cast().map(Self::List),
            TypstSyntaxKind::EnumItem => node.cast().map(Self::Enum),
            TypstSyntaxKind::TermItem => node.cast().map(Self::Term),
            TypstSyntaxKind::Equation => node.cast().map(Self::Equation),
            TypstSyntaxKind::Math => node.cast().map(Self::Math),
            TypstSyntaxKind::MathIdent => node.cast().map(Self::MathIdent),
            TypstSyntaxKind::MathAlignPoint => node.cast().map(Self::MathAlignPoint),
            TypstSyntaxKind::MathDelimited => node.cast().map(Self::MathDelimited),
            TypstSyntaxKind::MathAttach => node.cast().map(Self::MathAttach),
            TypstSyntaxKind::MathPrimes => node.cast().map(Self::MathPrimes),
            TypstSyntaxKind::MathFrac => node.cast().map(Self::MathFrac),
            TypstSyntaxKind::MathRoot => node.cast().map(Self::MathRoot),
            TypstSyntaxKind::Ident => node.cast().map(Self::Ident),
            TypstSyntaxKind::None => node.cast().map(Self::None),
            TypstSyntaxKind::Auto => node.cast().map(Self::Auto),
            TypstSyntaxKind::Bool => node.cast().map(Self::Bool),
            TypstSyntaxKind::Int => node.cast().map(Self::Int),
            TypstSyntaxKind::Float => node.cast().map(Self::Float),
            TypstSyntaxKind::Numeric => node.cast().map(Self::Numeric),
            TypstSyntaxKind::Str => node.cast().map(Self::Str),
            TypstSyntaxKind::CodeBlock => node.cast().map(Self::Code),
            TypstSyntaxKind::ContentBlock => node.cast().map(Self::Content),
            TypstSyntaxKind::Parenthesized => node.cast().map(Self::Parenthesized),
            TypstSyntaxKind::Array => node.cast().map(Self::Array),
            TypstSyntaxKind::Dict => node.cast().map(Self::Dict),
            TypstSyntaxKind::Unary => node.cast().map(Self::Unary),
            TypstSyntaxKind::Binary => node.cast().map(Self::Binary),
            TypstSyntaxKind::FieldAccess => node.cast().map(Self::FieldAccess),
            TypstSyntaxKind::FuncCall => node.cast().map(Self::FuncCall),
            TypstSyntaxKind::Closure => node.cast().map(Self::Closure),
            TypstSyntaxKind::LetBinding => node.cast().map(Self::Let),
            TypstSyntaxKind::DestructAssignment => node.cast().map(Self::DestructAssign),
            TypstSyntaxKind::SetRule => node.cast().map(Self::Set),
            TypstSyntaxKind::ShowRule => node.cast().map(Self::Show),
            TypstSyntaxKind::Conditional => node.cast().map(Self::Conditional),
            TypstSyntaxKind::WhileLoop => node.cast().map(Self::While),
            TypstSyntaxKind::ForLoop => node.cast().map(Self::For),
            TypstSyntaxKind::ModuleImport => node.cast().map(Self::Import),
            TypstSyntaxKind::ModuleInclude => node.cast().map(Self::Include),
            TypstSyntaxKind::LoopBreak => node.cast().map(Self::Break),
            TypstSyntaxKind::LoopContinue => node.cast().map(Self::Continue),
            TypstSyntaxKind::FuncReturn => node.cast().map(Self::Return),
            _ => Option::None,
        }
    }
...
impl<'a> Strong<'a> {
    ...
    pub fn body(self) -> TypstMarkup<'a> {
...
impl<'a> Ref<'a> {
    ...
    pub fn target(self) -> &'a str {
        self.0
            .children()
            .find(|node| node.kind() == TypstSyntaxKind::RefMarker)
...
impl<'a> Heading<'a> {
    ...
    pub fn level(self) -> NonZeroUsize {
        self.0
            .children()
            .find(|node| node.kind() == TypstSyntaxKind::HeadingMarker)
...
impl<'a> EnumItem<'a> {
    ...
    pub fn number(self) -> Option<usize> {
        self.0.children().find_map(|node| match node.kind() {
            TypstSyntaxKind::EnumMarker => node.text().trim_end_matches('.').parse().ok(),
            _ => Option::None,
        })
    }
...
impl<'a> TermItem<'a> {
    ...
    pub fn term(self) -> TypstMarkup<'a> {
        self.0.cast_first_match().unwrap_or_default()
    }

    /// The description of the term.
    pub fn description(self) -> TypstMarkup<'a> {
...
impl<'a> Equation<'a> {
    ...
    pub fn block(self) -> bool {
        let is_space = |node: Option<&TypstSyntaxNode>| {
            node.map(TypstSyntaxNode::kind) == Some(TypstSyntaxKind::Space)
        };
...
impl<'a> MathAttach<'a> {
    ...
    pub fn bottom(self) -> Option<Expr<'a>> {
        self.0
            .children()
            .skip_while(|node| !matches!(node.kind(), TypstSyntaxKind::Underscore))
            .find_map(TypstSyntaxNode::cast)
    }
    ...
    pub fn top(self) -> Option<Expr<'a>> {
        self.0
            .children()
            .skip_while(|node| !matches!(node.kind(), TypstSyntaxKind::Hat))
            .find_map(TypstSyntaxNode::cast)
    }
...
impl MathPrimes<'_> {
    pub fn count(self) -> usize {
        self.0
            .children()
            .filter(|node| matches!(node.kind(), TypstSyntaxKind::Prime))
            .count()
    }
}
...
impl<'a> Code<'a> {
    ...
    pub fn exprs(self) -> impl DoubleEndedIterator<Item = Expr<'a>> {
        self.0.children().filter_map(TypstSyntaxNode::cast)
    }
}
...
impl<'a> TypstAstNode<'a> for ArrayItem<'a> {
    fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self> {
        match node.kind() {
            TypstSyntaxKind::Spread => node.cast_first_match().map(Self::Spread),
...
impl<'a> TypstAstNode<'a> for DictItem<'a> {
    fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self> {
        match node.kind() {
            TypstSyntaxKind::Named => node.cast().map(Self::Named),
            TypstSyntaxKind::Keyed => node.cast().map(Self::Keyed),
            TypstSyntaxKind::Spread => node.cast_first_match().map(Self::Spread),
            _ => Option::None,
        }
    }
...
impl UnOp {
    ...
    pub fn from_kind(token: TypstSyntaxKind) -> Option<Self> {
        Some(match token {
            TypstSyntaxKind::Plus => Self::Pos,
            TypstSyntaxKind::Minus => Self::Neg,
            TypstSyntaxKind::Not => Self::Not,
    ...
    pub fn op(self) -> BinOp {
        ...
        self.0
            ...
            .find_map(|node| match node.kind() {
                ...
                TypstSyntaxKind::In if not => Some(BinOp::NotIn),
...
impl BinOp {
    ...
    pub fn from_kind(token: TypstSyntaxKind) -> Option<Self> {
        Some(match token {
            TypstSyntaxKind::Plus => Self::Add,
            TypstSyntaxKind::Minus => Self::Sub,
            TypstSyntaxKind::Star => Self::Mul,
            TypstSyntaxKind::Slash => Self::Div,
            TypstSyntaxKind::And => Self::And,
            TypstSyntaxKind::Or => Self::Or,
            TypstSyntaxKind::EqEq => Self::Eq,
            TypstSyntaxKind::ExclEq => Self::Neq,
            TypstSyntaxKind::Lt => Self::Lt,
            TypstSyntaxKind::LtEq => Self::Leq,
            TypstSyntaxKind::Gt => Self::Gt,
            TypstSyntaxKind::GtEq => Self::Geq,
            TypstSyntaxKind::Eq => Self::Assign,
            TypstSyntaxKind::In => Self::In,
            TypstSyntaxKind::PlusEq => Self::AddAssign,
            TypstSyntaxKind::HyphEq => Self::SubAssign,
            TypstSyntaxKind::StarEq => Self::MulAssign,
            TypstSyntaxKind::SlashEq => Self::DivAssign,
            _ => return Option::None,
        })
    }
...
impl<'a> Args<'a> {
    ...
    pub fn trailing_comma(self) -> bool {
        self.0
            ...
            .is_some_and(|n| n.kind() == TypstSyntaxKind::Comma)
    }
}
...
impl<'a> TypstAstNode<'a> for Arg<'a> {
    fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self> {
        match node.kind() {
            TypstSyntaxKind::Named => node.cast().map(Self::Named),
            TypstSyntaxKind::Spread => node.cast_first_match().map(Self::Spread),
...
impl<'a> TypstAstNode<'a> for Param<'a> {
    fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self> {
        match node.kind() {
            TypstSyntaxKind::Named => node.cast().map(Self::Named),
            TypstSyntaxKind::Spread => node.cast().map(Self::Sink),
            _ => node.cast().map(Self::Pos),
        }
    }
...
impl<'a> TypstAstNode<'a> for DestructuringKind<'a> {
    fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self> {
        match node.kind() {
            TypstSyntaxKind::Named => node.cast().map(Self::Named),
            TypstSyntaxKind::Spread => node.cast().map(Self::Sink),
            TypstSyntaxKind::Underscore => node.cast().map(Self::Placeholder),
...
impl<'a> TypstAstNode<'a> for Pattern<'a> {
    fn from_untyped(node: &'a TypstSyntaxNode) -> Option<Self> {
        match node.kind() {
            TypstSyntaxKind::Destructuring => node.cast().map(Self::Destructuring),
            TypstSyntaxKind::Underscore => node.cast().map(Self::Placeholder),
            _ => node.cast().map(Self::Normal),
        }
    }
    ...
    pub fn init(self) -> Option<Expr<'a>> {
        match self.kind() {
            LetBindingKind::Normal(Pattern::Normal(_)) => {
                self.0.children().filter_map(TypstSyntaxNode::cast).nth(1)
            }
...
impl<'a> SetRule<'a> {
    ...
    pub fn condition(self) -> Option<Expr<'a>> {
        self.0
            .children()
            .skip_while(|child| child.kind() != TypstSyntaxKind::If)
            .find_map(TypstSyntaxNode::cast)
    }
}
...
impl<'a> ShowRule<'a> {
    ...
    pub fn selector(self) -> Option<Expr<'a>> {
        self.0
            .children()
            .rev()
            .skip_while(|child| child.kind() != TypstSyntaxKind::Colon)
            .find_map(TypstSyntaxNode::cast)
    }
...
impl<'a> Conditional<'a> {
    ...
    pub fn if_body(self) -> Expr<'a> {
        self.0
            .children()
            .filter_map(TypstSyntaxNode::cast)
            .nth(1)
            .unwrap_or_default()
    }
    ...
    pub fn else_body(self) -> Option<Expr<'a>> {
        self.0.children().filter_map(TypstSyntaxNode::cast).nth(2)
    }
}
...
impl<'a> ForLoop<'a> {
    ...
    pub fn iterable(self) -> Expr<'a> {
        self.0
            .children()
            .skip_while(|&c| c.kind() != TypstSyntaxKind::In)
            .find_map(TypstSyntaxNode::cast)
            .unwrap_or_default()
    }
...
impl<'a> ModuleImport<'a> {
    ...
    pub fn imports(self) -> Option<Imports<'a>> {
        self.0.children().find_map(|node| match node.kind() {
            TypstSyntaxKind::Star => Some(Imports::Wildcard),
            TypstSyntaxKind::ImportItems => node.cast().map(Imports::Items),
            _ => Option::None,
        })
    }
    ...
    pub fn new_name(self) -> Option<Ident<'a>> {
        self.0
            .children()
            .skip_while(|child| child.kind() != TypstSyntaxKind::As)
            .find_map(TypstSyntaxNode::cast)
    }
}
...
impl<'a> ImportItems<'a> {
    ...
    pub fn iter(self) -> impl DoubleEndedIterator<Item = ImportItem<'a>> {
        self.0.children().filter_map(|child| match child.kind() {
            TypstSyntaxKind::RenamedImportItem => child.cast().map(ImportItem::Renamed),
            TypstSyntaxKind::Ident => child.cast().map(ImportItem::Simple),
            _ => Option::None,
        })
    }
}
