...
use crate::{is_newline, parse, FileId, LinkedNode, TypstSynSpan, TypstSyntaxNode, VirtualPath};
...
struct Repr {
    ...
    root: Prehashed<TypstSyntaxNode>,
    lines: Vec<Line>,
}
...
impl Source {
    ...
    pub fn root(&self) -> &TypstSyntaxNode {
        &self.0.root
    }
    ...
    pub fn find(&self, span: TypstSynSpan) -> Option<LinkedNode<'_>> {
    ...
    ///
    /// Returns `None` if the span does not point into this source file.
    pub fn range(&self, span: TypstSynSpan) -> Option<Range<usize>> {
