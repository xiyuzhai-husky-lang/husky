...
use crate::ast::TypstAstNode;
use crate::{FileId, TypstSynSpan, TypstSyntaxKind};

/// A node in the untyped syntax tree.
#[derive(Clone, Eq, PartialEq, Hash)]
pub struct TypstSyntaxNode(TypstSyntaxNodeRepr);

/// The three internal representations.
#[derive(Clone, Eq, PartialEq, Hash)]
enum TypstSyntaxNodeRepr {
    /// A leaf node.
    Leaf(LeafNode),
...
impl TypstSyntaxNode {
    /// Create a new leaf node.
    pub fn leaf(kind: TypstSyntaxKind, text: impl Into<EcoString>) -> Self {
        Self(TypstSyntaxNodeRepr::Leaf(LeafNode::new(kind, text)))
    }

    /// Create a new inner node with children.
    pub fn inner(kind: TypstSyntaxKind, children: Vec<TypstSyntaxNode>) -> Self {
        Self(TypstSyntaxNodeRepr::Inner(Arc::new(InnerNode::new(
            kind, children,
        ))))
    }
    ...
    pub fn error(message: impl Into<EcoString>, text: impl Into<EcoString>) -> Self {
        Self(TypstSyntaxNodeRepr::Error(Arc::new(ErrorNode::new(
            message, text,
        ))))
    }

    /// The type of the node.
    pub fn kind(&self) -> TypstSyntaxKind {
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.kind,
            TypstSyntaxNodeRepr::Inner(inner) => inner.kind,
            TypstSyntaxNodeRepr::Error(_) => TypstSyntaxKind::Error,
        }
    }
    ...
    pub fn len(&self) -> usize {
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.len(),
            TypstSyntaxNodeRepr::Inner(inner) => inner.len,
            TypstSyntaxNodeRepr::Error(node) => node.len(),
        }
    }
    ...
    pub fn span(&self) -> TypstSynSpan {
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.span,
            TypstSyntaxNodeRepr::Inner(inner) => inner.span,
            TypstSyntaxNodeRepr::Error(node) => node.error.span,
        }
    }
    ...
    pub fn text(&self) -> &EcoString {
        ...
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => &leaf.text,
            TypstSyntaxNodeRepr::Inner(_) => &EMPTY,
            TypstSyntaxNodeRepr::Error(node) => &node.text,
        }
    }
    ...
    pub fn into_text(self) -> EcoString {
        match self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.text,
            TypstSyntaxNodeRepr::Inner(inner) => inner
                .children
                .iter()
                .cloned()
                .map(Self::into_text)
                .collect(),
            TypstSyntaxNodeRepr::Error(node) => node.text.clone(),
        }
    }

    /// The node's children.
    pub fn children(&self) -> std::slice::Iter<'_, TypstSyntaxNode> {
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(_) | TypstSyntaxNodeRepr::Error(_) => [].iter(),
            TypstSyntaxNodeRepr::Inner(inner) => inner.children.iter(),
        }
    }

    /// Whether the node can be cast to the given AST node.
    pub fn is<'a, T: TypstAstNode<'a>>(&'a self) -> bool {
        self.cast::<T>().is_some()
    }

    /// Try to convert the node to a typed AST node.
    pub fn cast<'a, T: TypstAstNode<'a>>(&'a self) -> Option<T> {
        T::from_untyped(self)
    }

    /// Cast the first child that can cast to the AST type `T`.
    pub fn cast_first_match<'a, T: TypstAstNode<'a>>(&'a self) -> Option<T> {
        self.children().find_map(Self::cast)
    }

    /// Cast the last child that can cast to the AST type `T`.
    pub fn cast_last_match<'a, T: TypstAstNode<'a>>(&'a self) -> Option<T> {
    ...
    pub fn erroneous(&self) -> bool {
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(_) => false,
            TypstSyntaxNodeRepr::Inner(inner) => inner.erroneous,
            TypstSyntaxNodeRepr::Error(_) => true,
        }
    }
    ...
    pub fn errors(&self) -> Vec<SyntaxError> {
        ...
        if let TypstSyntaxNodeRepr::Error(node) = &self.0 {
    ...
    pub fn hint(&mut self, hint: impl Into<EcoString>) {
        if let TypstSyntaxNodeRepr::Error(node) = &mut self.0 {
            Arc::make_mut(node).hint(hint);
        }
    }

    /// Set a synthetic span for the node and all its descendants.
    pub fn synthesize(&mut self, span: TypstSynSpan) {
        match &mut self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.span = span,
            TypstSyntaxNodeRepr::Inner(inner) => Arc::make_mut(inner).synthesize(span),
            TypstSyntaxNodeRepr::Error(node) => Arc::make_mut(node).error.span = span,
        }
    }
    ...
    pub fn spanless_eq(&self, other: &Self) -> bool {
        match (&self.0, &other.0) {
            (TypstSyntaxNodeRepr::Leaf(a), TypstSyntaxNodeRepr::Leaf(b)) => a.spanless_eq(b),
            (TypstSyntaxNodeRepr::Inner(a), TypstSyntaxNodeRepr::Inner(b)) => a.spanless_eq(b),
            (TypstSyntaxNodeRepr::Error(a), TypstSyntaxNodeRepr::Error(b)) => a.spanless_eq(b),
            _ => false,
        }
    }
}
...
impl TypstSyntaxNode {
    ...
    pub(super) fn make_erroneous(&mut self) {
        if let TypstSyntaxNodeRepr::Inner(inner) = &mut self.0 {
    ...
    #[track_caller]
    pub(super) fn convert_to_kind(&mut self, kind: TypstSyntaxKind) {
        debug_assert!(!kind.is_error());
        match &mut self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.kind = kind,
            TypstSyntaxNodeRepr::Inner(inner) => Arc::make_mut(inner).kind = kind,
            TypstSyntaxNodeRepr::Error(_) => panic!("cannot convert error"),
        }
    }
    ...
    pub(super) fn convert_to_error(&mut self, message: impl Into<EcoString>) {
        ...
        *self = TypstSyntaxNode::error(message, text);
    }
    ...
    pub(super) fn numberize(&mut self, id: FileId, within: Range<u64>) -> NumberingResult {
        ...
        let mid = TypstSynSpan::new(id, (within.start + within.end) / 2).unwrap();
        match &mut self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.span = mid,
            TypstSyntaxNodeRepr::Inner(inner) => {
                Arc::make_mut(inner).numberize(id, None, within)?
            }
            TypstSyntaxNodeRepr::Error(node) => Arc::make_mut(node).error.span = mid,
        }
    ...
    pub(super) fn is_leaf(&self) -> bool {
        matches!(self.0, TypstSyntaxNodeRepr::Leaf(_))
    }

    /// The number of descendants, including the node itself.
    pub(super) fn descendants(&self) -> usize {
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(_) | TypstSyntaxNodeRepr::Error(_) => 1,
            TypstSyntaxNodeRepr::Inner(inner) => inner.descendants,
        }
    }

    /// The node's children, mutably.
    pub(super) fn children_mut(&mut self) -> &mut [TypstSyntaxNode] {
        match &mut self.0 {
            TypstSyntaxNodeRepr::Leaf(_) | TypstSyntaxNodeRepr::Error(_) => &mut [],
            TypstSyntaxNodeRepr::Inner(inner) => &mut Arc::make_mut(inner).children,
        }
    }
    ...
    pub(super) fn replace_children(
        &mut self,
        range: Range<usize>,
        replacement: Vec<TypstSyntaxNode>,
    ) -> NumberingResult {
        if let TypstSyntaxNodeRepr::Inner(inner) = &mut self.0 {
    ...
    pub(super) fn upper(&self) -> u64 {
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.span.number() + 1,
            TypstSyntaxNodeRepr::Inner(inner) => inner.upper,
            TypstSyntaxNodeRepr::Error(node) => node.error.span.number() + 1,
        }
    }
    ...
    pub(super) const fn arbitrary() -> Self {
        Self(TypstSyntaxNodeRepr::Leaf(LeafNode {
            kind: TypstSyntaxKind::Eof,
            text: EcoString::new(),
...
impl Debug for TypstSyntaxNode {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match &self.0 {
            TypstSyntaxNodeRepr::Leaf(leaf) => leaf.fmt(f),
            TypstSyntaxNodeRepr::Inner(inner) => inner.fmt(f),
            TypstSyntaxNodeRepr::Error(node) => node.fmt(f),
        }
    }
}

impl Default for TypstSyntaxNode {
    fn default() -> Self {
        Self::arbitrary()
    }
}
...
impl LeafNode {
    /// Create a new leaf node.
    #[track_caller]
    fn new(kind: TypstSyntaxKind, text: impl Into<EcoString>) -> Self {
        debug_assert!(!kind.is_error());
        Self {
            kind,
            text: text.into(),
...
struct InnerNode {
    ...
    children: Vec<TypstSyntaxNode>,
}

impl InnerNode {
    /// Create a new inner node with the given kind and children.
    #[track_caller]
    fn new(kind: TypstSyntaxKind, children: Vec<TypstSyntaxNode>) -> Self {
        debug_assert!(!kind.is_error());
    ...
    /// Set a synthetic span for the node and all its descendants.
    fn synthesize(&mut self, span: TypstSynSpan) {
        self.span = span;
    ...
    fn numberize(
        ...
        let descendants = match &range {
            ...
            Some(range) => self.children[range.clone()]
                .iter()
                .map(TypstSyntaxNode::descendants)
                .sum::<usize>(),
        ...
        if range.is_none() {
            ...
            self.span = TypstSynSpan::new(id, (start + end) / 2).unwrap();
    ...
    fn replace_children(
        ...
        self.len = self.len + replacement.iter().map(TypstSyntaxNode::len).sum::<usize>()
            - superseded.iter().map(TypstSyntaxNode::len).sum::<usize>();
        ...
        self.erroneous = replacement.iter().any(TypstSyntaxNode::erroneous)
            || (self.erroneous
                && (self.children[..range.start]
                    .iter()
                    .any(TypstSyntaxNode::erroneous))
    ...
    fn update_parent(
        ...
        self.erroneous = self.children.iter().any(TypstSyntaxNode::erroneous);
    }
}
...
impl<'a> LinkedNode<'a> {
    ...
    pub fn new(root: &'a TypstSyntaxNode) -> Self {
        Self {
            node: root,
            parent: None,
            index: 0,
    ...
    pub fn get(&self) -> &'a TypstSyntaxNode {
        self.node
    }
    ...
    pub fn find(&self, span: TypstSynSpan) -> Option<LinkedNode<'a>> {
        if self.span() == span {
            return Some(self.clone());
        }

        if let TypstSyntaxNodeRepr::Inner(inner) = &self.0 {
...
pub struct LinkedChildren<'a> {
    ...
    iter: std::iter::Enumerate<std::slice::Iter<'a, TypstSyntaxNode>>,
    front: usize,
    back: usize,
}
