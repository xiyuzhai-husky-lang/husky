...
use crate::{
    is_newline, parse, reparse_block, reparse_markup, TypstSynSpan, TypstSyntaxKind,
    TypstSyntaxNode,
};
...
pub fn reparse(
    ...
    try_reparse(text, replaced, replacement_len, None, root, 0).unwrap_or_else(|| {
        ...
        *root = parse(text);
        if let Some(id) = id {
            root.numberize(id, TypstSynSpan::FULL).unwrap();
        }
        0..text.len()
    })
}
...
fn try_reparse(
    ...
    replaced: Range<usize>,
    replacement_len: usize,
    parent_kind: Option<TypstSyntaxKind>,
    node: &mut TypstSyntaxNode,
    offset: usize,
) -> Option<Range<usize>> {
    ...
    if overlap.is_empty()
        || node.kind() != TypstSyntaxKind::TypstMarkup
        || !matches!(parent_kind, None | Some(TypstSyntaxKind::ContentBlock))
    {
        return None;
    }
    ...
    loop {
        ...
        // Also take hash.
        if start > 0 && children[start - 1].kind() == TypstSyntaxKind::Hash {
            start -= 1;
        }
        ...
        let stop_kind = match parent_kind {
            Some(_) => TypstSyntaxKind::RightDelimiter,
...
fn expand(node: &TypstSyntaxNode) -> bool {
    let kind = node.kind();
    kind.is_trivia()
        || kind.is_error()
        || kind == TypstSyntaxKind::Semicolon
        || node.text() == "/"
        || node.text() == ":"
}
...
fn next_at_start(node: &TypstSyntaxNode, at_start: &mut bool) {
    let kind = node.kind();
    if kind.is_trivia() {
        *at_start |= kind == TypstSyntaxKind::Parbreak
            || (kind == TypstSyntaxKind::Space && node.text().chars().any(is_newline));
    } else {
        *at_start = false;
    }
}
...
fn next_nesting(node: &TypstSyntaxNode, nesting: &mut usize) {
...
mod tests {
    ...
    use crate::{parse, Source, TypstSynSpan};
    ...
    fn test(prev: &str, range: Range<usize>, with: &str, incremental: bool) {
        ...
        found.synthesize(TypstSynSpan::detached());
        expected.synthesize(TypstSynSpan::detached());
        if found != expected {
