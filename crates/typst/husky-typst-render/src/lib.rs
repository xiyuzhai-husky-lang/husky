...
use husky_typst::visualize::{
    DashPattern, Gradient, ImageKind, LineCap, LineJoin, Path, PathItem, Pattern, RasterFormat,
    RelativeTo, TypstColor, TypstFixedStroke, TypstGeometry, TypstImage, TypstPaint, TypstShape,
};
...
pub fn render(frame: &TypstFrame, pixel_per_pt: f32, fill: TypstColor) -> sk::Pixmap {
    let size = frame.size();
...
pub fn render_merged(
    ...
    let pxh = padding
        + pixmaps
            .iter()
            .map(|pixmap| pixmap.height() + padding)
            .sum::<u32>();
...
fn render_frame(canvas: &mut sk::Pixmap, state: State, frame: &TypstFrame) {
    for (pos, item) in frame.items() {
        match item {
            ...
            TypstFrameItem::Meta(meta, _) => match meta {
                TypstMeta::Link(_) => {}
                TypstMeta::Elem(_) => {}
                TypstMeta::Hide => {}
            },
...
fn render_text(canvas: &mut sk::Pixmap, state: State, text: &TypstTextItem) {
    let mut x = 0.0;
...
fn render_bitmap_glyph(
    ...
    let image = TypstImage::new(raster.data.into(), RasterFormat::Png.into(), None).ok()?;
    ...
    if ppem > 100.0 || ts.kx != 0.0 || ts.ky != 0.0 || ts.sx != ts.sy || text.stroke.is_some() {
        ...
        }) = &text.stroke
        {
            if thickness.to_f32() > 0.0 {
                ...
                let paint =
                    to_sk_paint(paint, state_ts, Size::zero(), true, None, &mut pixmap, None);
...
#[comemo::memoize]
fn scaled_texture(image: &TypstImage, w: u32, h: u32) -> Option<Arc<sk::Pixmap>> {
...
impl<'a> PatternSampler<'a> {
    fn new(pattern: &'a Pattern, pixmap: &'a sk::Pixmap, state: &State, on_text: bool) -> Self {
...
impl PaintSampler for PatternSampler<'_> {
    ...
    fn sample(self, (x, y): (u32, u32)) -> sk::PremultipliedColorU8 {
        ...
        let x = (point.x * self.pixel_per_pt)
            .rem_euclid(self.size.x.to_f32())
            .floor() as u32;
        let y = (point.y * self.pixel_per_pt)
            .rem_euclid(self.size.y.to_f32())
            .floor() as u32;
...
fn to_sk_dash_pattern(
    pattern: &DashPattern<TypstAbsLength, TypstAbsLength>,
) -> Option<sk::StrokeDash> {
...
fn offset_bounding_box(bbox: Size, stroke_width: TypstAbsLength) -> Size {
