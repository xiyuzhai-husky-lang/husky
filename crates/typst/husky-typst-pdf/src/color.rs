use husky_typst::visualize::{ColorSpace, TexColor, TexPaint};
...
impl ColorSpaces {
    ...
    pub fn write(&mut self, color_space: ColorSpace, writer: writers::ColorSpace, alloc: &mut Ref) {
        match color_space {
    ...
    pub fn write_color_spaces(&mut self, mut spaces: Dict, alloc: &mut Ref) {
        ...
        if self.use_linear_rgb {
            self.write(
                ColorSpace::LinearRgb,
                spaces.insert(LINEAR_SRGB).start(),
                alloc,
            );
        }
    }
...
pub trait ColorEncode {
    ...
    fn encode(&self, color: TexColor) -> [f32; 4];
}
...
impl PaintEncode for TexColor {
    fn set_as_fill(&self, ctx: &mut PageContext, _: bool, _: Transforms) {
        match self {
            ...
            TexColor::Oklab(_) | TexColor::Oklch(_) | TexColor::Hsl(_) | TexColor::Hsv(_) => {
...
pub(super) trait ColorSpaceExt {
    ...
    fn convert<U: QuantizedColor>(self, color: TexColor) -> [U; 3];
}
...
impl QuantizedColor for u16 {
    fn quantize(color: f32, [min, max]: [f32; 2]) -> Self {
        ...
        (value * Self::MAX as f32)
            .round()
            .clamp(0.0, Self::MAX as f32) as Self
    }
}
