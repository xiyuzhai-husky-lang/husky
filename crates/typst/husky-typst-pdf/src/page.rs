...
#[husky_typst_macros::time(name = "construct pages")]
...
#[husky_typst_macros::time(name = "construct page")]
pub(crate) fn construct_page(ctx: &mut PdfContext, frame: &TypstFrame) -> (Ref, EncodedPage) {
...
fn write_page(ctx: &mut PdfContext, i: usize) {
    ...
    for (dest, rect) in &page.links {
        ...
        let pos = match dest {
            ...
            TypstDestination::Position(pos) => *pos,
            TypstDestination::Location(loc) => ctx.document.introspector.position(*loc),
        };
...
impl PdfPageLabel {
    ...
    fn generate(numbering: &Numbering, number: usize) -> Option<PdfPageLabel> {
        ...
        if pat.suffix.is_empty() {
            use {husky_typst::model::NumberingKind as Kind, PdfPageLabelStyle as Style};
            match (kind, case) {
                ...
                (Kind::Letter, Case::Lower) if number <= 26 => style = Some(Style::LowerAlpha),
                (Kind::Letter, Case::Upper) if number <= 26 => style = Some(Style::UpperAlpha),
                _ => {}
            }
        }
...
pub struct EncodedPage {
    ...
    pub links: Vec<(TypstDestination, Rect)>,
...
pub struct PageContext<'a, 'b> {
    ...
    bottom: f32,
    uses_opacities: bool,
    links: Vec<(TypstDestination, Rect)>,
...
struct State {
    ...
    size: Size,
    font: Option<(TypstFont, TypstAbsLength)>,
...
impl PageContext<'_, '_> {
    ...
    fn set_font(&mut self, font: &TypstFont, size: TypstAbsLength) {
    ...
    pub fn set_fill_color_space(&mut self, space: Name<'static>) {
        if self.state.fill_space != Some(space) {
            self.content
                .set_fill_color_space(ColorSpaceOperand::Named(space));
    ...
    pub fn set_stroke_color_space(&mut self, space: Name<'static>) {
        if self.state.stroke_space != Some(space) {
            self.content
                .set_stroke_color_space(ColorSpaceOperand::Named(space));
...
fn write_frame(ctx: &mut PageContext, frame: &TypstFrame) {
    for &(pos, ref item) in frame.items() {
        let x = pos.x.to_f32();
        let y = pos.y.to_f32();
        match item {
            TypstFrameItem::Group(group) => write_group(ctx, pos, group),
            TypstFrameItem::Text(text) => write_text(ctx, pos, text),
            TypstFrameItem::Shape(shape, _) => write_shape(ctx, pos, shape),
            TypstFrameItem::Image(image, size, _) => write_image(ctx, x, y, image, *size),
            TypstFrameItem::Meta(meta, size) => match meta {
                TypstMeta::Link(dest) => write_link(ctx, pos, dest, *size),
                TypstMeta::Elem(_) => {}
                TypstMeta::Hide => {}
            },
        }
...
fn write_text(ctx: &mut PageContext, pos: Point, text: &TypstTextItem) {
    ...
    *ctx.parent.languages.entry(text.lang).or_insert(0) += text.glyphs.len();
    ...
    let mut adjustment = TypstEmLength::zero();
    let mut encoded = vec![];
...
fn write_path(ctx: &mut PageContext, x: f32, y: f32, path: &Path) {
    for elem in &path.0 {
        match elem {
            PathItem::MoveTo(p) => ctx.content.move_to(x + p.x.to_f32(), y + p.y.to_f32()),
            PathItem::LineTo(p) => ctx.content.line_to(x + p.x.to_f32(), y + p.y.to_f32()),
...
fn write_image(ctx: &mut PageContext, x: f32, y: f32, image: &TypstImage, size: Size) {
    ...
    ctx.resources.insert(
        PageResource::new(ResourceKind::XObject, name.clone()),
        index,
    );
