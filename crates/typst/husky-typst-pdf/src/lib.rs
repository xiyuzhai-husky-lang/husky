...
use husky_typst::layout::{Transform, TypstAbsLength, TypstEmLength, TypstLayoutDirection};
use husky_typst::model::TypstDocument;
use husky_typst::text::{Lang, TypstFont};
use husky_typst::util::Deferred;
use husky_typst::visualize::TypstImage;
...
fn hash_base64<T: Hash>(value: &T) -> String {
    base64::engine::general_purpose::STANDARD
        .encode(husky_typst::util::hash128(value).to_be_bytes())
}
    ...
    T: Eq + Hash + Clone,
{
    ...
    fn pdf_indices<'a>(&'a self, refs: &'a [Ref]) -> impl Iterator<Item = (Ref, usize)> + 'a {
...
impl AbsExt for TypstAbsLength {
    fn to_f32(self) -> f32 {
        self.to_pt() as f32
    }
}
