...
use husky_typst::visualize::{
    ColorSpace, ImageKind, RasterFormat, RasterImage, SvgImage, TypstImage,
};
...
#[comemo::memoize]
pub fn deferred_image(image: TypstImage) -> Deferred<EncodedImage> {
...
#[husky_typst_macros::time(name = "write images")]
...
fn encode_svg(svg: &SvgImage) -> Chunk {
    ...
    unsafe {
        svg.with(|tree| {
            svg2pdf::convert_tree_into(tree, svg2pdf::Options::default(), &mut chunk, Ref::new(1));
        });
    }
