    ...
    } in ctx.gradient_map.items().cloned().collect::<Vec<_>>()
    {
        ...
        let mut shading_pattern = match &gradient {
            Gradient::Linear(_) => {
                ...
                ctx.colors
                    .write(color_space, shading.color_space(), &mut ctx.alloc);
            ...
            Gradient::Conic(_) => {
                ...
                ctx.colors
                    .write(color_space, stream_shading.color_space(), &mut ctx.alloc);
...
fn shading_function(ctx: &mut PdfContext, gradient: &Gradient, color_space: ColorSpace) -> Ref {
...
fn register_gradient(
    ...
    if transforms.size.x.is_zero() {
        transforms.size.x = TypstAbsLength::pt(1.0);
    }

    if transforms.size.y.is_zero() {
        transforms.size.y = TypstAbsLength::pt(1.0);
    }
    ...
    let (offset_x, offset_y) = match gradient {
        ...
        _ => (TypstAbsLength::zero(), TypstAbsLength::zero()),
    };
...
///  - points: `[u16; 24]`
///  - colors: `[u16; 12]`
fn write_patch(target: &mut Vec<u8>, t: f32, t1: f32, c0: [u16; 3], c1: [u16; 3], angle: Angle) {
...
fn compute_vertex_stream(gradient: &Gradient, aspect_ratio: Ratio) -> Arc<Vec<u8>> {
    let Gradient::Conic(conic) = gradient else {
        unreachable!()
    };
