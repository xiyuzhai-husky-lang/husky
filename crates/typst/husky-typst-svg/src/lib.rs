...
use husky_typst::visualize::{
    Gradient, ImageFormat, LineCap, LineJoin, Path, PathItem, Pattern, RasterFormat, RatioOrAngle,
...
#[husky_typst_macros::time(name = "svg")]
pub fn svg(frame: &TypstFrame) -> String {
...
pub fn svg_merged(document: &TypstDocument, padding: TypstAbsLength) -> String {
...
impl SVGRenderer {
    ...
    fn render_pattern_frame(&mut self, state: State, ts: Transform, frame: &TypstFrame) -> String {
    ...
    fn render_frame(&mut self, state: State, ts: Transform, frame: &TypstFrame) {
        ...
        for (pos, item) in frame.items() {
            ...
            if matches!(item, TypstFrameItem::Meta(_, _)) {
                continue;
            }
            ...
            match item {
                TypstFrameItem::Group(group) => self.render_group(state.pre_translate(*pos), group),
                TypstFrameItem::Text(text) => self.render_text(state.pre_translate(*pos), text),
                TypstFrameItem::Shape(shape, _) => {
                    self.render_shape(state.pre_translate(*pos), shape)
                }
                TypstFrameItem::Image(image, size, _) => self.render_image(image, size),
                TypstFrameItem::Meta(_, _) => unreachable!(),
            };
    ...
    /// be created.
    fn render_group(&mut self, state: State, group: &TypstGroupItem) {
    ...
    fn render_text(&mut self, state: State, text: &TypstTextItem) {
    ...
    fn render_svg_glyph(
        ...
        let upem = TypstAbsLength::raw(text.font.units_per_em());
        ...
        let id = self
            ...
            .insert_with(glyph_hash, || RenderedGlyph::Image {
                ...
                width: upem.to_pt(),
                height: upem.to_pt(),
                ts: Transform::translate(
                    TypstAbsLength::zero(),
                    TypstAbsLength::pt(-origin_ascender),
                )
    ...
    fn render_bitmap_glyph(
        ...
        let id = self.glyphs.insert_with(glyph_hash, || {
            ...
            let ts = Transform::translate(
                TypstAbsLength::pt(bitmap_x_offset),
                TypstAbsLength::pt(-height - bitmap_y_offset),
            );
            RenderedGlyph::Image {
                url,
                width,
    ...
    fn render_outline_glyph(
        ...
        self.write_fill(
            &text.fill,
            Size::new(TypstAbsLength::pt(width), TypstAbsLength::pt(height)),
    ...
    fn shape_paint_transform(
        ...
        if shape_size.x.to_pt() == 0.0 {
            shape_size.x = TypstAbsLength::pt(1.0);
        }

        if shape_size.y.to_pt() == 0.0 {
            shape_size.y = TypstAbsLength::pt(1.0);
        }
    ...
    fn render_image(&mut self, image: &TypstImage, size: &Axes<TypstAbsLength>) {
        let url = convert_image_to_base64_url(image);
...
#[comemo::memoize]
fn convert_bitmap_glyph_to_image(font: &TypstFont, id: GlyphId) -> Option<(TypstImage, f64, f64)> {
...
fn convert_svg_glyph_to_base64_url(font: &TypstFont, id: GlyphId) -> Option<EcoString> {
    ...
    let upem = TypstAbsLength::raw(font.units_per_em());
...
#[comemo::memoize]
fn convert_image_to_base64_url(image: &TypstImage) -> EcoString {
