...
pub(crate) struct ResolutionScope<'db> {
    scope: semantics::SemanticsScope<'db>,
    node: SyntaxNode,
}
...
pub(crate) struct ResolvedPath {
    pub(crate) resolution: semantics::PathResolution,
...
pub(crate) struct UfcsCallInfo {
    ...
    pub(crate) function: semantics::Function,
    pub(crate) qualifier_type: Option<semantics::Type>,
}
...
impl Resolver<'_, '_> {
            ...
            .filter_map(|(path_node, resolved)| {
                if let Some(grandparent) = path_node.parent().and_then(|parent| parent.parent()) {
                    if let Some(call_expr) = ast::CallExpr::cast(grandparent.clone()) {
                        if let semantics::PathResolution::AssocItem(
                            semantics::AssocItem::Function(function),
                        ) = resolved.resolution
                        {
    ...
    fn ok_to_use_path_resolution(&self, resolution: &semantics::PathResolution) -> bool {
        match resolution {
            semantics::PathResolution::AssocItem(semantics::AssocItem::Function(function)) => {
                if function
            ...
            semantics::PathResolution::AssocItem(_) => {
...
impl<'db> ResolutionScope<'db> {
    pub(crate) fn new(
        sema: &semantics::Semantics<'db, ide_db::RootDatabase>,
    ...
    fn resolve_path(&self, path: &ast::Path) -> Option<semantics::PathResolution> {
        ...
        if let semantics::PathResolution::Def(semantics::ModuleDef::Adt(adt)) = resolved_qualifier {
            ...
            adt.ty(self.scope.db).iterate_path_candidates(
                ...
                |_ty, assoc_item| {
                    ...
                    if item_name.to_smol_str().as_str() == name.text() {
                        Some(semantics::PathResolution::AssocItem(assoc_item))
                    } else {
                        None
                    }
                },
    ...
    fn qualifier_type(&self, path: &SyntaxNode) -> Option<semantics::Type> {
        use syntax::ast::AstNode;
        if let Some(path) = ast::Path::cast(path.clone()) {
            if let Some(qualifier) = path.qualifier() {
                if let Some(semantics::PathResolution::Def(semantics::ModuleDef::Adt(adt))) =
