...
pub struct Match {
    ...
    pub(crate) rendered_template_paths: FxHashMap<SyntaxNode, semantics::ModPath>,
}
...
impl<'db, 'sema> Matcher<'db, 'sema> {
    ...
    fn check_expr_type(
        &self,
        pattern_type: &semantics::Type,
        expr: &ast::Expr,
    ) -> Result<usize, MatchFailed> {
        use semantics::HirDisplay;
        let code_type = self
            .sema
            .type_of_expr(expr)
            .ok_or_else(|| {
...
impl Match {
    fn render_template_paths(
        ...
        for (path, resolved_path) in &template.resolved_paths {
            if let semantics::PathResolution::Def(module_def) = resolved_path.resolution {
