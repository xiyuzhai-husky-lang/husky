    ...
    pub fn diagnostics(self, db: &dyn HirDatabase, acc: &mut Vec<AnyDiagnostic>) {
        let _p = profile::span("Module::diagnostics").detail(|| {
            format!(
                "{:?}",
                self.name(db)
                    .map_or("<unknown>".into(), |name| name.to_string())
            )
        });
        let def_map = self.id.def_map(db.upcast());
...
impl Field {
    ...
    pub fn ty(&self, db: &dyn HirDatabase) -> Type {
        ...
        let ty = db.field_types(var_id)[self.id]
            .clone()
...
impl Struct {
    pub fn module(self, db: &dyn HirDatabase) -> Module {
        Module {
            id: self.id.lookup(db.upcast()).container,
        }
    }
...
impl Enum {
    ...
    pub fn variants(self, db: &dyn HirDatabase) -> Vec<Variant> {
        db.enum_data(self.id)
            .variants
            .iter()
            .map(|(id, _)| Variant { parent: self, id })
            .collect()
    }
...
impl Variant {
    ...
    pub(crate) fn variant_data(self, db: &dyn HirDatabase) -> Arc<VariantData> {
        db.enum_data(self.parent.id).variants[self.id]
            .variant_data
            .clone()
    }
}
    ...
    pub fn diagnostics(self, db: &dyn HirDatabase, acc: &mut Vec<AnyDiagnostic>) {
        ...
        for diag in source_map.diagnostics() {
            match diag {
                BodyDiagnostic::InactiveCode { node } => {
                    acc.push(InactiveCode { node: node.clone() }.into())
                }
        ...
        for diagnostic in BodyValidationDiagnostic::collect(db, self.into()) {
            match diagnostic {
                ...
                BodyValidationDiagnostic::MismatchedArgCount {
                    call_expr,
                    expected,
                    found,
                ...
                BodyValidationDiagnostic::AddReferenceHere {
                    arg_expr,
                    mutability,
...
impl Function {
    ...
    pub fn debug_hir(self, db: &dyn HirDatabase) -> String {
        ...
        format_to!(
            result,
            "HIR expressions in the body of `{}`:\n",
            self.name(db)
        );
...
impl Local {
    pub fn is_param(self, db: &dyn HirDatabase) -> bool {
        ...
        match src.value {
            Either::Left(bind_pat) => bind_pat
                .syntax()
                .ancestors()
                .any(|it| ast::Param::can_cast(it.kind())),
    ...
    pub fn is_mut(self, db: &dyn HirDatabase) -> bool {
        ...
        matches!(
            &body[self.pat_id],
            Pat::Bind {
                mode: BindingAnnotation::Mutable,
                ..
            }
        )
    }
    ...
    pub fn is_ref(self, db: &dyn HirDatabase) -> bool {
        ...
        matches!(
            &body[self.pat_id],
            Pat::Bind {
                mode: BindingAnnotation::Ref | BindingAnnotation::RefMut,
                ..
            }
        )
    }
    ...
    pub fn source(self, db: &dyn HirDatabase) -> InFile<Either<ast::IdentPat, ast::SelfParam>> {
        ...
        src.map(|ast| {
            ast.map_left(|it| it.cast().unwrap().to_node(&root))
                .map_right(|it| it.to_node(&root))
        })
    }
}
...
impl Type {
    ...
    fn from_def(
        ...
        let ty = TyBuilder::def_ty(db, def.into())
            .fill_with_unknown()
            .build();
        Type::new(db, krate, def, ty)
    }
    ...
    pub fn is_mutable_reference(&self) -> bool {
        matches!(
            ...
            TyKind::Ref(hir_ty::Mutability::Mut, ..)
        )
    }
