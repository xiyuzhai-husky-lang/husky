...
pub fn is_convex(line_segment_sketch: &LineSegmentSketch, index: i32) -> bool {
    ...
    let current_displacement = line_segment_sketch.strokes[index.rem_euclid(L)].displacement();
    let previous_displacement = line_segment_sketch.strokes[(index - 1).rem_euclid(L)].displacement();
    ...
    if is_rotation_counterclockwise_result == 0 {
        ...
        let previous_interval = line_segment_sketch.strokes[(index - 1).rem_euclid(L)].points;
        ...
        let current_interval = line_segment_sketch.strokes[index.rem_euclid(L)].points;
