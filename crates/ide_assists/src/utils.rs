...
use semantics::{db::HirDatabase, HasSource, HirDisplay};
use ide_db::{
...
pub fn filter_assoc_items(
    ...
    items
        ...
        .filter_map(|i| {
            let item = match i {
                semantics::AssocItem::Function(i) => ast::AssocItem::Fn(i.source(db)?.value),
                semantics::AssocItem::TypeAlias(i) => ast::AssocItem::TypeAlias(i.source(db)?.value),
                semantics::AssocItem::Const(i) => ast::AssocItem::Const(i.source(db)?.value),
            };
            Some(item)
        })
        .filter(has_def_name)
...
pub fn add_trait_assoc_items_to_impl(
    ...
    impl_: ast::Impl,
    target_scope: semantics::SemanticsScope,
...
// FIXME: It should return a new semantics::Type, but currently constructing new types is too cumbersome
...
fn handle_copy(ty: &semantics::Type, db: &dyn HirDatabase) -> Option<ReferenceConversionType> {
...
fn handle_as_ref_str(
    ...
    let str_type = semantics::BuiltinType::str().ty(db, module);
...
fn handle_as_ref_slice(
    ...
    let slice_type = semantics::Type::new_slice(type_argument);
