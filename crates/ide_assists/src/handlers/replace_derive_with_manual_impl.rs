...
pub(crate) fn replace_derive_with_manual_impl(
    ...
    .flat_map(|trait_| {
        current_module
            .find_use_path(ctx.sema.db, semantics::ModuleDef::Trait(trait_))
            .as_ref()
            .map(mod_path_to_ast)
            .zip(Some(trait_))
    });
...
fn impl_def_from_trait(
    sema: &semantics::Semantics<ide_db::RootDatabase>,
    adt: &ast::Adt,
