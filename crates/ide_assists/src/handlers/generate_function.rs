...
use semantics::{HasSource, HirDisplay, Module, Semantics, TypeInfo};
...
fn gen_fn(acc: &mut Assists, ctx: &AssistContext) -> Option<()> {
    ...
    let (target, file, insert_offset) = match path.qualifier() {
        Some(qualifier) => match ctx.sema.resolve_path(&qualifier) {
            Some(semantics::PathResolution::Def(semantics::ModuleDef::Module(module))) => {
                target_module = Some(module);
                get_fn_target(ctx, &target_module, call.clone())?
            }
            Some(semantics::PathResolution::Def(semantics::ModuleDef::Adt(adt))) => {
...
fn add_func_to_accumulator(
    ...
    file: FileID,
    adt_name: Option<semantics::Name>,
    label: String,
) -> Option<()> {
...
impl FunctionBuilder {
    ...
    fn from_call(
        ...
        call: &ast::CallExpr,
        fn_name: &str,
        target_module: Option<semantics::Module>,
        target: GeneratedFunctionTarget,
    ) -> Option<Self> {
...
fn fn_arg_type(ctx: &AssistContext, target_module: semantics::Module, fn_arg: &ast::Expr) -> String {
...
fn next_space_for_fn_in_module(
    db: &dyn semantics::db::AstDatabase,
    module_source: &semantics::InFile<semantics::ModuleSource>,
) -> Option<(FileID, GeneratedFunctionTarget)> {
    let file = module_source.file_id.original_file(db);
    let assist_item = match &module_source.value {
        semantics::ModuleSource::SourceFile(it) => match it.items().last() {
        ...
        semantics::ModuleSource::Module(it) => match it.item_list().and_then(|it| it.items().last()) {
            Some(last_item) => GeneratedFunctionTarget::BehindItem(last_item.syntax().clone()),
            None => GeneratedFunctionTarget::InEmptyItemList(it.item_list()?.syntax().clone()),
        },
        semantics::ModuleSource::BlockExpr(it) => {
...
fn module_is_descendant(module: &semantics::Module, ans: &semantics::Module, ctx: &AssistContext) -> bool {
    if module == ans {
        return true;
    }
