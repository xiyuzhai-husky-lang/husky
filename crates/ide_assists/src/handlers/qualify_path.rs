...
pub(crate) enum QualifyCandidate<'db> {
    ...
    ImplMethod(&'db RootDatabase, ast::MethodCallExpr, semantics::Function),
}
...
impl QualifyCandidate<'_> {
    pub(crate) fn qualify(
        ...
        import: &semantics::ModPath,
        item: semantics::ItemInNs,
    ) {
    ...
    fn qualify_fn_call(
        ...
        if let Some(self_access) = hir_fn.self_param(db).map(|sp| sp.access(db)) {
            let receiver = match self_access {
                semantics::Access::Shared => make::expr_ref(receiver, false),
                semantics::Access::Exclusive => make::expr_ref(receiver, true),
                semantics::Access::Owned => receiver,
            };
            replacer(format!(
                "{}::{}{}{}",
                import,
...
) -> Option<semantics::Function> {
    if let Some(semantics::AssocItem::Function(method)) =
        trait_.items(db).into_iter().find(|item: &semantics::AssocItem| {
            item.name(db)
...
fn item_as_trait(db: &RootDatabase, item: semantics::ItemInNs) -> Option<semantics::Trait> {
    let item_module_def = item.as_module_def()?;

    match item_module_def {
        semantics::ModuleDef::Trait(trait_) => Some(trait_),
