use semantics::{db::HirDatabase, HasSource, HasVisibility, PathResolution};
...
fn add_vis_to_referenced_record_field(acc: &mut Assists, ctx: &AssistContext) -> Option<()> {
    ...
    let (offset, current_visibility, target) = match in_file_source.value {
        semantics::FieldSource::Named(it) => {
            let s = it.syntax();
            (vis_offset(s), it.visibility(), s.text_range())
        }
        semantics::FieldSource::Pos(it) => {
            let s = it.syntax();
            (vis_offset(s), it.visibility(), s.text_range())
        }
    };
...
fn target_data_for_def(
    db: &dyn HirDatabase,
    def: semantics::ModuleDef,
) -> Option<(TextSize, Option<ast::Visibility>, TextRange, FileID, Option<semantics::Name>)> {
    ...
    let (offset, current_visibility, target, target_file) = match def {
        semantics::ModuleDef::Function(f) => {
            target_name = Some(f.name(db));
            offset_target_and_file_id(db, f)?
        }
        semantics::ModuleDef::Adt(adt) => {
            target_name = Some(adt.name(db));
            match adt {
                semantics::Adt::Struct(s) => offset_target_and_file_id(db, s)?,
                semantics::Adt::Union(u) => offset_target_and_file_id(db, u)?,
                semantics::Adt::Enum(e) => offset_target_and_file_id(db, e)?,
            }
        }
        semantics::ModuleDef::Const(c) => {
            target_name = c.name(db);
            offset_target_and_file_id(db, c)?
        }
        semantics::ModuleDef::Static(s) => {
            target_name = Some(s.name(db));
            offset_target_and_file_id(db, s)?
        }
        semantics::ModuleDef::Trait(t) => {
            target_name = Some(t.name(db));
            offset_target_and_file_id(db, t)?
        }
        semantics::ModuleDef::TypeAlias(t) => {
            target_name = Some(t.name(db));
            offset_target_and_file_id(db, t)?
        }
        semantics::ModuleDef::Module(m) => {
        ...
        semantics::ModuleDef::Variant(_) | semantics::ModuleDef::BuiltinType(_) => return None,
    };
