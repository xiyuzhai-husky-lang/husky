...
use ast::make;
use either::Either;
use semantics::{HirDisplay, InFile, Local, ModuleDef, Semantics, TypeInfo};
use ide_db::{
...
enum FunType {
    Unit,
    Single(semantics::Type),
    Tuple(Vec<semantics::Type>),
}
...
struct ContainerInfo {
    ...
    ret_type: Option<semantics::Type>,
}
...
impl Param {
    ...
    fn to_param(&self, ctx: &AssistContext, module: semantics::Module) -> ast::Param {
...
impl TryKind {
    fn of_ty(ty: semantics::Type, ctx: &AssistContext) -> Option<TryKind> {
        if ty.is_unknown() {
...
impl FlowKind {
    ...
    fn expr_ty(&self, ctx: &AssistContext) -> Option<semantics::Type> {
        match self {
...
fn expr_require_exclusive_access(ctx: &AssistContext, expr: &ast::Expr) -> Option<bool> {
    ...
    if let Some(method_call) = ast::MethodCallExpr::cast(parent.clone()) {
        ...
        return Some(matches!(access, semantics::Access::Exclusive));
    }
...
fn is_defined_outside_of_body(
    ctx: &AssistContext,
    body: &FunctionBody,
    src: &semantics::InFile<Either<ast::IdentPat, ast::SelfParam>>,
) -> bool {
...
impl Function {
    fn make_param_list(&self, ctx: &AssistContext, module: semantics::Module) -> ast::ParamList {
    ...
    fn make_ret_ty(&self, ctx: &AssistContext, module: semantics::Module) -> Option<ast::RetType> {
...
impl FunType {
    fn make_ty(&self, ctx: &AssistContext, module: semantics::Module) -> ast::Type {
        match self {
...
fn format_type(ty: &semantics::Type, ctx: &AssistContext, module: semantics::Module) -> String {
    ty.display_source_code(ctx.db(), module.into()).ok().unwrap_or_else(|| "_".to_string())
}

fn make_ty(ty: &semantics::Type, ctx: &AssistContext, module: semantics::Module) -> ast::Type {
