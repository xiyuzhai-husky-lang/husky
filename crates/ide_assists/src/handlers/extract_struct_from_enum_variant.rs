...
use either::Either;
use semantics::{Module, ModuleDef, Name, Variant};
use ide_db::{
    defs::Definition,
    helpers::{
...
fn existing_definition(db: &RootDatabase, variant_name: &ast::Name, variant: &Variant) -> bool {
    variant
        ...
        .filter(|(_, def)| match def {
            ...
            semantics::ScopeDef::ModuleDef(def) => matches!(
                def,
                ModuleDef::Module(_)
                    | ModuleDef::Adt(_)
                    | ModuleDef::Variant(_)
...
fn apply_references(
    ...
    node: SyntaxNode,
    import: Option<(ImportScope, semantics::ModPath)>,
) {
...
) -> Vec<(ast::PathSegment, SyntaxNode, Option<(ImportScope, semantics::ModPath)>)> {
...
) -> Option<(ast::PathSegment, SyntaxNode, semantics::Module)> {
    let segment =
