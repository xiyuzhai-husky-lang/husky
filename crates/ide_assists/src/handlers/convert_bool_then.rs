use semantics::{known, AsAssocItem, Semantics};
use ide_db::{
    helpers::{
        for_each_tail_expr,
...
pub(crate) fn convert_bool_then_to_if(acc: &mut Assists, ctx: &AssistContext) -> Option<()> {
    ...
    match assoc.container(ctx.sema.db) {
        semantics::AssocItemContainer::Impl(impl_) if impl_.self_ty(ctx.sema.db).is_bool() => {}
        _ => return None,
    }
...
fn option_variants(
    ...
    expr: &SyntaxNode,
) -> Option<(semantics::Variant, semantics::Variant)> {
...
fn is_invalid_body(
    sema: &Semantics<RootDatabase>,
    some_variant: semantics::Variant,
    expr: &ast::Expr,
) -> bool {
        ...
        for_each_tail_expr(expr, &mut |e| {
            ...
            if let Some(ast::Expr::CallExpr(call)) = e {
                if let Some(ast::Expr::PathExpr(p)) = call.expr() {
                    ...
                    if let Some(semantics::PathResolution::Def(semantics::ModuleDef::Variant(v))) = res {
