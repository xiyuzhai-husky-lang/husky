...
fn compute_method_ranks(path: &ast::Path, ctx: &AssistContext) -> Option<FxHashMap<String, usize>> {
    ...
    Some(
        td.items(ctx.db())
            .iter()
            .flat_map(|i| match i {
                semantics::AssocItem::Function(f) => Some(f),
                _ => None,
            })
            .enumerate()
...
fn trait_definition(path: &ast::Path, sema: &Semantics<RootDatabase>) -> Option<semantics::Trait> {
    match sema.resolve_path(path)? {
        PathResolution::Def(semantics::ModuleDef::Trait(trait_)) => Some(trait_),
        _ => None,
    }
}
