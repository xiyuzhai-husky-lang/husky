...
pub(crate) fn replace_qualified_name_with_use(
    ...
    match ctx.sema.resolve_path(&path)? {
        semantics::PathResolution::Def(def) if def.as_assoc_item(ctx.sema.db).is_none() => (),
        semantics::PathResolution::Macro(_) => (),
        _ => return None,
    }
    ...
    let module = match ctx.sema.resolve_path(&path.first_qualifier_or_self())? {
        semantics::PathResolution::Def(module @ semantics::ModuleDef::Module(_)) => module,
        _ => return None,
    };
