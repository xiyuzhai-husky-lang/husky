...
use either::Either;
use semantics::{Adt, HasSource, ModuleDef, Semantics};
...
fn lift_enum(e: semantics::Enum) -> ExtendedEnum {
    ExtendedEnum::Enum(e)
}
    ...
    sema.type_of_expr(expr)?
        ...
        .map(|ty| {
            ty.autoderef(sema.db).find_map(|ty| match ty.as_adt() {
                ...
                // convert Vec<Option<semantics::Enum>> into Option<Vec<semantics::Enum>>.
                _ => ty.is_bool().then(|| ExtendedEnum::Bool),
            })
        })
        .collect()
...
fn build_pat(db: &RootDatabase, module: semantics::Module, var: ExtendedVariant) -> Option<ast::Pat> {
    match var {
