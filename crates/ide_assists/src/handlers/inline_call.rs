use ast::make;
use either::Either;
use semantics::{db::HirDatabase, PathResolution, Semantics, TypeInfo};
use ide_db::{
    ...
    let (function, label) = match &call_info.node {
        ast::CallableExpr::Call(call) => {
            ...
            let function = match ctx.sema.resolve_path(&path)? {
                PathResolution::Def(semantics::ModuleDef::Function(f)) => f,
                PathResolution::AssocItem(semantics::AssocItem::Function(f)) => f,
                _ => return None,
            };
...
) -> Option<Vec<(ast::Pat, Option<ast::Type>, semantics::Param)>> {
...
fn inline(
    ...
    params: &[(ast::Pat, Option<ast::Type>, semantics::Param)],
