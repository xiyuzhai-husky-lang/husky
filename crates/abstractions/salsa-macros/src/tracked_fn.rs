...
pub(crate) fn tracked_impl(
    ...
    let extra_impls = item_impl
        ...
        .filter_map(|item| {
            ...
            let salsa_tracked_attr_index = item_method.attrs.iter().position(|attr| {
            ...
            let salsa_tracked_attr = item_method.attrs.remove(salsa_tracked_attr_index);
...
fn fn_configuration(args: &FnArgs, item_fn: &syn::ItemFn) -> Configuration {
    ...
    let (cycle_strategy, recover_fn) = if let Some(recovery_fn) = &args.recovery_fn {
        ...
        let cycle_fullback = parse_quote! {
            fn recover_from_cycle(__db: &::salsa::Db, __cycle: &salsa::Cycle, __id: Self::Key) -> Self::Value {
...
fn set_lru_capacity_fn(
    ...
    let lru_fn = parse_quote! {
        #[allow(dead_code, clippy::needless_lifetimes)]
        fn set_lru_capacity(__db: &::salsa::Db, __value: usize) {
            let (__jar, __runtime) = __db.jar::<#jar_ty>();
            let __ingredients =
...
fn make_fn_return_ref(fn_sig: &mut syn::Signature) -> syn::Result<()> {
    ...
    let (right_arrow, elem) = match fn_sig.output.clone() {
        ReturnType::Default => (
            syn::Token![->](fn_sig.paren_token.span.span()),
            parse_quote!(()),
        ),
...
/// Given a function signature, identifies the name given to the `&::salsa::Db` reference
