...
impl<T> Idx<T> {
    ...
    pub(crate) fn from_dec(raw: RawIdx) -> Self {
        Idx {
            raw,
            _ty: PhantomData,
        }
...
impl<T> Iterator for IdxRange<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.range.next().map(|raw| Idx::from_dec(raw.into()))
    }
}

impl<T> DoubleEndedIterator for IdxRange<T> {
    fn next_back(&mut self) -> Option<Self::Item> {
        self.range.next_back().map(|raw| Idx::from_dec(raw.into()))
    }
}
...
impl<T> Arena<T> {
    ...
    pub fn iter(
        ...
        self.data
            .iter()
            .enumerate()
            .map(|(idx, value)| (Idx::from_dec(RawIdx(idx as u32)), value))
    }
    ...
    fn next_idx(&self) -> Idx<T> {
        Idx::from_dec(RawIdx(self.data.len() as u32))
    }
}
