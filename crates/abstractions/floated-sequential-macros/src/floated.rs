...
pub(crate) fn floated(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let from_ref = match fields.len() {
        1 => {
            ...
            quote! {
                ...
                impl<'db> #ty_ident<'db> {
                    #vis fn from_ref<Q: Eq + std::hash::Hash + ?Sized>(q: &Q, db: &'db ::floated_sequential::db::FloaterDb) -> Self
                    where
    ...
    let expanded = quote! {
        ...
        #vis struct #ty_ident<'db>(::floated_sequential::Floated<'db, #data_ty_ident>);

        impl<'db> #ty_ident<'db> {
            #vis fn new(#(#ctor_params),*, db: &'db ::floated_sequential::db::FloaterDb) -> Self {
