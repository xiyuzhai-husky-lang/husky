...
/// A "route" is a function that, given a `&Jars`, returns an `&dyn Ingredient`.
...
#[allow(unused_parens)]
pub type DynRoute = dyn Fn(&Jars) -> (&dyn Ingredient) + Send + Sync;

/// Like a `DynRoute`, but for `&mut` references.
#[allow(type_alias_bounds)]
#[allow(unused_parens)]
pub type DynMutRoute = dyn Fn(&mut Jars) -> (&mut dyn Ingredient) + Send + Sync;
...
pub struct Routes {
    ...
    /// a function which can be applied to the `Jars` to yield
    /// the `dyn Ingredient.
    #[allow(clippy::type_complexity)]
    routes: Vec<(Box<DynRoute>, Box<DynMutRoute>)>,
...
impl Routes {
    ...
    pub fn push<I>(
        &mut self,
        route: impl (Fn(&Jars) -> &I) + Send + Sync + 'static,
        mut_route: impl (Fn(&mut Jars) -> &mut I) + Send + Sync + 'static,
    ) -> IngredientIndex
    where
        I: Ingredient + IngredientRequiresReset + 'static,
    {
    ...
    pub fn route(&self, index: IngredientIndex) -> &dyn Fn(&Jars) -> &dyn Ingredient {
    ...
    pub fn route_mut(&self, index: IngredientIndex) -> &dyn Fn(&mut Jars) -> &mut dyn Ingredient {
    ...
    pub fn reset_routes(
        &self,
    ) -> impl Iterator<Item = &dyn Fn(&mut Jars) -> &mut dyn Ingredient> + '_ {
