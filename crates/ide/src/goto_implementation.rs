use semantics::{AsAssocItem, Impl, Semantics};
use ide_db::{
...
fn impls_for_ty(sema: &Semantics<RootDatabase>, ty: semantics::Type) -> Vec<NavigationTarget> {
    Impl::all_for_type(sema.db, ty).into_iter().filter_map(|imp| imp.try_to_nav(sema.db)).collect()
}

fn impls_for_trait(sema: &Semantics<RootDatabase>, trait_: semantics::Trait) -> Vec<NavigationTarget> {
...
fn impls_for_trait_item(
    ...
    trait_: semantics::Trait,
    fun_name: semantics::Name,
