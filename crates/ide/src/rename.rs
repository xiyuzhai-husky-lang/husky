...
use semantics::{AsAssocItem, InFile, Semantics};
use ide_db::{
    base_db::FileID,
...
fn rename_to_self(sema: &Semantics<RootDatabase>, local: semantics::Local) -> RenameResult<SourceChange> {
    if never!(local.is_self(sema.db)) {
        bail!("rename_to_self invoked on self");
    }

    let fn_def = match local.parent(sema.db) {
        semantics::DefWithBody::Function(func) => func,
    ...
    let impl_ = match assoc_item.container(sema.db) {
        semantics::AssocItemContainer::Trait(_) => {
            bail!("Cannot rename parameter to self for trait functions");
        }
        semantics::AssocItemContainer::Impl(impl_) => impl_,
    };
...
fn rename_self_to_param(
    sema: &Semantics<RootDatabase>,
    local: semantics::Local,
    self_param: semantics::SelfParam,
    new_name: &str,
) -> RenameResult<SourceChange> {
    if new_name == "self" {
