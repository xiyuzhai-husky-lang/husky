...
use ast::HasName;
use cfg::CfgExpr;
use semantics::{AsAssocItem, HasAttrs, HasSource, HirDisplay, Semantics};
...
pub(crate) fn runnables(db: &RootDatabase, file_id: FileID) -> Vec<Runnable> {
    ...
    let mut in_macro_expansion = FxHashMap::<semantics::HirFileID, Vec<Runnable>>::default();
        ...
        if let Definition::SelfType(impl_) = def {
            impl_.items(db).into_iter().for_each(|assoc| {
                let runnable = match assoc {
                    semantics::AssocItem::Function(it) => {
                        runnable_fn(&sema, it).or_else(|| module_def_doctest(sema.db, it.into()))
                    }
                    semantics::AssocItem::Const(it) => module_def_doctest(sema.db, it.into()),
                    semantics::AssocItem::TypeAlias(it) => module_def_doctest(sema.db, it.into()),
                };
...
fn find_related_tests_in_module(
    ...
    syntax: &SyntaxNode,
    fn_def: &ast::Fn,
    parent_module: &semantics::Module,
    tests: &mut FxHashSet<Runnable>,
) {
    ...
    let range = match &mod_source.value {
        semantics::ModuleSource::Module(m) => m.syntax().text_range(),
        semantics::ModuleSource::BlockExpr(b) => b.syntax().text_range(),
        semantics::ModuleSource::SourceFile(f) => f.syntax().text_range(),
    };
...
fn parent_test_module(sema: &Semantics<RootDatabase>, fn_def: &ast::Fn) -> Option<semantics::Module> {
...
pub(crate) fn runnable_fn(sema: &Semantics<RootDatabase>, def: semantics::Function) -> Option<Runnable> {
    ...
    let kind = if name == "main" && def.module(sema.db) == root {
        ...
        let test_id = || {
            let canonical_path = {
                let def: semantics::ModuleDef = def.into();
...
pub(crate) fn runnable_mod(sema: &Semantics<RootDatabase>, def: semantics::Module) -> Option<Runnable> {
...
pub(crate) fn runnable_impl(sema: &Semantics<RootDatabase>, def: &semantics::Impl) -> Option<Runnable> {
    let attrs = def.attrs(sema.db);
...
fn runnable_mod_outline_definition(
    ...
    match def.definition_source(sema.db).value {
        semantics::ModuleSource::SourceFile(_) => Some(Runnable {
...
fn module_def_doctest(db: &RootDatabase, def: Definition) -> Option<Runnable> {
    ...
    let path = (|| {
        ...
        if let Some(assoc_item) = def.as_assoc_item(db) {
            if let semantics::AssocItemContainer::Impl(imp) = assoc_item.container(db) {
...
fn has_runnable_doc_test(attrs: &semantics::Attrs) -> bool {
...
fn has_test_function_or_multiple_test_submodules(
    ...
    for item in module.declarations(sema.db) {
        match item {
            ...
            semantics::ModuleDef::Module(submodule) => {
