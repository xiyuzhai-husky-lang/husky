...
use semantics::{db::HirDatabase, Adt, AsAssocItem, AssocItem, AssocItemContainer, Crate, HasAttrs};
use ide_db::{
...
pub(crate) fn extract_definitions_from_docs(
    docs: &semantics::Documentation,
) -> Vec<(TextRange, String, Option<semantics::Namespace>)> {
...
pub(crate) fn doc_attributes(
    ...
    node: &SyntaxNode,
) -> Option<(semantics::AttrsWithOwner, Definition)> {
    match_ast! {
        match node {
            ...
            ast::Struct(it)      => sema.to_def(&it).map(|def| (def.attrs(sema.db), Definition::Adt(semantics::Adt::Struct(def)))),
            ast::Union(it)       => sema.to_def(&it).map(|def| (def.attrs(sema.db), Definition::Adt(semantics::Adt::Union(def)))),
            ast::Enum(it)        => sema.to_def(&it).map(|def| (def.attrs(sema.db), Definition::Adt(semantics::Adt::Enum(def)))),
    ...
    let res = match def {
        ...
        Definition::Field(field) => {
            let def = match field.parent_def(db) {
                semantics::VariantDef::Struct(it) => Definition::Adt(it.into()),
                semantics::VariantDef::Union(it) => Definition::Adt(it.into()),
                semantics::VariantDef::Variant(it) => Definition::Variant(it),
            };
            let (_, file, _) = filename_and_frag_for_def(db, def)?;
...
///                                                       ^^^^^^^^^^^^^^
/// ```
fn get_assoc_item_fragment(db: &dyn HirDatabase, assoc_item: semantics::AssocItem) -> Option<String> {
...
"#,
    ...
    fn def_under_cursor(
        sema: &Semantics<RootDatabase>,
        position: &FilePosition,
    ) -> (Definition, semantics::Documentation) {
        let (docs, def) = sema
    ...
    fn node_to_def(
        sema: &Semantics<RootDatabase>,
        node: &SyntaxNode,
    ) -> Option<Option<(Option<semantics::Documentation>, Definition)>> {
        Some(match_ast! {
            match node {
                ...
                ast::Struct(it)      => sema.to_def(&it).map(|def| (def.docs(sema.db), Definition::Adt(semantics::Adt::Struct(def)))),
                ast::Union(it)       => sema.to_def(&it).map(|def| (def.docs(sema.db), Definition::Adt(semantics::Adt::Union(def)))),
                ast::Enum(it)        => sema.to_def(&it).map(|def| (def.docs(sema.db), Definition::Adt(semantics::Adt::Enum(def)))),
