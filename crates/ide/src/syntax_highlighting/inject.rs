...
use either::Either;
use semantics::{InFile, Semantics};
use ide_db::{
...
fn find_doc_string_in_attr(attr: &semantics::Attr, it: &ast::Attr) -> Option<ast::String> {
    match it.expr() {
        // #[doc = lit]
...
fn module_def_to_hl_tag(def: Definition) -> HlTag {
    let symbol = match def {
        ...
        Definition::Adt(semantics::Adt::Struct(_)) => SymbolKind::Struct,
        Definition::Adt(semantics::Adt::Enum(_)) => SymbolKind::Enum,
        Definition::Adt(semantics::Adt::Union(_)) => SymbolKind::Union,
