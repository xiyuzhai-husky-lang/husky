...
use semantics::{AsAssocItem, HasVisibility, Semantics};
use ide_db::{
...
pub(super) fn element(
    ...
    krate: Option<semantics::Crate>,
    bindings_shadow_count: &mut FxHashMap<semantics::Name, u32>,
...
fn highlight_name_ref_in_attr(sema: &Semantics<RootDatabase>, name_ref: ast::NameRef) -> Highlight {
    match NameRefClass::classify(sema, &name_ref) {
        Some(name_class) => match name_class {
            ...
            NameRefClass::Definition(Definition::Macro(m)) if m.kind() == semantics::MacroKind::Attr => {
                    ...
                    Definition::Field(field) => {
                        if let Some(parent) = name_ref.syntax().parent() {
                            if matches!(parent.kind(), FIELD_EXPR | RECORD_PAT_FIELD) {
                                if let semantics::VariantDef::Union(_) = field.parent_def(db) {
                                    h |= HlMod::Unsafe;
                                }
                            }
                        }
...
fn highlight_name(
    ...
    match name_kind {
        ...
        Some(NameClass::PatFieldShorthand { field_ref, .. }) => {
            ...
            if let semantics::VariantDef::Union(_) = field_ref.parent_def(db) {
                h |= HlMod::Unsafe;
            }
            h
        }
...
fn calc_binding_hash(name: &semantics::Name, shadow_count: u32) -> u64 {
...
fn highlight_def(
    ...
    let mut h = match def {
            ...
            if let Some(item) = func.as_assoc_item(db) {
                ...
                match func.self_param(db) {
                    Some(sp) => match sp.access(db) {
                        semantics::Access::Exclusive => {
                            h |= HlMod::Mutable;
                            h |= HlMod::Reference;
                        }
                        semantics::Access::Shared => h |= HlMod::Reference,
                        semantics::Access::Owned => h |= HlMod::Consuming,
                    },
                    None => h |= HlMod::Static,
                }

                match item.container(db) {
                    semantics::AssocItemContainer::Impl(i) => {
                        if i.trait_(db).is_some() {
                            h |= HlMod::Trait;
                        }
                    }
                    semantics::AssocItemContainer::Trait(_t) => {
                        h |= HlMod::Trait;
                    }
                }
            }
        ...
        Definition::Adt(adt) => {
            let h = match adt {
                semantics::Adt::Struct(_) => HlTag::Symbol(SymbolKind::Struct),
                semantics::Adt::Enum(_) => HlTag::Symbol(SymbolKind::Enum),
                semantics::Adt::Union(_) => HlTag::Symbol(SymbolKind::Union),
            };
        ...
        Definition::GenericParam(it) => match it {
            semantics::GenericParam::TypeParam(_) => Highlight::new(HlTag::Symbol(SymbolKind::TypeParam)),
            semantics::GenericParam::ConstParam(_) => {
                Highlight::new(HlTag::Symbol(SymbolKind::ConstParam))
            }
            semantics::GenericParam::LifetimeParam(_) => {
    ...
    let def_crate = def.module(db).map(semantics::Module::krate).or_else(|| match def {
    ...
    let is_public = def.visibility(db) == Some(semantics::Visibility::Public);
...
fn highlight_method_call(
    ...
    let is_public = func.visibility(sema.db) == semantics::Visibility::Public;
    ...
    match parent.kind() {
        ...
        FIELD_EXPR => {
            ...
            let is_union = ast::FieldExpr::cast(parent)
                ...
                .map_or(false, |field| {
                    matches!(field.parent_def(sema.db), semantics::VariantDef::Union(_))
                });
            if is_union {
                h | HlMod::Unsafe
            } else {
...
fn is_consumed_lvalue(node: &SyntaxNode, local: &semantics::Local, db: &RootDatabase) -> bool {
