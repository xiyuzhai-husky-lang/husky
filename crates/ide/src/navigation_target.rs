...
impl NavigationTarget {
    ...
    pub(crate) fn from_module_to_decl(db: &RootDatabase, module: semantics::Module) -> NavigationTarget {
...
impl TryToNav for semantics::ModuleDef {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        match self {
            semantics::ModuleDef::Module(it) => Some(it.to_nav(db)),
            semantics::ModuleDef::Function(it) => it.try_to_nav(db),
            semantics::ModuleDef::Adt(it) => it.try_to_nav(db),
            semantics::ModuleDef::Variant(it) => it.try_to_nav(db),
            semantics::ModuleDef::Const(it) => it.try_to_nav(db),
            semantics::ModuleDef::Static(it) => it.try_to_nav(db),
            semantics::ModuleDef::Trait(it) => it.try_to_nav(db),
            semantics::ModuleDef::TypeAlias(it) => it.try_to_nav(db),
            semantics::ModuleDef::BuiltinType(_) => None,
        }
    }
}
...
impl ToNavFromAst for semantics::Function {
    const KIND: SymbolKind = SymbolKind::Function;
}
impl ToNavFromAst for semantics::Const {
    const KIND: SymbolKind = SymbolKind::Const;
}
impl ToNavFromAst for semantics::Static {
    const KIND: SymbolKind = SymbolKind::Static;
}
impl ToNavFromAst for semantics::Struct {
    const KIND: SymbolKind = SymbolKind::Struct;
}
impl ToNavFromAst for semantics::Enum {
    const KIND: SymbolKind = SymbolKind::Enum;
}
impl ToNavFromAst for semantics::Variant {
    const KIND: SymbolKind = SymbolKind::Variant;
}
impl ToNavFromAst for semantics::Union {
    const KIND: SymbolKind = SymbolKind::Union;
}
impl ToNavFromAst for semantics::TypeAlias {
    const KIND: SymbolKind = SymbolKind::TypeAlias;
}
impl ToNavFromAst for semantics::Trait {
...
impl ToNav for semantics::Module {
    fn to_nav(&self, db: &RootDatabase) -> NavigationTarget {
...
impl TryToNav for semantics::Impl {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        let src = self.source(db)?;
...
impl TryToNav for semantics::Field {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        let src = self.source(db)?;
...
impl TryToNav for semantics::MacroDef {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        let src = self.source(db)?;
...
impl TryToNav for semantics::Adt {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        match self {
            semantics::Adt::Struct(it) => it.try_to_nav(db),
            semantics::Adt::Union(it) => it.try_to_nav(db),
            semantics::Adt::Enum(it) => it.try_to_nav(db),
        }
    }
}

impl TryToNav for semantics::AssocItem {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        match self {
...
impl TryToNav for semantics::GenericParam {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        match self {
            semantics::GenericParam::TypeParam(it) => it.try_to_nav(db),
            semantics::GenericParam::ConstParam(it) => it.try_to_nav(db),
            semantics::GenericParam::LifetimeParam(it) => it.try_to_nav(db),
        }
    }
}

impl ToNav for semantics::Local {
    fn to_nav(&self, db: &RootDatabase) -> NavigationTarget {
        let src = self.source(db);
...
impl ToNav for semantics::Label {
    fn to_nav(&self, db: &RootDatabase) -> NavigationTarget {
        let src = self.source(db);
...
impl TryToNav for semantics::TypeParam {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        let src = self.source(db)?;
...
impl TryToNav for semantics::LifetimeParam {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        let src = self.source(db)?;
...
impl TryToNav for semantics::ConstParam {
    fn try_to_nav(&self, db: &RootDatabase) -> Option<NavigationTarget> {
        let src = self.source(db)?;
        let full_range = src.value.syntax().text_range();
        Some(NavigationTarget {
