...
pub(super) fn parse_intra_doc_link(s: &str) -> (&str, Option<semantics::Namespace>) {
    let s = s.trim_matches('`');

    [
        (semantics::Namespace::Types, (TYPES.0.iter(), TYPES.1.iter())),
        (semantics::Namespace::Values, (VALUES.0.iter(), VALUES.1.iter())),
        (semantics::Namespace::Macros, (MACROS.0.iter(), MACROS.1.iter())),
    ]
    .into_iter()
