...
use either::Either;
use semantics::{HasSource, Semantics};
use ide_db::{
    base_db::FileRange,
    defs::Definition,
...
impl HoverAction {
    fn goto_type_from_targets(db: &RootDatabase, targets: Vec<semantics::ModuleDef>) -> Self {
        let targets = targets
            .into_iter()
            .filter_map(|it| {
...
pub(crate) fn hover(
    ...
    let sema = &semantics::Semantics::new(db);
...
fn goto_type_action_for_def(db: &RootDatabase, def: Definition) -> Option<HoverAction> {
    let mut targets: Vec<semantics::ModuleDef> = Vec::new();
    let mut push_new_def = |item: semantics::ModuleDef| {
        if !targets.contains(&item) {
            targets.push(item);
        }
    };

    if let Definition::GenericParam(semantics::GenericParam::TypeParam(it)) = def {
        it.trait_bounds(db).into_iter().for_each(|it| push_new_def(it.into()));
    } else {
        let ty = match def {
            Definition::Local(it) => it.ty(db),
            Definition::GenericParam(semantics::GenericParam::ConstParam(it)) => it.ty(db),
...
fn walk_and_push_ty(
    db: &RootDatabase,
    ty: &semantics::Type,
    push_new_def: &mut dyn FnMut(semantics::ModuleDef),
) {
    ty.walk(db, |t| {
