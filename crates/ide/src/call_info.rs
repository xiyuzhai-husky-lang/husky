...
use either::Either;
use semantics::{HasAttrs, HirDisplay, Semantics};
...
pub(crate) fn call_info(db: &RootDatabase, position: FilePosition) -> Option<CallInfo> {
    ...
    match callable.kind() {
        semantics::CallableKind::Function(func) => {
            res.doc = func.docs(db).map(|it| it.into());
            format_to!(res.signature, "fn {}", func.name(db));
        }
        semantics::CallableKind::TupleStruct(strukt) => {
            res.doc = strukt.docs(db).map(|it| it.into());
            format_to!(res.signature, "struct {}", strukt.name(db));
        }
        semantics::CallableKind::TupleEnumVariant(variant) => {
        ...
        semantics::CallableKind::Closure => (),
    }
    ...
    match callable.kind() {
        semantics::CallableKind::Function(_) | semantics::CallableKind::Closure => {
            let ret_type = callable.return_type();
            if !ret_type.is_unit() {
                format_to!(res.signature, " -> {}", ret_type.display(db));
            }
        }
        semantics::CallableKind::TupleStruct(_) | semantics::CallableKind::TupleEnumVariant(_) => {}
    }
    Some(res)
}
