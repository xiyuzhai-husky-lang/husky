use either::Either;
use semantics::{known, Callable, HasVisibility, HirDisplay, Semantics, TypeInfo};
    ...
    if next == SyntaxKind::WHITESPACE {
        ...
        if next_next == T![.] {
            ...
            if matches!(expr, ast::Expr::PathExpr(_)) {
                if let Some(semantics::Adt::Struct(st)) = ty.as_adt() {
...
fn is_named_constructor(
    ...
    if let Some(semantics::CallableKind::TupleStruct(_) | semantics::CallableKind::TupleEnumVariant(_)) =
        callable_kind
    {
...
fn hint_iterator(
    ...
    if !(strukt.visibility(db) == semantics::Visibility::Public
    ...
    if ty.impls_trait(db, iter_trait, &[]) {
        let assoc_type_item = iter_trait.items(db).into_iter().find_map(|item| match item {
            semantics::AssocItem::TypeAlias(alias) if alias.name(db) == known::Item => Some(alias),
            _ => None,
        })?;
...
fn pat_is_enum_variant(db: &RootDatabase, bind_pat: &ast::IdentPat, pat_ty: &semantics::Type) -> bool {
    if let Some(semantics::Adt::Enum(enum_data)) = pat_ty.as_adt() {
...
fn should_not_display_type_hint(
    ...
    if let Some(semantics::Adt::Struct(s)) = pat_ty.as_adt() {
...
fn should_hide_param_name_hint(
    ...
    let fn_name = match callable.kind() {
        semantics::CallableKind::Function(it) => Some(it.name(sema.db).to_smol_str()),
        _ => None,
    };
...
fn get_callable(
    sema: &Semantics<RootDatabase>,
    expr: &ast::Expr,
) -> Option<(semantics::Callable, ast::ArgList)> {
    match expr {
