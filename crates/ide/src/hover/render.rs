...
use either::Either;
use semantics::{AsAssocItem, HasAttrs, HasSource, HirDisplay, Semantics, TypeInfo};
use ide_db::{
...
pub(super) fn try_expr(
    ...
    if let Some((semantics::Adt::Enum(inner), semantics::Adt::Enum(body))) = adts {
...
fn definition_owner_name(db: &RootDatabase, def: &Definition) -> Option<String> {
    match def {
        ...
        Definition::Function(f) => match f.as_assoc_item(db)?.container(db) {
            semantics::AssocItemContainer::Trait(t) => Some(t.name(db)),
            semantics::AssocItemContainer::Impl(i) => i.self_ty(db).as_adt().map(|adt| adt.name(db)),
        },
...
pub(super) fn path(db: &RootDatabase, module: semantics::Module, item_name: Option<String>) -> String {
    let crate_name =
...
fn label_and_docs<D>(db: &RootDatabase, def: D) -> (String, Option<semantics::Documentation>)
where
...
fn label_value_and_docs<D, E, V>(
    ...
    def: D,
    value_extractor: E,
) -> (String, Option<semantics::Documentation>)
where
    D: HasAttrs + HirDisplay,
    E: Fn(&D) -> Option<V>,
    V: Display,
...
fn builtin(famous_defs: &FamousDefs, builtin: semantics::BuiltinType) -> Option<Markup> {
...
fn find_std_module(famous_defs: &FamousDefs, name: &str) -> Option<semantics::Module> {
...
fn local(db: &RootDatabase, it: semantics::Local) -> Option<Markup> {
    let ty = it.ty(db);
