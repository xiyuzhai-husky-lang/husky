use folded::Generator;
use folded::{FoldedList, FoldedStorage};
use std::sync::Arc;

use crate::{error::ExprResultArc, parser::ExprParser, *};

pub type ExprText = FoldedList<ExprResult>;

#[salsa::query_group(ExprQueryStorage)]
pub trait ExprQuery: atom::AtomQuery {
    fn expr_text(&self, id: file::FileId) -> ExprResultArc<ExprText>;
}

fn expr_text(this: &dyn ExprQuery, id: file::FileId) -> ExprResultArc<ExprText> {
    let atomized_text = this.atomized_text(id)?;
    let mut parser = ExprParser::new();
    parser.transform_all(atomized_text.folded_iter(0));
    Ok(Arc::new(parser.take_folded_results()))
}
