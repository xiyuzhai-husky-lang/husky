...
impl HuskySource {
  ...
  pub fn new(filepath: PathBuf) -> Result<HuskySource, ParserError> {
    let mut source = HuskySource {
      filepath,
      lines: vec![],
      indents: vec![],
    };
...
pub struct SourceTree {
  pub key: String,
  pub source: HuskySource,
  pub children: Vec<SourceTree>,
}
impl SourceTree {
  pub fn new_package(folderpath: PathBuf) -> Result<SourceTree, ParserError> {
    ...
    let mut sources = SourceTree {
      key: package_name,
      source: HuskySource::new(mainpath)?,
      children: vec![],
    };
    sources.load();
    Ok(sources)
  }
  fn new_module(path: PathBuf) -> Option<SourceTree> {
    ...
    if module_path.is_dir() {
      ...
      if module_path.is_file() {
        let mut sources = SourceTree {
          key: module_name,
          source: HuskySource::new(module_path).expect("success"),
          children: vec![],
        };
        sources.load();
        Some(sources)
    ...
    if self.source.filepath.ends_with("mod.hsk") || self.source.filepath.ends_with("main.hsk") {
      ...
      for entry in std::fs::read_dir(folderpath).expect("success") {
        ...
        if !path.ends_with("mod.hsk") && !path.ends_with("main.hsk") {
          if let Some(sources) = SourceTree::new_module(path) {
  ...
  // pub fn get_child(&self, key: &String) -> &SourceTree {
  //     td!()
  // }
}
