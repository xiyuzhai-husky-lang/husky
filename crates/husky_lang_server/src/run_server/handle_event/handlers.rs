...
use lsp_types::{
    ...
    CodeLens, CompletionItem, Diagnostic, DocumentFormattingParams, FoldingRange,
    FoldingRangeParams, Location, NumberOrString, Position, PrepareRenameResponse, RenameParams,
...
pub(crate) fn handle_memory_usage(state: &mut Server, _: ()) -> Result<String> {
    ...
    let mut mem = state.ide_db_proxy.per_query_memory_usage();
...
pub(crate) fn handle_folding_range(
    ...
    let folds = snapshot.analysis.folding_ranges(file_id)?;
    let text = snapshot.analysis.file_text(file_id)?;
    let line_index = snapshot.file_line_collection(file_id)?;
