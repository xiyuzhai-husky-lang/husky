...
pub(crate) fn extend_end<'eval>(ct: &'eval crate::raw_contour::RawContour<'eval>, start: i32, r: f32) -> i32 {
    ...
    let mut r_max = 0f32;
    while end <= max_end && right_bound.rotation_direction_to(&dp) >= 0i32 && dp.rotation_direction_to(&left_bound) >= 0i32 {
        let dp_norm = dp.norm();
        if dp_norm < r_max - r {
            break;
...
pub(crate) fn extend_start<'eval>(ct: &'eval crate::raw_contour::RawContour<'eval>, start0: i32, end: i32, r: f32) -> i32 {
    ...
    while start >= min_start {
        ...
        if right_bound.rotation_direction_to(&left_bound) >= 0i32 {
            if start <= start0 && !(right_bound.rotation_direction_to(&dp) >= 0i32 && dp.rotation_direction_to(&left_bound) >= 0i32) {
                break;
            }
            start -= 1;
        } else {
...
pub(crate) fn find_line_segments<'eval>(ct: &'eval crate::raw_contour::RawContour<'eval>, r: f32) -> Vec<crate::line_segment_sketch::LineSegment<'eval>> {
    ...
    while end <= max_end {
        ...
        if line_segments.ilen() > 0i32 {
            ...
            if dp_extend_end.cross(&dp_previous).abs() < 0.01f32 && dp_extend_end.dot(&dp_previous) > 0f32 {
                let N = ct.points.ilen();
                line_segments.last() = crate::line_segment_sketch::LineSegment::new(ct, line_segments.last().points.start, end);
    ...
    if last_line_segment.points.end >= first_line_segment_points_end + N {
        line_segments.pop();
        line_segments.first() = crate::line_segment_sketch::LineSegment::new(ct, last_line_segment.points.start - N, line_segments.first().points.end - 1i32);
    }
