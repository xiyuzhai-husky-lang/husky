...
pub(crate) fn find_line_segments<'eval>(
    ...
    while end <= max_end {
        ...
        if line_segments.ilen() > 0i32 {
            ...
            {
                let N = ct.points.ilen();
                line_segments.last_mut().unwrap() = crate::line_segment_sketch::LineSegment::new(
                    ct,
                    line_segments.last().unwrap().points.start,
                    end,
                );
        ...
        if extend_start_flag {
            ...
            if line_segments.ilen() > 0i32 {
                ...
                {
                    line_segments.pop();
                    ls = crate::line_segment_sketch::LineSegment::new(
                        ct,
    ...
    if last_line_segment.points.end >= first_line_segment_points_end + N {
        line_segments.pop();
        line_segments.first_mut().unwrap() = crate::line_segment_sketch::LineSegment::new(
            ct,
            last_line_segment.points.start - N,
            line_segments.first().unwrap().points.end - 1i32,
        );
    }
