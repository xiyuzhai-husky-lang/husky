...
pub fn quick_sort<T: Ord>(ref mut arr: &mut [T]) {
    let len = &arr.len();
    crate::quick_sort_aux(&mut arr, 0, (len - 1) as isize)
}

#[rustfmt::skip]
pub fn quick_sort_aux<T: Ord>(ref mut arr: &mut [T], low: isize, high: isize) {
    if low < high {
        let p = crate::partition(&mut arr, low, high);
        crate::quick_sort_aux(&mut arr, low, p - 1);
        crate::quick_sort_aux(&mut arr, p + 1, high)
    }
}
...
pub fn partition<T: Ord>(ref mut arr: &mut [T], low: isize, high: isize) -> isize {
    ...
    while true {
        ...
        if store_index >= last_index {
            ...
            &mut arr.swap(store_index as usize, last_index as usize)
        }
    }
    &mut arr.swap(store_index as usize, pivot as usize);
    store_index
}
