...
pub fn partition<T: Ord>(ref mut arr: &mut [T], low: isize, high: isize) -> isize {
    ...
    while true {
        store_index += 1;
        while arr.index(store_index as usize as usize) < arr.index(pivot as usize) {
            store_index += 1
        }
        last_index -= 1;
        while last_index >= 0 && arr.index(last_index as usize as usize) > arr.index(pivot as usize) {
            last_index -= 1
        }
