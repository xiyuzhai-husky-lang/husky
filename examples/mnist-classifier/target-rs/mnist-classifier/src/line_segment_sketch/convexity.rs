...
pub fn is_convex(line_segment_sketch: &crate::line_segment_sketch::LineSegmentSketch, index: i32) -> bool {
    ...
    let current_displacement = line_segment_sketch.strokes.index(index.rem_euclid(L) as usize).displacement();
    let previous_displacement = line_segment_sketch.strokes.index((index - 1).rem_euclid(L) as usize).displacement();
    ...
    if is_rotation_counterclockwise_result == 0 {
        ...
        let previous_interval = line_segment_sketch.strokes.index((index - 1).rem_euclid(L) as usize).points;
        ...
        let current_interval = line_segment_sketch.strokes.index(index.rem_euclid(L) as usize).points;
