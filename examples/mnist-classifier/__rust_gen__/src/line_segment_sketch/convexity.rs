...
pub(crate) fn is_convex<'eval>(
    line_segment_sketch: &crate::line_segment_sketch::LineSegmentSketch<'eval>,
    index: i32,
) -> bool {
    let L = line_segment_sketch.strokes.ilen();
    let current_displacement =
        line_segment_sketch.strokes[(index.rem_euclid(L)) as usize].displacement();
    let previous_displacement =
        line_segment_sketch.strokes[((index - 1).rem_euclid(L)) as usize].displacement();
    ...
    if is_rotation_counterclockwise_result == 0 {
        ...
        let previous_interval =
            &line_segment_sketch.strokes[((index - 1).rem_euclid(L)) as usize].points;
        for i1 in previous_interval.start..previous_interval.end {
            let displacement = line_segment_sketch
                .contour
                .displacement(previous_interval.start, i1);
        ...
        for i2 in current_interval.start..current_interval.end {
            let displacement = line_segment_sketch
                .contour
                .displacement(previous_interval.start, i2);
