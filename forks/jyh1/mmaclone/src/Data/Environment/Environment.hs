...
insertPattern lhs rhs = pattern %~ addPatt (List lhs, rhs)

insertValue :: [LispVal] -> LispVal -> Down -> Down
insertValue lhs rhs = value %~ M.insert (List lhs) rhs
...
updateDownValue val rhs =
  ...
  in M.insertWith update name initial

updateContext :: LispVal -> LispVal -> Context -> Context
updateContext (Atom name) rhs = own %~ M.insert name rhs
updateContext val@(List _) rhs = down %~ updateDownValue val rhs
...
getLhs (List [Atom "Condition", p, t]) = List [Atom "Condition", getLhs p, t]
...
replaceOwnValue val@(Atom name) ownVal = fromMaybe val $ M.lookup name ownVal

replaceContext :: LispVal -> Context -> Primi
replaceContext val@(Atom _) con = return
  $ con ^. own . to (replaceOwnValue val)
replaceContext val@(List _) con = con ^. down . to (replaceDownValue val)
