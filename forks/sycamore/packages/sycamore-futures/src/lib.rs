...
/// Spawns a `!Send` future on the current visibility. If the visibility is destroyed before the future is
/// completed, it is aborted immediately. This ensures that it is impossible to access any
/// values referencing the visibility after they are destroyed.
pub fn spawn_local_scoped<'a>(cx: Scope<'a>, f: impl Future<Output = ()> + 'a) {
    ...
    // immediately aborted once the reactive visibility is dropped.
